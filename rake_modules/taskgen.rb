  ...
  def setup_tox
    ...
    namespace :tox do
      if @changed_files.include?('tox.ini') || @changed_files.include?('rake_modules/taskgen.rb')
        ...
        openstack_files = filter_files_by("modules/openstack/files/**")
        unless openstack_files.empty?
          desc 'Run tox for openstack puppet enc'
          task :openstack do
            res = system("tox -e openstack")
            raise 'Tests for openstack failed!'.red unless res
          end
          tasks << 'tox:openstack'
        end
