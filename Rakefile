...
def run_module_spec(module_name)
    ...
    Dir.chdir("modules/#{module_name}") do
        ...
        system('rake spec')
        # Cleanup to remove unwanted 'to commit' files
        system("rm #{link_name}") unless link_exists
        system("rm #{site_file_name}") unless site_file_name_exists
        Dir.rmdir(manifests_directory) unless manifests_directory_exists
        Dir.rmdir(fixture_directory) unless fixture_directory_exists
    end
end
