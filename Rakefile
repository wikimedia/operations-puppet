...
task :typos do
    def gnu_grep(cmd='grep')
        return cmd if system(
            "#{cmd} --version|grep '(GNU grep)'",
            { :out => '/dev/null' , :err => '/dev/null' })
    end
    cmd = gnu_grep || gnu_grep('ggrep')
    raise 'Can not find GNU grep (grep or ggrep).' unless cmd

    system("#{cmd} -E -r --binary-files=without-match --color=always --file=typos --exclude=typos --exclude-dir=.git .")
