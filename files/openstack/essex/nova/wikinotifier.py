...
class WikiStatus(object):
    ...
    def _wiki_login(self):
        if not self._wiki_logged_in:
            if not self.site:
	        LOG.debug(" -- Andrew, trying to connect to %s" % FLAGS.wiki_host)
                self.site = mwclient.Site(("https",FLAGS.wiki_host),
                                          retry_timeout=5,
                                          max_retries=3)
            if self.site:
	        LOG.debug(" -- Andrew, trying to log in to %s using %s and %s" % (FLAGS.wiki_host, FLAGS.wiki_login, FLAGS.wiki_password))
	...
	LOG.debug(" -- Andrew, here are some config settings: %s %s" % (FLAGS.wiki_host, FLAGS.wiki_page_prefix))
        ...
        if event_type == 'compute.instance.delete.end':
            page_string = "\n%s\n%s\n%s\n" % (begin_comment,
                                          _("This instance has been deleted."),
                                              end_comment)
        else:
            template_param_dict = {}
