...
def parseCommandLine():
    ...
    parser.add_argument('-c','--config', help='Specify yaml config file. Default /etc/skrill.yaml')
    ...
    try:
        ...
        if forall == True:
            ...
            for database in dbtuple:
                ...
                print "Output from {0}:{1} on {2} : {3}".format( instanceHost, str(instancePort), str(database), str(rows) )
...
def main():
    ...
    for instance in executionInstances:
        ...
        executeQuery(instanceHost, instancePort, instancePassword, args["query"], args["forall"])
