...
if [ ! -f $FLAG ]; then
    ...
    if ( grep "eqiad" /etc/resolv.conf ); then
        ...
        echo "Clearing out resolv.conf"
        # resolf.conf will contain eqiad.wmflabs but it's otherwise scrambled by the
        #  change of venue.  So, wipe it out and reboot for a fresh start.
        rm /etc/resolv.conf
