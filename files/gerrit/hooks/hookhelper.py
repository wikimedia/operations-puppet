...
class HookHelper:
	...
	def match(patterns, name):
		...
		iswildcard = lambda string: "*" in string # returns bool if the parameter contains a "*"
		wildcardpatterns = filter(iswildcard, patterns.keys()) # filter out the non-wildcard patterns
		wildcardpatterns = sorted(wildcardpatterns, key=len, reverse=True) # sort patterns by length, longest first
	...
	def log_to_file(self, project, branch, message):
		...
		for fname in logs:
			...
			if type(self).__name__ == "CommentAdded":
				if os.path.split(fname)[1] in hookconfig.nocommentlogs:
					continue
	...
	def get_log_filename(self, project, branch, message):
		...
		logs = isinstance(logs, basestring) and [logs] or logs
		return [os.path.join(hookconfig.logdir, log) for log in logs]
