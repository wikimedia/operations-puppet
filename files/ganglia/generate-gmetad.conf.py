...
def cond_restart_gmetad():
  ...
  if filecmp.cmp(gconf, gconfnew):
    syslog.syslog("INFO:", gdaemon, "config has not changed; no reload needed")
  else:
    ...
    if os.system(grestart) == 0:
      syslog.syslog("INFO:", gdaemon, "new config loaded successfully")
    else:
      shutil.copyfile(gconfbk, gconf)
      # verify revert attempt
      if os.system(grestart) == 0:
        syslog.syslog("WARN:", gdaemon, "failed to load new config, but",
                      "has been successfully reverted")
        sys.exit(1)
      else:
        syslog.syslog("ERROR:", gdaemon, "failed to load new config, and",
                      "revert failed;", gdaemon, "is not running!")
        sys.exit(2)
