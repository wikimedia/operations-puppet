...
if ($ACTION =~ /^s/) {
    ...
    my $mntastat=&Backticks("/usr/bin/stat", "-f", "-c", "%i", "/mnt/a");
    my $mntstat=&Backticks("/usr/bin/stat", "-f", "-c", "%i", "/mnt");
    if ( $mntastat ne $mntstat ) {
        $dummy=&Backticks("/bin/umount", "/mnt/a");
    }
    $dummy=&Backticks("/bin/mount", "-o", "noatime,nobarrier,nouuid", "/dev/tank/$SNAME", "/mnt/a");
    exit 0;
}
else {
