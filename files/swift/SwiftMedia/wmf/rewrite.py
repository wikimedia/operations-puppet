    ...
    def handle404(self, reqorig, url, container, obj):
        ...
        try:
            ...
            if self.backend_url_format == 'sitelang':
                match = re.match(r'^http://(?P<host>[^/]+)/(?P<proj>[^-/]+)/(?P<lang>[^/]+)/(?P<path>.+)', encodedurl)
                if match:
                    proj = match.group('proj')
                    lang = match.group('lang')
                    #TODO import all the strange host manglings from thumb_handler
                    ...
                    encodedurl = 'http://%s/%s' % (hostname, match.group('path'))
