...
class _WMFRewriteContext(WSGIContext):
    ...
    def handle404(self, reqorig, url, container, obj):
        ...
        try:
            ...
            if self.backend_url_format == 'sitelang':
                ...
                re = (r'^http://(?P<host>[^/]+)/(?P<proj>[^-/]+)/(?P<lang>[^/]+)/thumb/(?P<path>.+)')  # noqa
                ...
                match = re.match(r'^http://(?P<host>[^/]+)/(?P<proj>[^-/]+)/(?P<lang>[^/]+)/thumb/(?P<path>.+)',  # noqa
                                 encodedurl)
                if match:
                    proj = match.group('proj')
                    lang = match.group('lang')
            ...
            class CopiedHTTPError(webob.exc.HTTPError):
                ...
                def __init__(self):
                    super(CopiedHTTPError,
                          self).__init__(detail="".join(error.readlines()),
    ...
    def handle_request(self, env, start_response):
        ...
        # regular uploads
        match = re.match(r'^/(?P<proj>[^/]+)/(?P<lang>[^/]+)/((?P<zone>transcoded|thumb|temp)/)?(?P<path>((temp|archive)/)?[0-9a-f]/(?P<shard>[0-9a-f]{2})/.+)$',  # noqa
                         req.path)
        if match:
            proj = match.group('proj')
            lang = match.group('lang')
        ...
        if match is None:
            ...
            match = re.match(r'^/(?P<proj>[^/]+)/(?P<lang>[^/]+)/(?P<repo>timeline)/(?P<path>.+)$',  # noqa
                             req.path)
            if match:
        ...
        if match is None:
            ...
            #  (legacy)
            match = re.match(r'^(/(?P<proj>[^/]+)/(?P<lang>[^/]+))?/(?P<repo>math)/(?P<path>(?P<shard1>[0-9a-f])/(?P<shard2>[0-9a-f])/.+)$',  # noqa
                             req.path)
