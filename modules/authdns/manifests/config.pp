define authdns_template($destdir) {
    file { "${destdir}/${title}":
        ensure  => 'present',
        owner   => 'root',
        group   => 'root',
...
class authdns::config {
    ...
    $staging_dir   = "${staging_root}/staging"  # managed, but not copied
    $staging_etc   = "${staging_dir}/etc"       # -> /etc/gdnsd/
    $staging_zones = "${staging_dir_etc}/zones" # -> /etc/gdnsd/zones/
...
<% done -%>')
    ...
    authdns_template { $config_templates: destdir => $staging_etc }
    authdns_file { $config_files: destdir => $staging_etc }
    authdns_file { $var_files: destdir => $staging_var }
    authdns_zone { $zone_files: destdir => $staging_zones }
}
