...
class bugzilla (
	...
	exec { 'check setup':
		command => "perl checksetup.pl ${dir}/answerfile.pl",
		creates => "${install_dir}/localconfig",
		cwd     => $install_dir,
		path    => '/usr/bin',
		require => [ File['answer file'], Exec['extra modules'] ],
		user    => $user,
	}
	...
	exec { 'run setup':
		...
		cwd       => $install_dir,
		path      => '/usr/bin',
		require   => Exec['check setup'],
		subscribe => Exec['check setup'],
		user      => $user,
	}
}
