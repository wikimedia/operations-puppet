...
class wikistats::db {
    ...
    $wikistats::backupdir = '/root/wsbackup'

    # db backup
    cron { 'mysql-dump-wikistats':
        ensure  => 'present',
        command => "/usr/bin/mysqldump -u root -p$ wikistats > ${wikistats::backupdir}/wikistats_db_${date}.sql && gzip ${backupdir}/wikistats_db_${date}.sql",
        user    => 'root',
        hour    => '0',
        minute  => '15',
    }
