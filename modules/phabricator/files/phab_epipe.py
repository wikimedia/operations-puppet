...
class WMFEmailParsingError(Exception):
    pass
...
def main():
    ...
    if sys.stdin.isatty():
        raise WMFEmailParsingError('no stdin')
    ...
    def mail2task(sender, date, subject, body, project, security):
        ...
        if not project_info['data']:
            raise WMFEmailParsingError("project %s does not exist" % (project))

        return phab.maniphest.createtask(title=subject,
                                         description=block,
                                         projectPHIDs=[project_info['data'].keys()[0]],
                                         auxiliary={"std:maniphest:security_topic": security})
        ...
        for payload in msg.get_payload():
            ...
            if payload.get_content_type() == 'text/plain':
                ...
                if not isinstance(msgparts, list):
                    body = 'unknown body format: is not multipart/alternative list'
                    continue
                else:
                    for e in msgparts:
                        known_body_types = ['text/plain']
                        # text/plain; charset="utf-8"
                        # Inconsistent content-types so we try to match only basic string
                        if e['Content-Type'].split(';')[0] in known_body_types:
                            body = e.get_payload()
                            break
                    else:
                        err = 'Unknown email body format: from %s to %s'
                        raise WMFEmailParsingError(error % (src_address, str(dest_addresses)))
    ...
    if dtask:
        ...
        if src_address.count('@') > 1:
            raise WMFEmailParsingError('invalid source address: %s' % (src_address))
        ...
        if dtask_match is None:
            raise WMFEmailParsingError('no matching direct task allowed project')

        src_domain = src_addy.split('@')[1]
        if not src_domain in direct_com[dtask_match] and not '*':
            raise WMFEmailParsingError('direct comment not allowed from this domain')

        log(mail2comment(dtask, src_name, msg['date'], msg['subject'], body, uploads))
...
if __name__ == '__main__':
    ...
    except WMFEmailParsingError as e:
        syslog.syslog("WMFEmailParsingError: %s" % (str(e)))
        print "%s\n\n%s" % (e, contact)
        sys.exit(1)
