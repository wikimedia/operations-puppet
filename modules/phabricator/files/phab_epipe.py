...
* address routing allows redirecting an email address to create new tasks in project
...
def main():
    ...
    for name, value in parser.items('direct_comments_allowed'):
        direct_comments_allowed[name] = value.split(',')
    ...
    def external_user_comment(task, email, text):
        ...
        :returns: json
        :note: https://secure.phabricator.com/book/phabricator/article/remarkup/
        """
    ...
    routed_addresses = [address for address in address_routing.keys() if address in to_addresses]
    ...
    if dtask:
        ...
        def dtask_project(project_list):
            """determine if task has an associate project
            :param project_list: array of project phid's
            :returns: string
            """
            for p in project_list:
                pname = phab.phid.info(phid=p)['name']
                if pname in direct_comments_allowed.keys():
                    return pname
            return ''

        dtask_match = dtask_project(proj_info)
        if dtask_match and src_address.split('@')[1] in direct_comments_allowed[dtask_match]:
            log(external_user_comment(dtask, src_address, body))
        else:
            log('direct task did not match conditions: %s' % (dtask,))
        ...
        handler = os.path.join(parser.get(parser_mode, 'root_dir'), "scripts/mail/mail_handler.php")
