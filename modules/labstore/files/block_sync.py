...
def run_remote(cmd, host):
    remote_cmd = 'ssh -i /root/.ssh/id_labstore {}@{} "{}"'.format(
        R_USER, host, cmd)
...
def bdsync(local_device, r_host, r_vg, r_snapshot_name):
    remotenice = 10
    blocksize = 16384
    server = 'nice -{} {} --server'.format(remotenice, BDSYNC)
    remdata = 'ssh -i /root/.ssh/id_labstore {}@{} "{}"'.format(R_USER, r_host, server)
    sync_cmd = '{} --blocksize={} --remdata \'{}\' {} /dev/{}/{}'.format(
            BDSYNC, blocksize, remdata, local_device, r_vg, r_snapshot_name)
    progress_cmd = 'pv -p -t -e -r -a -b'
...
if __name__ == '__main__':
    ...
    try:
        ...
        run_remote('test -e {}'.format(BDSYNC), args.r_host)
        run_remote('test -e {}'.format(SNAPSHOT_MGR), args.r_host)
        run_local('test -e {}'.format(SNAPSHOT_MGR))
