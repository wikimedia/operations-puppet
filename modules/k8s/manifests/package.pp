...
define k8s::package (
    ...
    # Ensure the actual package and require 'apt-get update' to be run first (apt_package_from_component_${component_title})
    # This is usually done by the apt::package_from_component, but since we don't ensure any packages there it's not guaranteed
    # to have happened already.
    ensure_packages($package_name, {
        'require' => [Apt::Package_from_component[$component_title], Exec["apt_package_from_component_${component_title}"]],
