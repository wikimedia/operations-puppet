...
# at this point, if no hostname, try to fetch it from openstack itself
if [ "$(hostname)" == "localhost" ] ; then
    echo "Trying to fetch hostname from Openstack"
    hostname=$(curl -s http://169.254.169.254/openstack/latest/meta_data.json/ | sed -r 's/^.*name\": \"//'  | sed -r 's/\".*$//g')
    hostname $hostname
fi
...
then
	master="labtest-puppetmaster.wikimedia.org"
