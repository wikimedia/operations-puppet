...
define planet::cronjob {
    ...
    if $planet::planet_http_proxy
      $use_proxy = "http_proxy=\"${planet::planet_http_proxy}\" https_proxy=\$http_proxy"
    } else {
      $use_proxy = ''
    }
    ...
    cron { "update-${title}-planet":
        ...
        command => "${use_proxy} ${planet_bin} ${planet_cmd} > ${planet_logfile} 2>&1",
        user    => 'planet',
        minute  => $minute,
        require => [
