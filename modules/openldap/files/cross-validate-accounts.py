...
def main():
    ...
    os.chdir(tmp_dir)
    try:
        x = subprocess.check_output(["git", "clone", "http://gerrit.wikimedia.org:/r/p/operations/puppet.git"], stderr=subprocess.STDOUT, shell=False)
    ...
    for table in ['users', 'ldap_only_users']:
        for username, userdata in yamldata[table].items():
            ...
            if userdata.get('email', None) == None:
    ...
    if sorted(get_ldap_group_members('ops')) != sorted(yamldata['groups']['ops']['members'] + yamldata['groups']['datacenter-ops']['members']):
        print "Membership of ops group in LDAP and YAML (consisting of ops and datacenter-ops) is not identical"
    ...
    for i in users.keys():
        ...
        if attrs.has_key('ldap_only') and not attrs['ldap_only']:
            if "ops" in attrs['prod_groups']:
                if len(attrs['prod_groups']) < 2:
                    print_error(i, "malformed membership for ops user, needs at least 'gitpuppet' and 'ops'")
                elif len(attrs['prod_groups']) > 2:
                    # ops and gitpuppet are default for all ops users
                    # analytics-privatedata-users enables mysql access in addition to cluster-wide root permissions, so might be used in addition to ops privs
                    # researchers enables mysql access in addition to cluster-wide root permissions, so might be used in addition to ops privs
                    ...
                    for j in ['gitpuppet', 'ops', 'researchers', 'statistics-privatedata-users', 'analytics-privatedata-users', 'deploy-phabricator', 'analytics-search-users']:
                        if attrs['prod_groups'].count(j) > 0:
                            attrs['prod_groups'].remove(j)
                    if len(attrs['prod_groups']) > 0:
                        print_error(i, "malformed membership for ops user, has additional group memberships other than 'gerritpuppet' and 'ops':")
