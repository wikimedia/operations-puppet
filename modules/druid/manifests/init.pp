...
#
#   CREATE DATABASE $druid_metadata_database DEFAULT CHARACTER SET utf8;
#   GRANT ALL ON $druid_metadata_database.*
#       TO '$druid_metadata_username'@'%'
#       IDENTIFIED BY '$druid_metadata_password';
#
#
...
)
{
    ...
    # Default to using derby for metadata storage.
        ...
        $default_metadata_properties = {
            ...
            'druid.metadata.storage.connector.connectURI' => inline_template(
                'jdbc:mysql://<%= @properties.fetch("druid.metadata.storage.connector.host", "localhost") %>:<%= @properties.fetch("druid.metadata.storage.connector.port", "1527") %>/var/lib/druid/metadata.db;create=true'
            )
        }
    ...
    if $properties['druid.storage.type'] == 'hdfs' {
        $default_deep_storage_properties = {
            # Make sure this directory exists in HDFS!
            'druid.storage.storageDirectory' => '/user/druid/deep-storage',
         }

         # Load the hdfs storage extension
         $storage_extensions = ['druid-hdfs-storage']
    }
    # If deep storage type is local, default to storing it in /var/lib/druid.
    elsif $properties['druid.storage.type'] == 'local' {
        $default_deep_storage_properties = {
