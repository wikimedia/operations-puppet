...
export PATH="/data/wdbuilder/composer/vendor/bin/:$PATH"
...
if [ "${build_exit_value}" -eq "0" ] ; then
	...
	cp -r /data/wdbuilder/wikidata/.git /data/wdbuilder/wikidata-tmp/
	...
	GLOBIGNORE=.:..:.git cp -r /data/wdbuilder/buildresources/{*,.*} /data/wdbuilder/wikidata-tmp/
