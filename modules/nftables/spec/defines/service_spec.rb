  ...
  on_supported_os(WMFConfig.test_on).each do |os, facts|
    context "on #{os}" do
        ...
        it do
          is_expected.to contain_file('/etc/nftables/input/10_test_service.nft')
            ...
            .with_content(/
                    #\sManaged\sby\spuppet\s+
                    #\ssome\sdesc\s+
                    ip\ssaddr\s{\s192.0.2.1\s}\sip\sdaddr\s{\s192.0.2.2\s}\stcp\sdport\s{\s80,\s443\s}\saccept\s+
                    /x)
        end
      end
      ...
      describe 'Empty list of source IPs' do
        ...
        it { is_expected.to compile.with_all_deps }
        it do
          is_expected.not_to contain_file('/etc/nftables/input/10_test_service.nft')
        end
      end
      ...
      describe "QOS rsyncd" do
        let(:params) do
          super().merge(
            src_ips: %w[208.80.153.7 208.80.154.13 208.80.154.144 2620:0:860:1:208:80:153:7 2620:0:861:1:208:80:154:13 2620:0:861:2:208:80:154:144],
            proto: 'tcp',
            desc: 'some desc',
            port: [873, 1873],
            qos: 'low'
      ...
      describe 'QOS Empty list of destination IPs' do
        ...
        it do
          is_expected.not_to contain_file('/etc/nftables/input/10_test_service.nft')
          is_expected.not_to contain_file('/etc/nftables/postrouting/10_test_service_service_low.nft')
        end
      end
      describe "notrack no dst_ips" do
        ...
        it do
          is_expected.not_to contain_file('/etc/nftables/input/10_test_service.nft')
          is_expected.not_to contain_file('/etc/nftables/notrack/10_test_service.nft')
        end
      end
