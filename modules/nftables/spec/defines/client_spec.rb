  ...
  on_supported_os(WMFConfig.test_on).each do |os, facts|
    context "on #{os}" do
      ...
      describe "basic destination filter with QOS" do
        let(:params) { super().merge(dst_ips: ['192.0.2.1', 'fe80::1'], qos: 'low') }
        it { is_expected.to compile.with_all_deps }
        it do
          is_expected.to contain_file('/etc/nftables/output/10_test_client.nft')
            .with_content(<<~EOF)
              # Managed by puppet
              # some desc
        ...
        it do
          is_expected.to contain_file('/etc/nftables/postrouting/10_test_client_client_low.nft')
            .with_content(<<~EOF)
              # Managed by puppet
              # some desc
