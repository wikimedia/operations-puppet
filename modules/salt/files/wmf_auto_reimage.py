...
def run_puppet(hosts):
    ...
    """
    success_hosts = []
    logger.info("Executing 'wmfpuppet.run agent test' on {hosts}".format(
        hosts=hosts))
...
def reimage_hosts(puppetmaster_host, hosts, custom_mgmts, ipmi_password):
    ...
    for host, result in proxy_command(
            ...
            audit_commands=audit_commands, timeout=7200):
...
def run(args, user):
    ...
    if not args.no_reboot:
        ...
        hosts = wait_puppet_run(hosts, start=reboot_time)
