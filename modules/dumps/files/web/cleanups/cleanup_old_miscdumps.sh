...
for entry in $config_entries; do
  IFS_SAVE=$IFS
  IFS=','
  read subdir keep <<<$entry
  IFS=$IFS_SAVE

  if [ ! -d "$subdir" ]; then
      echo "subdir $subdir does not existing, skipping"
      continue
  elif [[ ! "$keep" =~ ^[0-9]+$ ]]; then
  ...
  runs=$( cd "$dumpsdir"; ls -d [0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9] | sort )
  if [ -z "$runs" ]; then
      continue
  fi
  ...
  unwanted=${numruns[@]:0:${num_unwanted}}
  for $dirname in unwanted; do
      if[ -n "$dryrun" ]; then
	  echo "for subdir $subdir, would rm -f $dirname"
      else
	  rm -f "$dirname"
      fi
  done
