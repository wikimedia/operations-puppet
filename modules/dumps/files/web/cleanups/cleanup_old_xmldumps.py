...
class DumpsCleaner(object):
    ...
    def get_dates_from_filenames(filenames):
        '''
        given a list of filenames with date string -YYYYMMDD- somewhere
        in them, return a list of these date strings, sorted, with
    ...
    def cleanup_wiki_latestfiles(self, wiki):
        ...
        links_by_date, dates = self.get_dates_from_filenames(latestlinks)
        if len(dates) <= 2:
            # nothing to do
            return
        ...
        cutoff = dates[-2]
        path = os.path.join(self.dumpsdir, wiki, 'latest')
        for (filename, filedate) in links_by_date:
            if filedate < cutoff:
                if self.dryrun:
