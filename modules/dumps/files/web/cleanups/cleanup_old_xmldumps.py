...
If prefetch files are to be saved for some number of dumps,
remove all but prefetch files from older dumps.
"""
...
Usage: cleanup_old_xmldumps.py --keeps_conffile path --wikilists dir
           [--preserve_list path] [--subdirs] [--dryrun] [--help]
...
Options:
  ...
  --preserve_list (-p): list of regexes of filenames which
...
def get_keeps(keeps_conffile):
    ...
    for line in lines:
        ...
        if ':' in keepvalue:
            keepvalue, prefetches = keepvalue.split(':', 1)
        else:
            prefetches = '0'
        keepvalue = keepvalue.strip()
        partialkeepvalue = prefetches.strip()
...
class DumpsCleaner(object):
    ...
    def __init__(self, keeps_conffile, preservelist_path, wikilists_dir,
        ...
        self.keeps_per_list, self.prefetches_per_list = get_keeps(keeps_conffile)
    ...
    def get_prefetches_for_wiki(self, wiki):
        """
        find the prefetches value for the specified wiki
        by checking through the various prefetches conf settings,
        falling back to the default setting, if there is one
        if no setting is found, return None
        """
        for wikilist in self.wikilists:
            if wiki in self.wikilists[wikilist]:
                return self.prefetches_per_list[wikilist]
        if 'default' in self.prefetches_per_list:
            return self.prefetches_per_list['default']
        return None

    def cleanup_all_but_prefetch(self, wiki, dirname):
        """
        remove eveything but prefetch files and status files
        from specified dump directory for the given wiki
        """
        dirpath = os.path.join(self.dumpsdir, wiki, dirname)
        files = os.listdir(dirpath)
        files = [filename for filename in files
                 if not is_keeper(filename, self.preserve_patterns) and
                 not file_is_redundant(filename, files)]
        for filename in files:
    ...
    def cleanup_dirs(self, wiki, dirs, partial=False):
        ...
        for dirname in dirs:
            if partial:
                self.cleanup_all_but_prefetch(wiki, dirname)
            else:
    ...
    def cleanup_wiki(self, keeps, prefetches, wiki):
        """
        remove oldest dumps if we have more than the number to keep;
        keep prefetch files around for the specified number of most
        ...
        if to_remove <= 0:
            return
        if prefetches:
            remove_completely = to_remove - int(prefetches)
        if remove_completely > 0:
            self.cleanup_dirs(wiki, dumps[0:remove_completely])
        else:
            remove_completely = 0
        self.cleanup_dirs(wiki,
                          dumps[remove_completely, remove_completely + int(prefetches)],
                          partial=True)
    ...
    def clean(self):
        ...
        for wiki in self.wikistoclean:
            ...
            if tokeep is None:
                continue
            prefetches = self.get_prefetches_for_wiki(wiki)
            if prefetches is None:
                prefetches = 0  # no setting available
            self.cleanup_wiki(tokeep, prefetches, wiki)
