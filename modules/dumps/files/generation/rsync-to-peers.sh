...
while [ 1 ]; do
    ...
    /usr/bin/find "${xmldumpsdir}/public/" -maxdepth 3 -regextype sed -regex '.*/*\.\(html\|json\|txt\)' | /usr/bin/xargs /bin/tar cvfp "$tarballpath"
	...
	# send tarball over last, remote can unpack it when it notices the arrival
	# this way, content of status and html files always reflects dump output already
	# made available via rsync
	/usr/bin/rsync -a  --contimeout=600 --timeout=600 "${tarballpath}.gz" "$dest" > /dev/null 2>&1
    done
