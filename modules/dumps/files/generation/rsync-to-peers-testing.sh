...
while [ 1 ]; do
    ...
    for dest in $xmlremotedirs_list; do
        echo /usr/bin/rsync -a  --contimeout=600 --timeout=600 --exclude='**bad/' --exclude='**save/' --exclude='**not/' --exclude='**temp/' --exclude='**tmp/' --exclude='*.inprog'  --exclude='*.html' --exclude='*.txt' --exclude-'*.json' ${xmldumpsdir}/public/*wik* "$dest"
	...
	# made available via rsync
        if [ -f "$tarballpathgz" ]; then
            echo /usr/bin/rsync -a  --contimeout=600 --timeout=600 "$tarballpathgz" "$dest"
        fi
    ...
    for dest in $miscremotedirs_list; do
        echo /usr/bin/rsync -a  --contimeout=600 --timeout=600 ${miscdumpsdir}/* "$dest"
    done
