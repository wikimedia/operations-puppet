...
get_wiki_stats() {
	...
	currentpages=$( /usr/bin/lbzip2 -n 2 -c -d "${wiki}-${date}-pages-articles.xml.bz2" | wc -c )
	...
	currentall=$( /usr/bin/lbzip2 -n 2 -c -d "${wiki}-${date}-pages-meta-current.xml.bz2" | wc -c )
	...
	fullhistory=$( /usr/bin/lbzip2 -n 2 -c -d ${wiki}-${date}-pages-meta-history*.xml*bz2 | wc -c )
