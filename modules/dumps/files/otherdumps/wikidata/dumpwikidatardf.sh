	...
	while [ $i -lt $shards ]; do
		(
			...
			errorLog="/var/log/wikidatadump/dumpwikidata${dumpFormat}-${filename}-${i}.log"
                        "$php" $multiversionscript extensions/Wikidata/extensions/Wikibase/repo/maintenance/dumpRdf.php \
                             --wiki wikidatawiki \
                             --shard "$i" \
                             --sharding-factor "$shards" \
                             --format "$dumpFormat" \
                             --flavor "$dumpFlavor" 2>> "$errorLog" \
                            | "$gzip" -9 > "${tempDir}/wikidata${dumpFormat}-${dumpName}.${i}.gz"
			exitCode=$?
...
while [ $i -lt $shards ]; do
	...
	/bin/cat "$tempFile" >> "${tempDir}/wikidata${dumpFormat}-${dumpName}.gz"
	/bin/rm "$tempFile"
	let i++
done

/bin/mv "${tempDir}/wikidata${dumpFormat}-${dumpName}.gz" "$targetFileGzip"
/bin/ln -fs "$today/$filename.$dumpFormat.gz" "$targetDirBase/latest-$dumpName.$dumpFormat.gz"
...
/bin/ln -fs "${today}/${filename}.${dumpFormat}.bz2" "${targetDirBase}/latest-${dumpName}.${dumpFormat}.bz2"
