...
/bin/ln -s "../wikibase/wikidatawiki/${today}/${filename}.json.gz" "${legacyDirectory}/${today}.json.gz"
find "$legacyDirectory" -name '*.json.gz' -mtime + $( expr $daysToKeep + 1 ) -delete

# (Re-)create the link to the latest
/bin/ln -fs "${today}/${filename}.json.gz" "${targetDirBase}/latest-all.json.gz"
...
/bin/ln -fs "${today}/${filename}.json.bz2" "${targetDirBase}/latest-all.json.bz2"
