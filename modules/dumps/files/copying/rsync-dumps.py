...
def usage(message):
    if message:
        ...
        help_message = """Usage: rsync-dumps.py servers <serverlist> [--dryrun] [--bandwidth <number>] [--list]

Arguments:
    servers    (-s) -- list of servers and their directories associated with each source
                       Format:  source=name,server=name,type=primary;source=name,server=name,type=secondary,dirs=a:b:c;...
                         source: one of the sources listed in this script (currently 'public' or 'other'). default: none
