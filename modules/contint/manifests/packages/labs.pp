...
class contint::packages::labs {
    ...
    file { '/etc/apt/apt.conf.d/51unattended-upgrades-wikimedia':
        ensure => present,
        source => 'puppet:///modules/contint/51unattended-upgrades-wikimedia',
        owner  => 'root',
        group  => 'root',
        mode   => '0444',
    }
    file { '/etc/apt/cron.hourly/apt':
        ensure  => link,
        target  => '/etc/apt/cron.daily/apt',
