    ...
    if ldap_connection:
        ...
        for member in project['member']:
            ...
            if not os.path.exists(local_path):
                logger.info('%s does not exist, symlinking to %s' % (local_path, path))
                os.symlink(path, local_path)

                if not os.path.exists(local_path):
                    logger.error('Failed to symlink %s to %s' % (path, local_path))
                else:
                    logger.info('Chowning %s to %d.%d' % (local_path, uid, gid))
