...
define planet_httpd::theme {
    ...
    if os_version('debian == stretch') {
        ...
        file { "/var/www/planet/${title}/style.css":
            ...
            source => 'puppet:///modules/planet_httpd/theme/rawdog_style.css';
        }
        file { "/var/www/planet/${title}/Wikimedia_Community_Logo.svg":
            ensure => 'present',
            source => 'puppet:///modules/planet_httpd/theme/Wikimedia_Community_Logo.svg';
        }
        file { "/etc/rawdog/theme/wikimedia/${title}/rd_page.tmpl":
            ensure  => 'present',
            content => template('planet_httpd/html/rd_page.html.tmpl.erb');
        }
        file { "/etc/rawdog/theme/wikimedia/${title}/rd_item.tmpl":
            ensure  => 'present',
            content => template('planet_httpd/html/rd_item.html.tmpl.erb');
        }
    } else {
        ...
        file { "${theme_path}/${title}/index.html.tmpl":
            ...
            content => template('planet_httpd/html/index.html.tmpl.erb');
        }
        # theme config file
        file { "${theme_path}/${title}/config.ini":
            source  => 'puppet:///modules/planet_httpd/theme/config.ini';
        }
        # style sheet for planet-venus
        file { "${theme_path}/${title}/planet.css":
            source  => "puppet:///modules/planet_httpd/theme/${css_file}";
        }
    }
}
