...
#
# [*dest*]
#   The output file where to write the result.
#
# [*class_name*]
...
#
# [*instance_prefix*]
#   Jmx instance prefix to target only specific jmx instances.
#   Useful on shared environments where $class_name may pick
#   up all of the jmx instances running.
#   Default: ''
#
# [*labels*]
#   Labels to attach to every host. 'Cluster' will be added automagically as well.
#   Default: {}
#
define prometheus::jmx_exporter_config(
    $dest,
    ...
    $resources = query_resources(
                  "Class[\"${class_name}\"]",
                  "Prometheus::Jmx_exporter_instance[~\"^${instance_prefix}.*\"]",
                  true)
