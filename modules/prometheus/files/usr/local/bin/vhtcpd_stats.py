...
def collect_vhtcp_stats(infile, registry):
    metrics = {
        ...
        'inpkts_recvd': Gauge(
            ...
            registry=registry),
        'inpkts_sane': Gauge(
            'inpkts_sane', 'Packets received sane', namespace='vhtcpd',
            registry=registry),
        'inpkts_enqueued': Gauge(
            'inpkts_enqueued', 'Packets received and enqueued',
            namespace='vhtcpd', registry=registry),
        'inpkts_dequeued': Gauge(
            'inpkts_dequeued', 'Packets dequeued',
            namespace='vhtcpd', registry=registry),
        'queue_overflows': Gauge(
            'queue_overflows', 'Queue overflows', namespace='vhtcpd',
            registry=registry),
        'queue_size': Gauge(
            'queue_size', 'Queue size', namespace='vhtcpd', registry=registry),
        'queue_max_size': Gauge(
            'queue_max_size', 'Queue maximum size', namespace='vhtcpd',
            registry=registry),
    }
