...
# == Define bigtop::hadoop::worker::paths
#
...
#
# == Usage:
# bigtop::hadoop::worker::paths { ['/mnt/hadoop/data/a', '/mnt/hadoop/data/b']: }
#
...
#
define bigtop::hadoop::worker::paths($basedir = $title) {
    Class['bigtop::hadoop'] -> Bigtop::Hadoop::Worker::Paths[$title]

    # hdfs, hadoop, and yarn users
    # are all added by packages
    # installed by bigtop::hadoop
    ...
    $dfs_data_path_parent = inline_template("<%= File.dirname('${::bigtop::hadoop::dfs_data_path}') %>")
    # create DataNode directories
    file { ["${basedir}/${dfs_data_path_parent}", "${basedir}/${::bigtop::hadoop::dfs_data_path}"]:
        ensure  => 'directory',
        owner   => 'hdfs',
        group   => 'hdfs',
        mode    => '0700',
