...
#                                                     rather than in YARN.  Include bigtop::spark::master on this host
#                                                     and bigtop::spark::worker on all standalone Spark Worker nodes.
#                                                     Default: undef
#
...
)
{
    ...
    Class['bigtop::hadoop'] -> Class['bigtop::spark']
    ...
    $config_directory = "/etc/spark/conf.${bigtop::hadoop::cluster_name}"
    # Create the $cluster_name based $config_directory.
    file { $config_directory:
        ensure  => 'directory',
        require => Package['spark-core'],
    }
    bigtop::alternative { 'spark-conf':
        link => '/etc/spark/conf',
    ...
    if !$standalone_enabled or $master_host == $::fqdn {
        ...
        bigtop::hadoop::directory { '/user/spark':
            owner   => 'spark',
            group   => 'spark',
            mode    => '0755',
            require => Package['spark-core'],
        ...
        bigtop::hadoop::directory { '/user/spark/share':
            owner   => 'spark',
            group   => 'spark',
            mode    => '0755',
            require => Bigtop::Hadoop::Directory['/user/spark'],

        }
        bigtop::hadoop::directory { '/user/spark/share/lib':
            owner   => 'spark',
            group   => 'spark',
            mode    => '0755',
            require => Bigtop::Hadoop::Directory['/user/spark/share'],
        }

        bigtop::hadoop::directory { ['/user/spark/applicationHistory']:
            owner   => 'spark',
            group   => 'spark',
            mode    => '1777',
            require => Bigtop::Hadoop::Directory['/user/spark'],
    ...
    $namenode_address = $::bigtop::hadoop::ha_enabled ? {
        ...
        default => $bigtop::hadoop::primary_namenode_host,
    }
    ...
    if !$standalone_enabled {
        ...
        kerberos::exec { 'spark_assembly_jar_install':
            ...
            user    => 'spark',
            require => Bigtop::Hadoop::Directory['/user/spark/share/lib'],
            before  => [
    ...
    file { "${config_directory}/spark-env.sh":
        content => template('bigtop/spark/spark-env.sh.erb'),
    }

    file { "${config_directory}/spark-defaults.conf":
        content => template('bigtop/spark/spark-defaults.conf.erb'),
    }

    file { "${config_directory}/log4j.properties":
        source => 'puppet:///modules/bigtop/spark/log4j.properties',
    }

    $hive_site_symlink_ensure = defined(Class['bigtop::hive']) ? {
        true    => 'link',
        default => 'absent'
    }
