...
class scaservice(
    ...
    $full_config = parseyaml(template("${module_name}/config.yaml.erb"))
    unless is_hash($config) {
        # if a hash is not given, assume it's a YAML string
        $config = parseyaml($config)
    }
    # merge the defaults with the values given by the caller
    $full_config['services'][0]['conf'] = merge(
        $full_config['services'][0]['conf'],
        $config
    )
