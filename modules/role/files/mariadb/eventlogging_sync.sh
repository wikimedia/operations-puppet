...
function usage {
    echo "
${script_name} [-n] [-b <batch-size>] [-D <cutoff-days>] [-d <database>] [-s <slave-database>] <master-host> [<slave-host>]
...
echo "Syncing EventLogging records in $master_host $database -> $slave_host $slave_database..."
# Execute endlessly in an infinite loop.
while true; do

    for table in $($master -e "$master_tables_query and table_name $table_regex order by rand()"); do

        echo -n "$(date +"%Y-%m-%dT%H:%M:%S") $slave_host $slave_database $table"
        ...
        if [ $($slave -e "$slave_tables_query and table_name = '$table'" | wc -l) -eq 0 ]; then
            echo -n ", create"
            if [  $dry_run -eq 1 ]; then
                echo -n " (dry-run) $dump_schema $database $table | $slave $slave_database"
            else
        ...
        if [ $($master -e "$has_id_column_query and table_name = '$table'") -eq 0 ]; then
            column='timestamp'
        fi
        ...
        max_slave=$($slave $slave_database -e "select ifnull(max($column),0) from \`$table\`")
