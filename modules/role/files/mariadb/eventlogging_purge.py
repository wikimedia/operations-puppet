...
def parse_whitelist_file(filepath):
    with open(filepath,'r') as whitelist_fd:
        ...
        allowed_attribute_format = re.compile("^[A-Za-z0-9_\.]+(\[[a-z0-9_]+\])?$")
        for line in whitelist_lines:
            if len(line) != 2:
                raise RuntimeError('Error in the whitelist: too many elements per line')
            ...
            if not allowed_attribute_format.match(attribute):
                ...
                raise RuntimeError('Error in the whitelist: attribute {} not'
                    'following the allowed format (^[A-Za-z0-9_\.]+(\[[a-z0-9_]+\])?$)'
                    .format(attribute))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--whitelist', action='store_true', default="whitelist.tsv",
                        help='The full path of the TSV whitelist file (default: whitelist.tsv)')
    parser.add_argument('--dbport', action='store_true', default=3306,
                        help='The target db port (default: 3306)')
    parser.add_argument('--dbname', action='store_true', default='log',
                        help='The EventLogging database name (default: log)')
    parser.add_argument('--retention', action='store_true', default=90,
