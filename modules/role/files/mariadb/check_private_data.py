...
def get_lists(all_dblist_path, private_dblist_path, deleted_dblist_path,
    ...
    private_dblist = (parse_file(private_dblist_path)
                      + parse_file(deleted_dblist_path))
...
def get_private_tables(db, private_tables, system_dbs, user_databases_regex):
    ...
    query = (('SELECT table_schema, table_name FROM'
             + ' information_schema.tables WHERE table_name IN (%s)')
             % format_strings)
...
def column_has_private_data(conn, database, table, column):
    ...
    try:
        query = (("SELECT count(*) FROM `%s`.`%s` WHERE "
                 + "NOT (`%s` IS NOT NULL OR `%s` <> '')")
