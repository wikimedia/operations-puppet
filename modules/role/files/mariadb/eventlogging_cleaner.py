...
class Terminator(object):
    ...
    def get_interval_factor(self, numrows):
        ...
        number of updated rows, the adapt factor will multiply the interval size
        ...
        return min( # the min ensures that the factor is not too big
            ...
            self.batch_size / float(max(numrows, 1)), # max prevents division by 0
            10
        )
    ...
    def sanitize(self, table):
        ...
        while interval_start < self.end:
            interval_size = self.get_new_interval_size(table, interval_start, interval_size)
...
class TestTerminator(unittest.TestCase):
    ...
    def test_sanitize_whitelist_error_condition(self):
        """
        An exception must be thrown if any of the fields in the whitelist, for
        a given table prefix, is not part of the ones returned by the query to
        the database. This might indicate an error in the whitelist, like a typo,
        that could lead to unnecessary deletes.
        """
        self.terminator.whitelist = {'AwesomeTable': ['field1', 'badtypoinfield']}
        error_msg = (
            "Sanitize has been called for table AwesomeTable, but the field badtypoinfield"
        )
        self.terminator.database.get_table_fields.return_value = ['id', 'uuid', 'field1',
                                                                  'field2', 'field3', 'field4']
        with self.assertRaisesRegex(RuntimeError, error_msg):
