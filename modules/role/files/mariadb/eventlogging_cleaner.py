...
class Database(object):
    ...
    def execute(self, command, dry_run=False):
        ...
        try:
            if dry_run:
                log.info("(DRY-RUN) Executing command: " + command)
                result.update({
                    "success": True,
                    "fields": [],
                    "rows": [],
            ...
            if cursor.rowcount > 0:
                ...
                fields = [] if not cursor.description else [x[0] for x in cursor.description]
...
class Terminator(object):
    ...
    def sanitize(self, table):
        ...
        while uuids_current_batch:
            ...
            uuids_current_batch_escaped = ["'" + x[0] + "'" for x in uuids_current_batch]
...
class TestTerminator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.terminator = Terminator(self.database, [], 120, 90, self.batch_size, dry_run=False)
    ...
    def test_purge(self):
        ...
        expected_result_template = (
            "DELETE FROM AwesomeTable WHERE timestamp >= '{}' AND timestamp < '{}' LIMIT {}"
            .format(self.terminator.start, self.terminator.end, '{}')
        )
        expected_result = expected_result_template.format(self.batch_size)
