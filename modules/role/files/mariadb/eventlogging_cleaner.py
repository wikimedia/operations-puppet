...
class Terminator(object):
    ...
    def sanitize(self, table):
        ...
        while end_ts:
            ...
            elif select_result['numrows'] > self.batch_size:
                log.warning("The table {} has more than batch size events "
                            "(but less than the safe threshold 2 * batch size) "
                            "between {} and {}. This might simply be the "
                            "indication that the batch size is too small, but "
                            "please manually verify to confirm."
                            .format(table, start_ts, end_ts))
...
class TestTerminator(unittest.TestCase):
    ...
    def test_get_last_ts(self):
        timestamps = [str(20170101000000 + ts) for ts in range(400)]
        self.terminator.database.execute.side_effect = [{'rows': timestamps}]
        result = self.terminator._get_last_ts("AwesomeTable", 10)
        expected_result = timestamps[-1][0]
        self.assertEqual(result, expected_result)
