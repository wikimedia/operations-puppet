...
class Terminator(object):
    ...
    def _get_uuids_and_last_ts(self, table, start_ts, override_batch_size=None):
        ...
        if result['rows']:
            ...
            if last_ts == start_ts:
                ...
                log.warn("All events in the batch have the same timestamp ({}) for table {}. "
    ...
    def sanitize(self, table):
        ...
        if len(list(fields_to_purge)) == 0:
            log.warn("No fields to purge for table {}.".format(table))
            return
...
if __name__ == '__main__':
    ...
    try:
        ...
        database = Database('localhost', args.dbname, db_user, db_password,
                            int(args.dbport), unix_socket)
...
class TestDatabase(unittest.TestCase):
    ...
    def test_dry_run(self, mock):
        ...
        db = Database("localhost", "log", "NaNaNaNaNa", 3306, "batman")
