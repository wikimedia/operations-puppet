...
Most of the functions should be run in a continuous loop, maintaining
...
def write_replica_cnf(file_path, uid, mysql_username, pwd):
    ...
    print("removing chattr for %s" % file_path)
...
def populate_new_tools(config):
    ...
    with db.cursor() as cur:
        ...
        for new_tool, new_tool_id in new_tools:
            ...
            write_replica_cnf(
                get_replica_path('tool', new_tool),
                new_tool_id,
                mysql_username,
                pwd
            )
...
def create_accounts(config):
    ...
    for host in config['labsdbs']['hosts']:
        ...
        with db.cursor() as cur:
            cur.execute("""
            SELECT mysql_username, password_hash, username, hostname, account_host.id as account_host_id
