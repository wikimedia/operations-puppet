...
keystone_client = keystoneclient.Client(session=keystone_admin_session, endpoint=config['nova_api_url'])
                ...
                if match:
                    ...
                    if match.group(1) not in server_addresses:
                        ...
                        if recordset['description'] == managed_description:
                            print("Deleting type A record for " + recordset['name'] + " - instance does not exist")
...
for delegated_PTR_FQDN, records in public_PTRs.items():
    if delegated_PTR_FQDN not in existing_public_PTRs and delegated_PTR_FQDN.endswith(floating_ip_zone):
        # Create!
