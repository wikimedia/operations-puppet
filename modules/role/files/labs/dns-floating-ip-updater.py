...
import designateclient.v2 as designateclient
...
argparser.add_argument(
    ...
    default='/etc/labs-floating-ips-dns.yaml',
...
keystone_client = KeystoneClient(session=keystone_admin_session, endpoint=config['nova_api_url'])
...
for tenant in keystone_client.tenants.list():
    ...
    server_addresses = {}
    nova_client = novaclient.Client("1.1", session=keystone_sessions[tenant.name])
    ...
    for zone in designate_client.zones.list():
        ...
        for recordset in designate_client.recordsets.list(zone['id'])
            ...
            if recordset['type'] == 'A': # No IPv6 support in labs so no AAAA
                ...
                if match:
                    ...
                    if match.group(1) not in server_addresses:
                        ...
                        if recordset['description'] == managed_description:
                            ...
                            print("Did not delete A record for " + recordset['name'] + " due to lack of managed_description!")
                        ...
                        if new_records != set(recordset['records']):
                            if recordset['description'] == managed_description:
                                ...
                                print("Did not update A record for " + recordset['name'] + " due to lack of managed_description!")
                else:
                    public_addrs[recordset['name'], tenant.name, False] = [recordset['data']]
...
# Go through each record in the delegated PTR zone, deleting any with our managed_description that don't match our public_PTRs data
    ...
    if recordset['type'] == 'PTR':
        if recordset['name'] not in public_PTRs:
            if recordset['description'] == managed_description:
                ...
                print("Did not delete PTR record for " + recordset['name'] + " due to lack of managed_description!")
            continue
        new_records = set(public_PTRs[recordset['name']])
        if new_records != set(recordset['records']):
            # Effectively, removes any records in the recordset that didn't match our public_PTRs data
            if recordset['description'] == managed_description:
                # Update!
                recordset['records'] = list(new_records)
                wmflabsdotorg_designate_client.recordsets.update(
                ...
                print("Did not update PTR record for " + recordset['name'] + " due to lack of managed_description!")

# Create PTRs in delegated PTR zone
for delegated_PTR_FQDN, records in public_PTRs:
