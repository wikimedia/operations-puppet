...
class role::prometheus::ops {
    ...
    $etcd_jobs = [ {'job_name' => 'etcd', 'file_sd_configs' => [{'files' => [ "${targets_path}/etcd_*.yaml" ]}]} ]
    ...
    prometheus::class_config{ "etcd_mirror_${::site}":
        dest             => "${targets_path}/etcd_mirror_${::site}.yaml",
        site             => $::site,
        class_name       => 'profile::etcd::replication',
        class_parameters => {'active' => true},
        port             => 8000,
    }
