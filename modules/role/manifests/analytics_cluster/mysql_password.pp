...
#
class role::analytics_cluster::mysql_password {
    Class['role::analytics_cluster::client'] -> Class['role::analytics_cluster::mysql_password']
    ...
    exec { 'hdfs_put_mysql-analytics-research-client-pw.txt':
        command => "echo ${::passwords::mysql::research::pass} | /usr/bin/hdfs dfs -put - ${path} && /usr/bin/hdfs dfs -chmod 600 ${path}",
