...
class role::analytics_cluster::refinery::data::drop {
    ...
    cron {"refinery-drop-apiaction-partitions":
        command => "export PYTHONPATH=\${PYTHONPATH}:${role::analytics_cluster::refinery::path}/python && ${role::analytics_cluster::refinery::path}/bin/refinery-drop-eventlogging-partitions -d ${mediawiki_retention_days} -l /wmf/data/raw/mediawiki/mediawiki_ApiAction >> ${mediawiki_log_file} 2>&1",
        user    => 'hdfs',
        minute  => '15',
        hour    => '*/4',
    }
    cron {"refinery-drop-cirrussearchrequestset-partitions":
        command => "export PYTHONPATH=\${PYTHONPATH}:${role::analytics_cluster::refinery::path}/python && ${role::analytics_cluster::refinery::path}/bin/refinery-drop-eventlogging-partitions -d ${mediawiki_retention_days} -l /wmf/data/raw/mediawiki/mediawiki_CirrusSearchRequestSet >> ${mediawiki_log_file} 2>&1",
        user    => 'hdfs',
        minute  => '25',
        hour    => '*/4',
    }
