...
class role::analytics_cluster::refinery::data::drop {
    ...
    cron {'refinery-drop-wdqs-extract-partitions':
        command => "export PYTHONPATH=\${PYTHONPATH}:${role::analytics_cluster::refinery::path}/python && ${role::analytics_cluster::refinery::path}/bin/refinery-drop-wdqs-extract-partitions -d ${wdqs_extract_retention_days} -D wmf -l /wmf/data/wmf/wdqs_extract >> ${wdqs_extract_log_file} 2>&1",
        user    => 'hdfs',
        minute  => '15',
        hour    => '*/4',
    }
