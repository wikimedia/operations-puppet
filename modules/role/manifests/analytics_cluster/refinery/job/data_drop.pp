...
class role::analytics_cluster::refinery::job::data_drop {
    ...
    cron {'refinery-drop-webrequest-sampled-druid':
        command => "${env} && ${role::analytics_cluster::refinery::path}/bin/refinery-drop-druid-deep-storage-data -d ${druid_webrequest_sampled_retention_days} webrequest >> ${$druid_webrequest_log_file} 2>&1",
        environment => "MAILTO=${mail_to}",
        user    => 'hdfs',
        minute  => '15',
        hour    => '5'
