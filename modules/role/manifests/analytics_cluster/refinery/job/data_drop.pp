...
class role::analytics_cluster::refinery::job::data_drop {
    ...
    cron { 'refinery-drop-webrequest-raw-partitions':
        command     => "${env} && ${role::analytics_cluster::refinery::path}/bin/refinery-drop-webrequest-partitions -d ${raw_retention_days} -D wmf_raw -l /wmf/data/raw/webrequest -w raw 2>&1 | tee -a ${webrequest_log_file} ",
    ...
    cron { 'refinery-drop-webrequest-refined-partitions':
        command     => "${env} && ${role::analytics_cluster::refinery::path}/bin/refinery-drop-webrequest-partitions -d ${refined_retention_days} -D wmf -l /wmf/data/wmf/webrequest -w refined 2>&1 | tee -a ${webrequest_log_file}",
        environment => "MAILTO=${mail_to}",
        user        => 'hdfs',
        minute      => '45',
        hour        => '*/4',
    ...
    cron {'refinery-drop-eventlogging-partitions':
        command     => "${env} && ${role::analytics_cluster::refinery::path}/bin/refinery-drop-eventlogging-partitions -d ${eventlogging_retention_days} -l /wmf/data/raw/eventlogging 2>&1 | tee -a ${eventlogging_log_file}",
        environment => "MAILTO=${mail_to}",
        user        => 'hdfs',
        minute      => '15',
        hour        => '*/4',
    ...
    cron {'refinery-drop-wdqs-extract-partitions':
        command     => "${env} && ${role::analytics_cluster::refinery::path}/bin/refinery-drop-hourly-partitions -d ${wdqs_extract_retention_days} -p hive -D wmf -t wdqs_extract -l /wmf/data/wmf/wdqs_extract 2>&1 | tee -a ${wdqs_extract_log_file}",
        environment => "MAILTO=${mail_to}",
        user        => 'hdfs',
        minute      => '0',
        hour        => '1',
    ...
    cron {'refinery-drop-apiaction-partitions':
        command     => "export PYTHONPATH=\${PYTHONPATH}:${role::analytics_cluster::refinery::path}/python && ${role::analytics_cluster::refinery::path}/bin/refinery-drop-hourly-partitions -d ${mediawiki_log_retention_days} -D wmf_raw -t apiaction -p camus -l /wmf/data/raw/mediawiki/mediawiki_ApiAction/hourly 2>&1 | tee -a ${mediawiki_log_file}",
        environment => "MAILTO=${mail_to}",
        user        => 'hdfs',
        minute      => '15',
        hour        => '*/4',
    ...
    cron {'refinery-drop-cirrussearchrequestset-partitions':
        command     => "export PYTHONPATH=\${PYTHONPATH}:${role::analytics_cluster::refinery::path}/python && ${role::analytics_cluster::refinery::path}/bin/refinery-drop-hourly-partitions -d ${mediawiki_log_retention_days} -D wmf_raw -t cirrussearchrequestset -p camus -l /wmf/data/raw/mediawiki/mediawiki_CirrusSearchRequestSet/hourly 2>&1 | tee -a ${mediawiki_log_file}",
        environment => "MAILTO=${mail_to}",
        user        => 'hdfs',
        minute      => '25',
        hour        => '*/4',
    ...
    cron {'refinery-drop-webrequest-sampled-druid':
        command     => "${env} && ${role::analytics_cluster::refinery::path}/bin/refinery-drop-druid-deep-storage-data -d ${druid_webrequest_sampled_retention_days} webrequest 2>&1 | tee -a ${druid_webrequest_log_file}",
        environment => "MAILTO=${mail_to}",
        user        => 'hdfs',
        minute      => '15',
        hour        => '5'
