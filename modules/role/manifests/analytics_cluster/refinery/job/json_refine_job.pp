...
define role::analytics_cluster::refinery::job::json_refine_job (
    ...
    $refinery_job_jar_path = $refinery_job_jar ? {
    ...
    $command = "PYTHONPATH=${refinery_path}/python ${refinery_path}/bin/is-yarn-app-running || /usr/bin/spark-submit --master yarn --deploy-mode client --driver-memory ${spark_driver_memory} --conf spark.dynamicAllocation.maxExecutors=${spark_max_executors} --class org.wikimedia.analytics.refinery.job.JsonRefine --name ${job_name} ${refinery_job_jar_path} --parallelism ${parallelism} --since ${since} ${whitelist_blacklist_opt} ${email_opts} --input-base-path ${input_base_path} --input-regex '${input_regex}' --input-capture '${input_capture}' --output-base-path ${output_base_path} --output-database ${output_database} >> ${log_file} 2>&1"
