...
class role::analytics_cluster::hadoop::worker {
    ...
    if $::realm == 'production' {
        ...
        $hadoop_jvm_warning_threshold  = ($::cdh::hadoop::hadoop_heapsize + 0) * 0.7
        $hadoop_jvm_critical_threshold = ($::cdh::hadoop::hadoop_heapsize + 0) * 0.9
        monitoring::graphite_threshold { 'analytics_hadoop_yarn_nodemanager':
            ...
            from          => '60min',
            warning       => $hadoop_jvm_warning_threshold,
            critical      => $hadoop_jvm_critical_threshold,
            percentage    => '60',
            contact_group => 'admins,analytics',
        }
