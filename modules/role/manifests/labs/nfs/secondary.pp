class role::labs::nfs::secondary($monitor = 'eth0') {
    ...
    $primary_interface_ip = '10.64.37.1'

    $drbd_resource_config = {
        test   => {port => '7790', device => '/dev/drbd1', disk => '/dev/misc/test'},
        tools  => {port => '7791', device => '/dev/drbd4', disk => '/dev/tools/tools-project'},
        others => {port => '7792', device => '/dev/drbd3', disk => '/dev/misc/others'},
    }

    labstore::drbd::resource { keys($drbd_resource_config):
