...
define monitoring::host (
    ...
    case $host_fqdn {
        undef: {
            if ! $ip_address {
                fail("Parameter $ip_address not defined, and no hostname provided!")
            }
            $nagios_address = $ip_address
        }
        default: { $nagios_address = $host_fqdn }
    }
    # If no ip address has been provided, see if a fqdn was provided instead
    if ! $ip_address {
        case $fqdn {
            undef: {

            }
            default: { $ip_address = $fqdn }
        }
    }
