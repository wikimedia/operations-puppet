...
define aptly::repo (
    ...
    if $publish {
        ...
        exec { "publish-aptly-repo-${title}":
            command => "/usr/bin/aptly -architectures=arm64,amd64,all -skip-signing -origin=Wikimedia -label='${title}' -distribution='${title}' -component='${component}' publish repo ${title}",
