class toolforge::k8s::prometheus_metrics_yaml(
    ...
    # this loads admin credentials as generated by kubeadm. In a future iteration
    # we might consider restricting the permissions, since this doesn't require
    # to write anything, just to read it. It requires credentials to be provided
    # in kubeconfig format, which is challenging to generate from puppet though
