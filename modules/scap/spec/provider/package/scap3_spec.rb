...
describe provider_class do
  before do
    ...
    @package_settings = { 'owner' => 'mwdeploy' }
    ...
    # Stub package settings and resolution of uid
    allow(@provider).to receive(:package_settings).and_return(@package_settings)
    allow(Etc).to receive(:getpwnam).
      with(@package_settings['owner']).
      and_return(OpenStruct.new(uid: 666))
  end

  describe '#install' do
    it 'should specify the right repo' do
      expect(@provider).to receive(:deploy_local).with('--repo', 'foo/deploy', '-D', 'log_json:False', uid: 666)
      @provider.install
    end
  end
  ...
  describe '#query' do
    ...
    context 'when the package is not installed' do
      before do
        expect(@provider).to receive(:git) { raise Puppet::ExecutionFailure.new('fail') }
      end
