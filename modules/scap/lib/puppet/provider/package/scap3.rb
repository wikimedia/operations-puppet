...
Puppet::Type.type(:package).provide(
  ...
  def install
    ...
    if deploy_user == "root"
      deploy_local('--repo', repo, '-D', 'log_json:False', 'fetch')
    else
      sudo('-u', deploy_user, self.class.command(:deploy_local),
           '--repo', repo, '-D', 'log_json:False', 'fetch')
    end
  end

  # Queries the current state of a scap3 managed package. The package is
  # assumed to be installed if `git rev-parse HEAD` outputs anything. Once we
  # have some sort of `--diff` functionality in `deploy-local`, we can improve
  # this.
  #
  def query
