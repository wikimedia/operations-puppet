...
Puppet::Type.type(:package).provide(
  ...
  def package_settings=(goal)
    FileUtils.chown_R(goal['owner'], goal['group'], cache_path)
  end
  ...
  def install
    ...
    # NOTE: This is chowning the top level directory. This actually reveals a
    # limitation in the scap3 design in that 2 different repos belonging to the
    # same upper level hierary can not be owned by different users.
    FileUtils.chown(@owner, @group, target_path.sub(%r{/\w+$}, ''))
  end
