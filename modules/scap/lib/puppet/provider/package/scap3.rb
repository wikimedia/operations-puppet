...
Puppet::Type.type(:package).provide(
  ...
  def latest_sha1
    @cached_sha1 || begin
      ...
      # NOTE: This seems like a trebuchet leftover. Assess whether we should
      # still use it with scap3
  ...
  def install
    ...
    git("--work-tree=#{target_path}", 'checkout', '--quiet', latest_sha1)
    FileUtils.chown_R(@owner, @group, target_path)
  end
