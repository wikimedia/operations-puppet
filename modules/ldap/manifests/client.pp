...
# It is recommended that client:nss be included on systems that
# include client::utils, since some scripts use getent for ldap user info
# Remember though, that including client::nss will mean users in the
# ldap database will then be listed as users of the system, so use care.
class client::utils($ldapconfig) {
...
class client::openldap($ldapconfig) {
	package { [ "ldap-utils" ]:
		ensure => latest;
	}
...
class client::autofs($ldapconfig) {
...
class client::includes($ldapincludes, $ldapconfig) {
	if "openldap" in $ldapincludes {
		class { "client::openldap":
