...
class server::iptables-accepts {

	require "server::iptables-purges"
...
class server::iptables-drops {

	require "server::iptables-accepts"
...
class server( $certificate_location, $certificate, $ca_name, $cert_pass, $base_dn, $proxyagent, $proxyagent_pass, $server_bind_ips, $initial_password, $first_master=false ) {
...
-prompt; rm /var/tmp/indexes.cmds";
		...
		'rebuild_indexes':
			...
			refreshonly => true,
			command => "/etc/init.d/opendj stop; su - opendj -c '/usr/opendj/bin/rebuild-index --rebuildAll -b ${server::config::base_dn}'; /etc/init.d/opendj start";				 
