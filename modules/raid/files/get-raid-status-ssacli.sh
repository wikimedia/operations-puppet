...
set +e
CONTROLLERS="$(/usr/bin/sudo /usr/sbin/ssacli controller all show | grep -Eo 'Slot [0-9] ' | cut -d' ' -f2; exit "${PIPESTATUS[0]}")"
EXIT_CODE=$((EXIT_CODE + ${?}))
set -e

while read -r CONTROLLER; do
    # Append the output while exposing the exit code of the ssacli command to the parent shell
    set +e
