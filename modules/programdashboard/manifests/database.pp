...
class programdashboard::database(
    ...
    exec { 'initialize-programdashboard-db':
        ...
        creates => "${datadir}/sql/mysql",
        require => Class['mariadb::config'],
    }

    service { 'mysql':
        ensure  => 'running',
        require => Exec['initialize-programdashboard-db'],
    }
    ...
    exec { 'grant-root-peercred-auth':
        command     => "/usr/bin/mysql --password= --execute=\"${sql}\"",
        refreshonly => true,
        subscribe   => Exec['initialize-programdashboard-db'],
