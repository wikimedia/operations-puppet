...
class CheckService(object):
    ...
    def __init__(self, host, service_name, logstash_search_url, auth='', delay=120, fail_threshold=2):
        """
        Initialize the checker
        """
        self.host = host
        self.service_name = service_name
        self.logstash_search_url = logstash_search_url
        self.auth = auth
        self.delay = delay
    ...
    def _logstashQuery(self):
        return {"facets": {"0": {
                             ...
                             "filter": {"bool": {
                                 ...
                                 "must_not": [
                                     {"fquery": {
                                         ...
                                         "_cache": True}},
                                     {"fquery": {"query": {
                                         "query_string": {"query": "level:(\"WARNING\")"}},
                                         "_cache": True}}
                                 ]}}}}}}
        }}, "size": 20,
            "query": {"filtered": {"query": {
                "query_string": {
                    "query": "type:scap AND "
                    "(channel.raw:scap.announce OR message:\"Started sync_wikiversions\")"}
            },
                "filter": {"bool": {"must": [
                    {"range": {"@timestamp": {"from": "now", "to": "now"}}}]}}}},
...
def main():
    ...
    parser.add_argument('--logstash-search-url',
