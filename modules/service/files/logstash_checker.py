...
class CheckService(object):
    ...
    def cache_dir(self):
        ...
        mw_stage_dir = 'MEDIAWIKI_STAGING_DIR'
        ...
        if self.service_name == 'mwdeploy' and os.path.isfile(mw_profile):
            ...
            with open(mw_profile) as f:
                for line in f:
                    if line.startswith(mw_stage_dir):
                        _, staging_dir = line.split('=', 1)
    ...
    def _save_cached_ts(self, ts):
        ...
        self.logger.debug('Saving cached timestamp: {}'.format(ts))
    ...
    def _rm_cached_ts(self):
        ...
        if os.path.exists(self.cached_timestamp_path):
            self.logger.debug(
                'Removing cached timestamp: {}'.format(
                    self.cached_timestamp_path
                )
            )
    ...
    def _get_cached_ts(self, ts):
        ...
        try:
            ...
            self.logger.debug(
                'Cached timestamp file could not be opened: {}'.format(
                    self.cached_timestamp_path
                )
            )
            return ts
            ...
            with open(self.cached_timestamp_path) as f:
                self.logger.debug(
                    'Cached timestamp file {} is not valid JSON: {}'.format(
                        self.cached_timestamp_path,
                        f.read()
                    )
                )
    ...
    def run(self):
        ...
        final_cutoff_ts = self._get_cached_ts(cutoff_ts)

        counts_before = [entry['doc_count'] for entry in entries
                         if entry['key'] < final_cutoff_ts]
        ...
        if over_threshold:
            ...
            # Don't overwrite the cached timestamp if we're already using one
            # since that will shift the baseline.
            if self.service_name == 'mwdeploy' and not self.using_cached:
