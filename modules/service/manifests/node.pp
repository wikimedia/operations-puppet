...
define service::node(
    ...
    if $deployment == 'scap3' and $deployment_config {
        # check that auto_refresh is not set to true, as Puppet cannot know
        # when and whether the config file has been changed
        if $auto_refresh {
            fail('auto_refresh cannot be enabled for Scap3-driven config deploys')
        }
