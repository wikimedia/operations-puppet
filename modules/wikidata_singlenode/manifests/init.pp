...
# The following are settings for all Wikidata instances (repo and client)
	...
	exec { "populateSitesTable":
			require => [Mediawiki_singlenode::Mw-extension["Wikibase"], File["${install_path}/LocalSettings.php"]],
	...
	exec { "update-script":
			require => [Mediawiki_singlenode::Mw-extension["Wikibase"], Exec["populateSitesTable"]],
			cwd => "$install_path",
	...
	exec { "localisation-cache":
			require => [Mediawiki_singlenode::Mw-extension["Wikibase"], Exec["populateSitesTable"], Exec["update-script"]],
			cwd => "$install_path",
...
# Wikibase client only:
	if $install_client == true {
		...
		exec { "populate_interwiki":
			require => [Mediawiki_singlenode::Mw-extension["Wikibase"], Exec["update-script"]],
			cwd => "$install_path",
		...
		exec { "client_import_data":
			require => [Mediawiki_singlenode::Mw-extension["Wikibase"], File["${install_path}/simple-elements.xml"]],
			cwd => "$install_path",
