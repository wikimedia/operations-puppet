...
class wikidata_singlenode( $install_path = "/srv/mediawiki",
							...
							$install_repo = true,
							$install_client = true,
							$role_requires = ['"$IP/extensions/Diff/Diff.php"', '"$IP/extensions/DataValues/DataValues.php"', '"$IP/extensions/DataTypes/DataTypes.php"', '"$IP/extensions/WikibaseDataModel/WikibaseDataModel.php"', '"$IP/extensions/Wikibase/lib/WikibaseLib.php"'],
...
# The following are settings for all Wikidata instances (repo and client)
	...
	mediawiki_singlenode::mw-extension { [ "Diff", "DataValues", "DataTypes", "WikibaseDataModel" ]:
...
# Wikibase repo only:
	if $install_repo == true {
		...
		mediawiki_singlenode::mw-extension { [ "Wikibase", "UniversalLanguageSelector", "Babel", "Translate", "AbuseFilter" ]:
			require => [Git::Clone["mediawiki"], Exec["mediawiki_setup"], Exec["repo_move_mainpage"],Mediawiki_singlenode::Mw-extension["Diff"], Mediawiki_singlenode::Mw-extension["DataValues"], Mediawiki_singlenode::Mw-extension["DataTypes"], Mediawiki_singlenode::Mw-extension["WikibaseDataModel"]],
			install_path => $install_path,
		}
...
# Wikibase client only:
	if $install_client == true {
		...
		mediawiki_singlenode::mw-extension { [ "Wikibase", "ParserFunctions", "AbuseFilter", "AntiBot", "AntiSpoof", "APC", "ArticleFeedback", "ArticleFeedbackv5", "AssertEdit", "Babel", "CategoryTree", "CharInsert", "CheckUser", "Cite", "cldr", "ClickTracking", "CodeEditor", "Collection", "CustomData", "Echo", "EditPageTracking", "EmailCapture", "ExpandTemplates", "FeaturedFeeds", "FlaggedRevs", "Gadgets", "GlobalUsage", "ImageMap", "InputBox", "Interwiki", "LocalisationUpdate", "MarkAsHelpful", "Math", "MobileFrontend", "MwEmbedSupport", "MWSearch", "NewUserMessage", "normal", "OATHAuth", "OpenSearchXml", "Oversight", "PagedTiffHandler", "PageTriage", "PdfHandler", "Poem", "PoolCounter", "PostEdit", "ReaderFeedback", "RelatedArticles", "RelatedSites", "Renameuser", "Scribunto", "SecurePoll", "SimpleAntiSpam", "SwiftCloudFiles", "SyntaxHighlight_GeSHi", "TemplateSandbox", "TitleKey", "TorBlock", "Translate", "UserDailyContribs", "UserMerge", "Vector", "WikiEditor", "wikihiero", "WikiLove", "WikimediaMaintenance", "WikimediaMessages" ]:
			require => [Git::Clone["mediawiki"], Exec["mediawiki_setup"], Mediawiki_singlenode::Mw-extension["Diff"], Mediawiki_singlenode::Mw-extension["WikibaseDataModel"], Mediawiki_singlenode::Mw-extension["DataValues"], Mediawiki_singlenode::Mw-extension["DataTypes"]],
			install_path => $install_path,
		}
	...
	if $ensure == 'latest' {
		exec { 'wikidata_update':
			require => $install_repo ? {
				true => [Git::Clone["mediawiki"], Mediawiki_singlenode::Mw-extension["UniversalLanguageSelector"], Mediawiki_singlenode::Mw-extension["Diff"], Mediawiki_singlenode::Mw-extension["DataValues"], Mediawiki_singlenode::Mw-extension["DataTypes"], Mediawiki_singlenode::Mw-extension["WikibaseDataModel"], Mediawiki_singlenode::Mw-extension["Wikibase"], File["${install_path}/LocalSettings.php"]],
				default => [Git::Clone["mediawiki"], Mediawiki_singlenode::Mw-extension["Diff"], Mediawiki_singlenode::Mw-extension["DataValues"], Mediawiki_singlenode::Mw-extension["DataTypes"], Mediawiki_singlenode::Mw-extension["WikibaseDataModel"], Mediawiki_singlenode::Mw-extension["Wikibase"], File["${install_path}/LocalSettings.php"]],
			},
