...
class MediaWikiAccessLogTest(unittest.TestCase):
    ...
    def testByEndpoint(self):
        ...
        expected_counts = {"load": 2, "rest_api": 1, "website": 3}
        actual_counts = {"load": 0, "rest_api": 0, "website": 0}

        s = self.store.get_samples("mediawiki_requests_by_endpoint_duration")
        endpoint_ids = {
            self.id_str("php7", endpoint=endpoint): endpoint
            for endpoint in expected_counts.keys()
        }
