...
class UpdateJenkinsThread(threading.Thread):
    ...
    def _get_jenkins_statistics(url):
        ...
        url += '/api/json?tree=jobs[color],overallLoad[busyExecutors[min[latest]],'
        url += 'queueLength[min[latest]],totalExecutors[min[latest]]]'
        ...
        result = {}
        result['jenkins_overallload_busy_executors'] = data[
            'overallLoad']['busyExecutors']['min']['latest']
        result['jenkins_overallload_queue_length'] = data[
            'overallLoad']['queueLength']['min']['latest']
        result['jenkins_overallload_total_executors'] = data[
            'overallLoad']['totalExecutors']['min']['latest']
        result['jenkins_jobs_total'] = len(data['jobs'])
        result['jenkins_jobs_red'] = 0
        result['jenkins_jobs_yellow'] = 0
        result['jenkins_jobs_grey'] = 0
        result['jenkins_jobs_disabled'] = 0
        result['jenkins_jobs_aborted'] = 0
        result['jenkins_jobs_notbuilt'] = 0
...
def metric_init(params):
    ...
    for name, desc in descriptions.iteritems():
        d = desc.copy()
        d['name'] = str(name)
        [d.setdefault(key, METRIC_DEFAULTS[key])
         for key in METRIC_DEFAULTS.iterkeys()]
...
if __name__ == '__main__':
    ...
    try:
        ...
        parser.add_option(
            ...
            help='Base-URL for jenkins api (default: http://127.0.0.1:8080)')
        parser.add_option(
            '-q',
            '--quiet',
            dest='quiet',
