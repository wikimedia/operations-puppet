...
#                   Default: 'package'.
#
# $puppet_source  - Puppet source path for GeoIP files.  Only used if $data_provider is 'puppet'.
#
  ...
  $product_ids    = [106])
{
  ...
  # geoip::data::puppet class
      ...
      if ($puppet_source == undef) {
          fail('Must provide puppet_source to GeoIP.dat files when using \'puppet\' geoip data_provider.')
      }
