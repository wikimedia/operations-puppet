...
if [[ $(hostname -f) == ${puppet_ca_server} ]]; then
    ...
    /usr/bin/printf "$(/bin/date -Iseconds): geoipupdate skipping download of MaxMind .dat files into \"${data_directory}\". This is not the CA server and we sync the entire volatile dir from there separately." >> ${geoipupdate_log}
fi
