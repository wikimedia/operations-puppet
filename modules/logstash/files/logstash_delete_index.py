...
class ESCleanup(object):
    ...
    def filter_to_delete(self, prefix, before, indices):
        ...
        >>> c = ESCleanup()
        >>> c.filter_to_delete('log', parse('2017.05.01'), ['log-2017.05.01', 'log-2017.04.15', 'log-2017.05.02', 'log-foo', 'foo'])
        ['log-2017.04.15']
        """
    ...
    def extract_date(self, index):
        ...
        if self.date_regex.match(index):
            return parse(search.group(1))
...
def parse_args(argv):
    ...
    ...
    SystemExit: 2
    >>> parse_args(['--date', '2017.05.01', '--server', 'my.server.domain', '--port', '1234', '--prefix', 'my-prefix'])
    Namespace(date='2017.05.01', port=1234, prefix='my-prefix', server='my.server.domain')
    """
    parser = argparse.ArgumentParser()
    parser.add_argument("--server", default='localhost', help='the elasticsearch server')
    parser.add_argument("--port", default=9200, type=int, help='the elasticsearch server')
    parser.add_argument("--prefix", default='logstash', help='the prefix identifying the logstash indices')
    parser.add_argument("--date", required=True, help='indices older than this date will be deleted')
    return parser.parse_args(argv)


if __name__ == "__main__":
    args = parse_args(sys.argv)
    ESCleanup(args.server, args.port).cleanup(args.prefix, parse(args.date))
