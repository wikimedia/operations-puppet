...
class logstash(
    ...
    $plugin_zip_path = '/srv/deployment/logstash/plugins/target/release/plugins-latest.zip'
    exec { 'install-logstash-plugins':
        command => "/usr/share/logstash/bin/logstash-plugin install ${plugin_zip_path} && sha256sum ${plugin_zip_path} > /etc/logstash/plugins.hash",
        # Only install plugins if hash of latest does not match stored state
        unless  => "test \"$(cat /etc/logstash/plugins.hash)\" = \"$(sha256sum ${plugin_zip_path})\"",
