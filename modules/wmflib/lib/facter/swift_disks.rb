...
Facter.add('swift_disks') do
  setcode do
    ...
    swift_disks[:accounts] = devices.select {|d| d.end_with?('-part6') }.sort
    swift_disks[:container] = devices.select {|d| d.end_with?('-part7') }.sort
    ...
    swift_disks[:objects] = devices.reject { |d| d.start_with?(*ssds) || d =~ (/-part[1-7]$/) }.sort
    swift_disks
  end
end
