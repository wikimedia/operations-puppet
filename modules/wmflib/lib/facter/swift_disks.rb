...
Facter.add('swift_disks') do
  setcode do
    ...
    swift_disks[:accounts] = devices.select {|d| d.end_with?('-part3') }.sort
    swift_disks[:container] = devices.select {|d| d.end_with?('-part4') }.sort
    ...
    swift_disks[:objects] = devices.reject { |d| d.start_with?(*ssds) || d =~ (/-part[1-4]$/) }.sort
    swift_disks
  end
end
