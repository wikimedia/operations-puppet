...
module OrderedKeys
    # OrderedKeys is a Hash mix-in that defines a `to_yaml` method that
    # is identical to Ruby 1.8's, except it emits keys in sorted order.
    def to_yaml(opts={})
...
def sort_keys_recursive(value)
    ...
    when Hash
        map = {}
        map.extend OrderedKeys if RUBY_VERSION.start_with? '1.8'
