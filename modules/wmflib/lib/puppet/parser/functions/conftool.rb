...
# $status = conftool({ name => 'cp1052.*', service => 'varnish.*'}) # returns a list of associated services
#
require 'json'

module Puppet::Parser::Functions
  newfunction(:conftool, :type => :rvalue, :arity => -1) do |args|
    case args.length
    when 2
      tags, type = args
    when 1
