...
with the same hostlist and period.  It is used to select a
determinstically-random "offset" for the splayed time values (so that the first
host doesn't always start at 00:00), and is also used to permute the order of
the hosts within each DC uniquely.
