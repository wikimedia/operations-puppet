...
describe 'wmflib::cidr_first_address' do
  it 'exists' do
  ...
  context 'when called with an IPv4 CIDR' do
    it { is_expected.to run.with_params('192.0.2.0/24').and_return('192.0.2.1') }
  end

  context 'when called with an IPv6 CIDR' do
    it { is_expected.to run.with_params('fe80::800:27ff:fe00:0/64').and_return('fe80::1') }
  end
end
