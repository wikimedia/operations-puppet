...
import re, sys, urllib2, urlparse, time, json
...
STATUS_MSG_PREFIX = {NAGIOS_CRITICAL : "CRITICAL",
                     NAGIOS_WARNING : "WARNING",
                     NAGIOS_OK : "OK",
                     NAGIOS_UNKNOWN : "UNKNOWN"}

parser = ArgumentParser(description="Monitor a Kafka consumer group using Burrow. (https://github.com/linkedin/Burrow)")
...
if __name__ == "__main__":
    ...
    req = urllib2.Request(url="%(base_url)s/v2/kafka/%(kafka_cluster)s/consumer/%(consumer_group)s/status" % args.__dict__)
    ...
    try:
        ...
        print "CRITICAL: Server %s returned error %d - %s (%s)" % (args.base_url, e.code, e.msg, e.read())
    ...
    try:
        ...
        print "CRITICAL: Error decoding API result to JSON - %s (API result: %s)" % (str(e), output)
        sys.exit(NAGIOS_CRITICAL)
    ...
    if json_output["status"]["maxlag"]:
        ...
        max_lag_detail = "Worst Lag: %s/p%d - lag:%d offset:%d" %  (max_lag["topic"],
                                                                           max_lag["partition"],
                                                                           max_lag["end"]["lag"],
                                                                           max_lag["end"]["offset"])
    ...
    for part in json_output["status"]["partitions"]:
        if part["status"] in ["WARN","STOP","STALL"]:
            if not problem_topic_partitions.has_key(part["topic"]):
    ...
    for topic in problem_topic_partitions.keys():
        problem_partition_detail = "%s(%s WARN:%d STOP:%d STAL:%d) " % (problem_partition_detail,
                                                                        topic,
