...
OCSP_URL=$(openssl x509 -in ${CERT_PATH} -text 2>/dev/null | grep "OCSP - URI:" | cut -d: -f2,3)
ISSUER_PATH="${ISSUER_DIR}/$(openssl x509 -in ${CERT_PATH} -out /dev/null -issuer_hash 2>/dev/null).0"
