...
## GC
for name, path in [('par_new', 'ParNew'), ('concurrent_mark_sweep', 'ConcurrentMarkSweep')]:
...
for name in ['generic', 'index', 'get', 'snapshot', 'merge', 'suggest', 'bulk',
        'optimize', 'warmer', 'flush', 'search', 'percolate', 'management',
        'refresh']:
...
# Global holding the last time we fetched data from ES.  We only fetch it every 3 seconds.
mainResult = dict()
...
def getStat(url, name):
    ...
    val = dig_it_up(mainResult['stats'], 'nodes.%s.%s' % (node, stats[name]['path']))
