...
# - $send_logs_to_logstash: send logs from elastic search to logstash
# - $gelf_host: host to which logs will be sent
...
class elasticsearch($cluster_name,
    ...
    if $send_logs_to_logstash {
      if $graylog_host == undef {
        fail("\$elasticsearch::gelf_host must be defined if sending logs to logstash is active.")
      }
