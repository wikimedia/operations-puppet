...
Puppet::Reports.register_report(:labsstatus) do
    ...
    def addMeta(key, value)
            command = "/usr/bin/nova --no-cache --os-region-name #{conf['region']} --os-auth-url #{conf['auth_url']} --os-password #{conf['password']} --os-username #{conf['username']} --os-tenant-name #{project} meta #{hostname} set #{key}=#{value}"
    ...
    def process
        ...
        if (!project.empty?) and (!hostname.empty?)
            addMeta('puppetstatus', self.status)
            addMeta('puppettimestamp', self.timestamp)
        end
    end
end
