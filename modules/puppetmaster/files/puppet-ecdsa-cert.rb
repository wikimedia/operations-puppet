...
class PuppetECDSAGen
  ...
  def parse_config(filename)
    ...
    conffile.each do |key, val|
      k = key.to_sym
      @config[k] = val if @@confkeys.include? k
    end
  end
  ...
  def csr_alt_names(csr)
    ...
    extensions = [OpenSSL::X509::ExtensionFactory.new.create_extension('subjectAltName', san_list.join(','))]
  ...
  def sign
    req = Net::HTTP::Put.new("/production/certificate_request/#{@common_name}", 'Content-Type' => 'text/plain')
    req.body = File.read csr_path
    resp, data = https.request(req)
    fail(PuppetECDSAGenError, 'Signing request to %s failed with code %s: %s' % [@config[:puppetca], resp.code, resp.body]) unless resp.code == '200'
    puts resp.message
  end
end
