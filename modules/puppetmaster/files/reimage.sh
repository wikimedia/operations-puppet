...
function enable_and_run_puppet {
    nodename=${1}
    hostsfile=$(mktemp /tmp/sshhostsXXXXX)
    log "Enabling puppet"
    ...
    ssh -i /root/.ssh/install_new -o "UserKnownHostsFile=${hostsfile}" -o "GlobalKnownHostsFile=/dev/null" ${nodename} "puppet agent -t" > ${nodename}.puppetrun.log 2>&1 &
