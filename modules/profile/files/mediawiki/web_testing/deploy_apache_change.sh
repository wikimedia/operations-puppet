...
confirm "Should we merge the puppet change?"
...
if [ "$apply" == "yes" ]; then
    ...
    confirm "Should we run apply the puppet change on all servers?"
    ...
    confirm  "Should puppet be reenabled on all servers?"
