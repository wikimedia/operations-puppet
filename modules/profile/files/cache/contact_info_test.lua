...
describe("HAProxy - set contact info", function()
    ...
    it("should accept Pywikibot standard format with wiki username", function()
        local txn = test_txn("script_name (wikipedia:en; User:SomeBotOrOther) Pywikibot/10.2.0 (g19732) requests/2.32.3 Python/3.11.2.final.0")
        haproxy.set_contact_info(txn)
        assert.are.equal("SomeBotOrOther", txn.req.contact_info)
    end)
