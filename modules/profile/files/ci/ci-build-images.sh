...
#
# Copyright © Antoine "hashar" Musso - 2022, 2026
# Copyright © Wikimedia Foundation - 2022, 2026
#
# Loosely adapted https://www.mediawiki.org/wiki/Continuous_integration/Qemu#Runbook
# by Timo Tijhof, Adam Shortland, Dave Pifke. See that page for more details.
...
# Options passed to virt-customize
customizations=(
...
function stage() {
    ...
    printf "[$script_name] $BOLD%s ... $NORMAL" "$message"
    if [ -f "$1" ]; then
        printf "✅ $DIM(%s exists)$NORMAL\n" "$file"
        return 1
    else
        printf "\n"
        return 0
...
&& {
    trap 'error_message "Error downloading image"; rm -f "$debian_image"' EXIT
    curl --progress-bar -L -O "$url"
    trap EXIT

    if [ -f "$ci_image" ] || [ -f "$grown_image" ]; then
        message "🧹 Deleting obsolete images '$grown_image' '$ci_image'"
        rm -vf "$grown_image" "$ci_image"
    fi
}

stage "$grown_image" "Creating grown image" \
&& {
    ...
    #
    message "${BOLD}Veryfing checksum of Debian image${NORMAL} '$debian_image'"
    sha512sum --quiet --check <(printf "%s %s\n" "$sha512sum" "$debian_image") || {
        error_message "ERROR: failed to verify sha512sum for '$debian_image'"
        rm -fv "$debian_image"
        exit 1
    }

    message "${BOLD}Creating a grown image with $disk_size disk size${NORMAL}"
    # Copied directly from virt-resize(1) man page
    # Albeit without `-o preallocation=metadata`
    trap 'error_message "Error creating grown image $grown_image"; rm -fv $grown_image' EXIT
    qemu-img create -f qcow2 "$grown_image" "$disk_size"
    virt-resize --expand /dev/sda1 "$debian_image" "$grown_image"
    trap EXIT

    if [ -f "$ci_image" ]; then
        message "🧹 Deleting obsolete CI image '$ci_image'"
        rm -vf "$ci_image"
    fi
}

stage "$ssh_key" "Generating a ssh key pair" \
&& {
...
stage "$ci_image" "Creating our CI specific qcow2 layer" \
&& {
    trap 'error_message "Error creating CI image $ci_image"; rm -fv $ci_image' EXIT
...
message "${BOLD}Completed successfully${NORMAL}."
