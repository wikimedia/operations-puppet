...
def __run_cumin(host, cmds, timeout=30, ignore_exit=False):
    try:
...
def disable_puppet(host, reason):
    cmd = 'disable-puppet "{message}"'.format(message=reason)
    return __run_cumin(host, [cmd,])

def enable_and_run_puppet(host, reason):
    cmd = 'run-puppet-agent --enable "{message}"'.format(message=reason)
    return __run_cumin(host, [cmd,], timeout=7200)

def is_puppet_enabled(host):
    return __run_cumin(host, ['puppet-enabled',])

def depool(host):
    return __run_cumin(host, ['depool',])

def pool(host):
    return __run_cumin(host, ['pool',])
...
def icinga_downtime(host, reason, seconds):
    ...
    return __run_cumin(icinga_host, [cmd,], timeout=60)

def icinga_cancel_downtime(host):
    icinga_host = lib.resolve_dns('icinga.wikimedia.org', 'CNAME')
    cmd = 'echo "[{now}] DEL_DOWNTIME_BY_HOST_NAME;{hostname}"  > {commandfile}'.format(
        now=int(time.time()), hostname=host.split('.')[0], commandfile='/var/lib/nagios/rw/nagios.cmd')
    return __run_cumin(icinga_host, [cmd,], timeout=300)

def pre_puppet(host, downgrading):
    experimental_repo = 'echo deb http://apt.wikimedia.org/wikimedia jessie-wikimedia experimental > /etc/apt/sources.list.d/wikimedia-experimental.list'
    ...
    return __run_cumin(host, cmds, ignore_exit=True, timeout=7200)
...
def check_http_responses(host):
    ...
    if req.status_code not in (200, 404):
        logger.error("Unexpected response from varnish-fe. Got {} instead of 200/404. Exiting.".format(req.status_code))
        return False

    req = requests.head('http://{}:3128'.format(host))
    if req.status_code not in (200, 404):
        logger.error("Unexpected response from varnish-be. Got {} instead of 200/404. Exiting.".format(req.status_code))
        return False
...
def main():
    ...
    if not is_puppet_enabled(args.host):
    ...
    # Disable puppet
    if not disable_puppet(args.host, reason):
    ...
    if not ask_confirmation("Waiting for you to puppet-merge the change toggling {}'s hiera settings".format(args.host)):
        return 1
    ...
    if not enable_and_run_puppet(args.host, reason):
