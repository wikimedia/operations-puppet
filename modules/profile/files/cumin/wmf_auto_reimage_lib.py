...
def puppet_first_run(host):
    ...
    host_query = 'D{{{host}}}'.format(host=host)
    print_line('Started first puppet run (sit back, relax, and enjoy the wait)', host=host)
    run_cumin('puppet_first_run', host_query, commands, timeout=3600, installer=True)
...
def wait_reboot(host, start=None, installer=False):
    ...
    print_line('Host up{msg}'.format(msg=msg), host=host.replace('D{', '').replace('}', ''))
...
def check_uptime(host, minimum=0, maximum=None, installer=False):
    ...
    print_line('Uptime checked', host=host.replace('D{', '').replace('}', ''))
