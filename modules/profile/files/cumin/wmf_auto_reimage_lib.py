...
def puppet_remove_local_cert(host):
    """Delete the local Puppet certificate.

    Arguments:
    host -- the host where to delete the local certificate.
    """
    run_cumin('puppet_remove_local_cert', host, ['rm -rf /var/lib/puppet/ssl'])
...
def puppet_generate_cert(host):
    ...
    """
    _, worker = run_cumin('puppet_generate_certs', host, ['puppet agent --test'], installer=True,
