...
COMMON_PERSISTENT_FIELDS = ('id', 'uuid', 'timestamp', 'clientValidated', 'isTruncated')
...
def parse_yaml_whitelist(whitelist_dict):
    ...
    for table_name, table_dict in whitelist_dict.items():
        # The yaml whitelist might have a defaults section
        # to be interpreted by the hive cleaner (not mysql)
        # If so, ignore it.
        if table_name.lower() == '__defaults__':
            continue
        if not re.match(r'^[a-zA-Z0-9_]+$', table_name):
            raise RuntimeError('Invalid table name: {}.'.format(table_name))
        whitelisted_fields = []
        for field_name, field_spec in table_dict.items():
            if not re.match(r'^[a-zA-Z0-9_.]+$', field_name):
                raise RuntimeError('Invalid field name: {}.'.format(field_name))
            elif field_name.lower() == 'event':
                # An event field should have an indented block spec.
                # If it is a string it means it's badly formatted.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yaml', dest='yaml', action='store_true', default=False,
...
class TestParser(unittest.TestCase):
    ...
    def test_parse_yaml_whitelist(self):
        ...
        structure returned must have a specific format and organization).
        The DEFAULTS section is ignored (used for the Hive sanitization).
        """
        whitelist_dict = {
            '__DEFAULTS__': {
                'wiki': 'keep'
