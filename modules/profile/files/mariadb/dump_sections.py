...
def logical_dump(name, config, default_config):
    ...
    output_dir = os.path.join(BACKUP_DIR, 'dump.{}.{}'.format(name, formatted_date))
...
def rotate_dumps(days):
    ...
    for entry in files:
        ...
        if not os.path.isdir(path):
            continue
        filename_parts = entry.split('.')
        pattern = re.compile('dump\.[a-z0-9\-]+\.(20\d\d-[01]\d-[0123]\d\T\d\d:\d\d:\d\d)')
