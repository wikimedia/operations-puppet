...
class profile::backup::director(
    ...
    bacula::director::pool { $offsite_pool:
        max_vols         => 240,
        storage          => "${offsite_sd}-FileStorageProductionCodfw",
        volume_retention => '90 days',
        label_fmt        => $offsite_pool,
        max_vol_bytes    => '536870912000',
    }

    # New eqiad pool for read-only External Storage backups
    ...
    # New codfw pool for read-only External Storage backups
    ...
    # New eqiad pool for read-write External storage backups
    ...
    # New codfw pool for read-write External storage backups
    ...
    # Eqiad pool for Repository storage backups
    bacula::director::pool { 'ReposEqiad':
        max_vols         => 200,
        storage          => 'backup1012-FileStorageReposEqiad',
        volume_retention => '90 days',
        label_fmt        => 'repos-eqiad',
        max_vol_bytes    => '536870912000',
    }
    # Codfw pool for read-write External storage backups
    bacula::director::pool { 'ReposCodfw':
        max_vols         => 200,
        storage          => 'backup2012-FileStorageReposCodfw',
        volume_retention => '90 days',
        label_fmt        => 'repos-codfw',
        max_vol_bytes    => '536870912000',
    }
