...
class profile::druid::monitoring::overlord {
    ...
    $java_opts = "-javaagent:/usr/share/java/prometheus/jmx_prometheus_javaagent.jar=${facts['networking']['ip']}:${prometheus_jmx_exporter_overlord_port}:${jmx_exporter_config_file}"
    profile::prometheus::jmx_exporter { "druid_overlord_${facts['networking']['hostname']}":
