...
# master host, copies it on localhost and then uns it against the local database.
...
class profile::mariadb::misc::eventlogging::replication (
    $master_host = hiera('profile::mariadb::eventlogging::replication::master_host'),
    $cutoff_days = hiera('profile::mariadb::eventlogging::replication::cutoff_days'),
    $batch_size  = hiera('profile::mariadb::eventlogging::replication::batch_size'),
) {
