...
class profile::dbbackups::bacula_es (
    ...
    if $active {
        ...
        # Temporarily disabled while read only backups run
        ...
        # taken on eqiad, stored on codfw: { es1:TBD es2:TBD es3:TBD}
        # taken on codfw, stored on eqiad: { es1:TBD es2:TBD es3:TBD}
        backup::set { 'mysql-srv-backups-dumps-latest':
