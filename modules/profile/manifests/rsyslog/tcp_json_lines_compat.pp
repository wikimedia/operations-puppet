...
class profile::rsyslog::tcp_json_lines_compat(
    Array $logging_kafka_brokers       = lookup('profile::rsyslog::kafka_shipper::kafka_brokers'),
    Integer $port                      = lookup('profile::rsyslog::tcp_json_lines_compat_port', {'default_value' => 11514}),
    Array[String] $queue_enabled_sites = lookup('profile::rsyslog::kafka_queue_enabled_sites',  {'default_value' => []}),
) {
    ...
    rsyslog::conf { 'tcp_json_lines_compat':
        content  => template('profile/rsyslog/tcp_json_lines_compat.conf.erb'),
        priority => 50,
    }
}
