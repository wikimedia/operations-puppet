...
class profile::hive::site_hdfs {
    ...
    exec { 'put-hive-site-in-hdfs':
        command     => "/usr/bin/hdfs dfs -put -f ${cdh::hive::config_directory}/hive-site.xml ${hdfs_path} && /usr/bin/hdfs dfs -chmod 640 ${hdfs_path} && /usr/bin/hdfs dfs -chown hdfs:hdfs ${hdfs_path}",
        user        => 'hdfs',
        refreshonly => true,
