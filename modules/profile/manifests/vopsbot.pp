...
class profile::vopsbot(
    ...
    Stdlib::Host                $active_alert_host     = lookup('profile::alertmanager::active_host'),
    Array[Stdlib::Host]         $alertmanager_partners = lookup('profile::alertmanager::partners'),
) {
    $passive_hosts = $alertmanager_partners.filter |$partner| {
        $partner != $active_alert_host
    }
    ...
    class { 'vopsbot':
        ...
        active_alert_host   => $active_alert_host,
        passive_alert_hosts => $passive_hosts,
    }
}
