class profile::nftables::basefirewall (
    ...
    $cloud_cumin_masters_ipv4 = filter($cloud_cumin_masters) |$addr| { $addr =~ Stdlib::IP::Address::V4 }
    $cloud_cumin_masters_ipv6 = filter($cloud_cumin_masters) |$addr| { $addr =~ Stdlib::IP::Address::V6 }
    ...
    $ssh_allowed_ipv4 = flatten($bastion_hosts_ipv4, $cumin_masters_ipv4, $cloud_cumin_masters_ipv4)
    $ssh_allowed_ipv6 = flatten($bastion_hosts_ipv6, $cumin_masters_ipv6, $cloud_cumin_masters_ipv6)
