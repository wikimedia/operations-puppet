...
define profile::analytics::refinery::job::refine_job (
    ...
    $monitor_since    = 28
    $monitor_until    = 4
    $monitor_command  = "/usr/bin/spark-submit --master yarn --deploy-mode cluster --class org.wikimedia.analytics.refinery.job.refine.RefineMonitor --name ${monitor_job_name} ${_refinery_job_jar} --since ${monitor_since} --until ${monitor_until} ${whitelist_blacklist_opt} ${email_opts} --input-base-path ${input_base_path} --input-regex '${input_regex}' --input-capture '${input_capture}' --output-base-path ${output_base_path} --database ${output_database}"
    $monitor_script   = "/usr/local/bin/${job_name}"
