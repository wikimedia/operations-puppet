...
class profile::analytics::refinery::job::sqoop_mediawiki {
    ...
    cron { 'refinery-sqoop-mediawiki':
        command  => "${env} && /usr/bin/python3 ${profile::analytics::refinery::path}/bin/sqoop-mediawiki-tables --job-name sqoop-mediawiki-monthly-$(/bin/date --date=\"$(/bin/date +\\%Y-\\%m-15) -1 month\" +'\\%Y-\\%m') --labsdb --jdbc-host ${labs_db_host} --output-dir ${$output_directory_labs} --wiki-file  ${wiki_file_labs} --tables archive,ipblocks,logging,page,pagelinks,redirect,revision,user,user_groups --jar-file ${orm_jar_file} --user ${labs_db_user} --password-file ${db_password_labs} --from-timestamp 20010101000000 --to-timestamp \$(/bin/date '+\\%Y\\%m01000000') --partition-name snapshot --partition-value \$(/bin/date --date=\"$(/bin/date +\\%Y-\\%m-15) -1 month\" +'\\%Y-\\%m') --mappers ${num_mappers} --processors ${num_processors} --log-file ${labs_log_file} > /dev/null",
        user     => 'hdfs',
        minute   => '0',
        hour     => '0',
