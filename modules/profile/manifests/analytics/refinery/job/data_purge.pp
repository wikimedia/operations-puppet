...
class profile::analytics::refinery::job::data_purge (
    ...
    kerberos::systemd_timer { 'refinery-drop-webrequest-refined-partitions':
        ...
        command     => "${refinery_path}/bin/refinery-drop-older-than --database='wmf' --tables='webrequest' --base-path='/wmf/data/wmf/webrequest' --path-format='.+/${hive_date_path_format}' --older-than='${extended_retention_days}' --allowed-interval='3' --skip-trash --execute='7fda1bf5f04e6883293a6b4a019b3b02'",
        interval    => '*-*-* 00/4:45:00',
        environment => $systemd_env,
        user        => 'analytics',
    }
    ...
    kerberos::systemd_timer { 'refinery-drop-pageview-actor-hourly-partitions':
        ...
        command     => "${refinery_path}/bin/refinery-drop-older-than --database='wmf' --tables='pageview_actor' --base-path='/wmf/data/wmf/pageview/actor' --path-format='${hive_date_path_format}' --older-than='${extended_retention_days}' --allowed-interval='3' --skip-trash --execute='4ea5bad542091393c2e2dcd088e49544'",
        interval    => '*-*-* 00/4:50:00',
        environment => $systemd_env,
        user        => 'analytics',
    }
    ...
    $druid_webrequest_sampled_retention_days = 90
    ...
    # drop hourly webrequest_actor data (3 datasets) used to compute automated agent-type after 90 days
        ...
        command     => "${refinery_path}/bin/refinery-drop-older-than --database='wmf' --tables='webrequest_actor_metrics_hourly' --base-path='/wmf/data/wmf/webrequest_actor/metrics/hourly' --path-format='${hive_date_path_format}' --older-than='${extended_retention_days}' --allowed-interval='3' --skip-trash --execute='2981b49bfdfb75e6f664341054a7c6ad'",
        environment => $systemd_env,
        interval    => '*-*-* 00/4:40:00',
        user        => 'analytics',
    }
    ...
    kerberos::systemd_timer { 'drop-webrequest-actor-label-hourly':
        ...
        command     => "${refinery_path}/bin/refinery-drop-older-than --database='wmf' --tables='webrequest_actor_label_hourly' --base-path='/wmf/data/wmf/webrequest_actor/label/hourly' --path-format='${hive_date_path_format}' --older-than='${extended_retention_days}' --allowed-interval='3' --skip-trash --execute='ad7b71960cf4dc53b13c39f901aeb536'",
        environment => $systemd_env,
        interval    => '*-*-* 00/4:50:00',
        user        => 'analytics',
    }
