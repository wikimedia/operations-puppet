...
class profile::analytics::refinery::job::data_purge (
    ...
    kerberos::systemd_timer { 'drop-el-unsanitized-events':
        ...
        command     => "${refinery_path}/bin/refinery-drop-older-than --database='event' --tables='^(?!wmdebanner|neflow)[A-Za-z0-9]+$$' --base-path='/wmf/data/event' --path-format='(?!WMDEBanner|netflow)[A-Za-z0-9]+/year=(?P<year>[0-9]+)(/month=(?P<month>[0-9]+)(/day=(?P<day>[0-9]+)(/hour=(?P<hour>[0-9]+))?)?)?' --older-than='90' --skip-trash --execute='30ae9f84cadde5ddf00754dbd6bc4509' --log-file='${el_unsanitized_log_file}'",
        interval    => '*-*-* 00:00:00',
        environment => $systemd_env,
        user        => 'analytics',
    }
