class profile::kubernetes::master(
    ...
    monitoring::check_prometheus { 'apiserver_request_latencies':
        ...
        query          => 'scalar(sum(rate(apiserver_request_latencies_summary_sum{job="k8s-api",verb!="WATCH"}[5m]))/sum(rate(apiserver_request_latencies_summary_count{job="k8s-api",verb!="WATCH"}[5m])))',
        prometheus_url => "http://prometheus.svc.${::site}.wmnet/k8s",
        warning        => 50,
        critical       => 100,
    }
    ...
    monitoring::check_prometheus { 'etcd_request_latencies':
        ...
        query          => 'scalar(sum(rate(etcd_request_latencies_summary_sum{job="k8s-api"}[5m]))/sum(rate(etcd_request_latencies_summary_count{job="k8s-api"}[5m])))',
        prometheus_url => "http://prometheus.svc.${::site}.wmnet/k8s",
        warning        => 30,
        critical       => 50,
    }
