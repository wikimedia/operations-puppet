class profile::mediawiki::maintenance::wikidata {
    ...
    $lvs_servers = $lvs::configuration::lvs_classes.filter |$host, $class| {
        $class == $my_lvs_class
    }
    if ($lvs_servers == []) {
        fail("could not find servers for class '${my_lvs_class}'")
    }
    $lb = $lvs_servers.map |$host, $_| { "--lb ${host}:9090"}.join(' ')

    $additional_args = "--lb-pool ${svc_lbl} ${lb}"
    profile::mediawiki::periodic_job { 'wikidata-updateQueryServiceLag':
        command  => "/usr/local/bin/mwscript extensions/Wikidata.org/maintenance/updateQueryServiceLag.php --wiki wikidatawiki --cluster wdqs --prometheus prometheus.svc.eqiad.wmnet --prometheus prometheus.svc.codfw.wmnet ${additional_args}",
        interval => '*-*-* *:*:00'
    }
}
