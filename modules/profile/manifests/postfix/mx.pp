...
# @param rspamd_network_discards
#     Sender IPs or CIDRs to silently discard.
...
class profile::postfix::mx (
    ...
    $rspamd_base_config = {
        ...
        multimap => {
            ...
            network_discards => {
                type      => 'ip',
                map       => 'file:///var/lib/rspamd/network_discards.map',
                symbol    => 'NETWORK_DISCARDS',
                action    => 'discard',
                prefilter => true,
            },
        },
    ...
    file { '/var/lib/rspamd/network_discards.map':
        ensure  => present,
        mode    => '0444',
        content => $rspamd_network_discards.reduce('') |$memo, $v| {
            "${memo}${v}\n"
        },
        # needed to create the /var/lib/rspamd dir
        require => Package['rspamd'],
