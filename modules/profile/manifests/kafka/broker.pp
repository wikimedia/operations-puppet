...
#
# This layout is built to work with certificates generated with
...
class profile::kafka::broker(
    ...
    if $ssl_enabled {
        ...
        $certificates_location              = '/etc/kafka/certificates'

        $ssl_keystore_secrets_path      = "certificates/kafka_broker_${::hostname}/kafka_broker_${::hostname}.keystore.jks"
        $ssl_keystore_location          = "${certificates_location}/kafka_broker_${::hostname}.keystore.jks"

        $ssl_truststore_secrets_path    = "certificates/kafka_broker_${::hostname}/truststore.jks"
        $ssl_truststore_location        = "${certificates_location}/truststore.jks"

        file { $certificates_location:
            ensure => 'directory',
            owner  => 'kafka',
            group  => 'kafka',
            mode   => '0555',
