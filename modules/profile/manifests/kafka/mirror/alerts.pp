...
define profile::kafka::mirror::alerts(
    ...
    monitoring::check_prometheus { "kafka-mirror-${mirror_name}-produce_rate":
        ...
        query       => "scalar(sum(avg_over_time(kafka_producer_producer_metrics_record_send_rate{client_id=~\"kafka-mirror-.*-main-eqiad_to_jumbo-eqiad@[0-9]+\"} [${monitoring_period}])))",
    }
