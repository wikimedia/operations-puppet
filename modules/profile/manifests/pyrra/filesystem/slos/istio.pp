          ...
          content => to_yaml({
            ...
            'spec'       => {
                ...
                'indicator' => {
                    'ratio' => {
                        'errors' => {
                            'metric' => "workload:istio_requests_total{kubernetes_namespace=\"istio-system\", destination_canonical_service=\"${destination_canonical_service}\", response_code=~\"${requests_errors_regex}\", site=\"${datacenter}\" }",
                        },
                        'total'  => {
                            'metric' => "istio_requests_total{kubernetes_namespace=\"istio-system\", destination_canonical_service=\"${destination_canonical_service}\", site=\"${datacenter}\" }",
                        },
                    },
                },
            },
          ...
          content => to_yaml({
            ...
            'spec'       => {
                ...
                'indicator' => {
                    'latency'  => {
                        'success' => {
                            'metric' => "workload:istio_request_duration_milliseconds_bucket{kubernetes_namespace=\"istio-system\", destination_canonical_service=\"${destination_canonical_service}\", le=\"${latency_max_seconds_bucket}\", site=\"${datacenter}\" }",
                        },
                        'total'   => {
                            'metric' => "workload:istio_request_duration_milliseconds_bucket{kubernetes_namespace=\"istio-system\", destination_canonical_service=\"${destination_canonical_service}\", site=\"${datacenter}\" }",
                        },
                    },
                },
            },
