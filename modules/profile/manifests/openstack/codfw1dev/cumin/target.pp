...
class profile::openstack::codfw1dev::cumin::target(
    $project_masters = hiera('profile::openstack::codfw1dev::cumin::project_masters'),
    $project_pub_key = hiera('profile::openstack::codfw1dev::cumin::project_pub_key'),
    $cluster = lookup('cluster'),
    $site = $::site,  # lint:ignore:wmf_styleguide
    Array[Stdlib::IP::Address] $cumin_masters = hiera('cumin_masters', []),
    Boolean $permit_port_forwarding = hiera('profile::openstack::codfw1dev::cumin::permit_port_forwarding', false),
) {
