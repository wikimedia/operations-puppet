...
class profile::openstack::main::cumin::master(
    ...
    $project_ssh_priv_key_path = hiera('profile::openstack::main::cumin::project_ssh_priv_key_path'),
        ...
        if os_version('debian == jessie') {
            $backport_packages = ['python3-keystoneauth1', 'python3-keystoneclient', 'python3-novaclient']
            $backport_packages.each |$backport_package| {
                apt::pin { "${backport_package}_package_from_backports":
                    package  => $backport_package,
