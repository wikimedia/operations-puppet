...
class profile::presto::monitoring::server {
    ...
    $java_opts = "-javaagent:/usr/share/java/prometheus/jmx_prometheus_javaagent.jar=${facts['networking']['ip']}:${prometheus_jmx_exporter_server_port}:${jmx_exporter_config_file}"
    profile::prometheus::jmx_exporter { "presto_server_${facts['networking']['hostname']}":
