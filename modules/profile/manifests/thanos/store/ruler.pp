...
# == Class: profile::thanos::store::ruler
#
# Thanos store implements the Store API on top of historical data in an object storage bucket.
#
# This specific instance (ruler) will serve only blocks generated by the ruler components
# and/or the backfill process, as long as they are newer than the cutoff threshold.
# Once they get older than the threshold, they will be served by the main store component.
#
# It avoids serving too many blocks from the object storage and allows the rulers
# to query data directly from the sidecars.
#
# = Parameters
