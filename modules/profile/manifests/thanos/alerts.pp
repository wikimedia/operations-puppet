...
class profile::thanos::alerts {
    ...
    monitoring::check_prometheus { 'thanos_compact_bucket_failures':
        description     => 'Thanos compact has high percentage of object storage failures',
        query           => @(QUERY/L)
        (
    ...
    monitoring::check_prometheus { 'thanos_compact_not_running':
        description     => 'Thanos compact has not run',
        query           => @(QUERY/L)
        (time() - max(thanos_objstore_bucket_last_successful_upload_time{job=~"thanos-compact.*"}))
        / 60 / 60
        | QUERY
        ,
        prometheus_url  => 'http://thanos-query.discovery.wmnet',
    ...
    monitoring::check_prometheus { 'thanos_query_high_latency_range_query':
        ...
        and
        sum by (job) (rate(http_request_duration_seconds_bucket{job=~"thanos-query.*", handler="query_range"}[5m])) > 0
        | QUERY
        ,
        prometheus_url  => 'http://thanos-query.discovery.wmnet',
        method          => 'ge',
