...
class profile::dumps::distribution::ferm(
    ...
    $rsync_clients_ipv4_ferm = join(concat(flatten($internal_clients_ipv4 + $ipv4_mirrors), ' '))
    $rsync_clients_ipv6_ferm = join(concat(flatten($internal_clients_ipv6 + $ipv6_mirrors), ' '))
