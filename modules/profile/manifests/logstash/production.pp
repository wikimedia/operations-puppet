...
class profile::logstash::production (
  ...
  # DEPRECATED: the webrequest.dumps.dev0 stream is being replaced by
  # webrequest.dumps.v1 (T425087). These dev0 inputs will be removed once the
  # producer has switched over to the new stream name.
  ...
  logstash::input::kafka { 'webrequest-dumps-v1-eqiad':
    kafka_cluster_name                    => 'logging-eqiad',
    topic                                 => 'eqiad.webrequest.dumps.v1',
    group_id                              => $input_kafka_consumer_group_id,
    type                                  => 'webrequest-dumps',
    tags                                  => ['input-kafka-webrequest-dumps-v1-eqiad', 'kafka', 'es'],
    codec                                 => 'json',
    security_protocol                     => 'SSL',
    ssl_truststore_location               => $ssl_truststore_location,
    ssl_truststore_password               => $ssl_truststore_password,
  ...
  logstash::input::kafka { 'webrequest-dumps-v1-codfw':
    kafka_cluster_name                    => 'logging-codfw',
    topic                                 => 'codfw.webrequest.dumps.v1',
    group_id                              => $input_kafka_consumer_group_id,
    type                                  => 'webrequest-dumps',
    tags                                  => ['input-kafka-webrequest-dumps-v1-codfw', 'kafka', 'es'],
    codec                                 => 'json',
    security_protocol                     => 'SSL',
    ssl_truststore_location               => $ssl_truststore_location,
    ssl_truststore_password               => $ssl_truststore_password,
