...
class sentry (
    ...
    if $manage_db {
        ...
        postgresql::db { $db_name:
            ...
            name       => $db_name,
            owner      => $db_user,
            require    => [User[$user], Postgresql::User[$db_user]],
        }

        $tables_exist = "/usr/bin/pg_dump --schema-only --dbname='${db_name}' --table='sentry_*'"
        exec { 'initialize sentry database':
            ...
            before  => [Base::Service_unit['sentry-worker'], Base::Service_unit['sentry']],
        }
