class snapshot::dumps::pagetitles(
    ...
    cron { 'pagetitles-ns0':
        ...
        environment => 'MAILTO=ops-dumps@wikimedia.org',
        user        => $user,
        command     => "cd ${snapshot::dumps::dirs::dumpsdir}; python onallwikis.py --configfile confs/wikidump.conf.monitor  --filenameformat '{w}-{d}-all-titles-in-ns-0.gz' --outdir '${snapshot::dumps::dirs::datadir}/public/other/pagetitles/{d}' --query \"'select page_title from page where page_namespace=0;'\"",
        minute      => '10',
        hour        => '8',
    ...
    cron { 'pagetitles-ns6':
        ...
        environment => 'MAILTO=ops-dumps@wikimedia.org',
        user        => $user,
        command     => "cd ${snapshot::dumps::dirs::dumpsdir}; python onallwikis.py --configfile confs/wikidump.conf.monitor  --filenameformat '{w}-{d}-all-media-titles.gz' --outdir '${snapshot::dumps::dirs::datadir}/public/other/mediatitles/{d}' --query \"'select page_title from page where page_namespace=6;'\"",
        minute      => '50',
        hour        => '8',
