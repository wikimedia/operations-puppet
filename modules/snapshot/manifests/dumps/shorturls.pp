class snapshot::dumps::shorturls(
    ...
    cron { 'shorturls':
        ...
        environment => 'MAILTO=ops-dumps@wikimedia.org',
        user        => $user,
        command     => "cd ${snapshot::dirs::dumpsdir}; python onallwikis.py --wiki metawiki --configfile confs/wikidump.conf.monitor  --filenameformat 'shorturls-{d}.txt' --outdir '${snapshot::dirs::datadir}/public/other/shorturls' --script extensions/UrlShortener/maintenance/dumpURLs.php '{DIR}'",
        minute      => '5',
        hour        => '8',
        weekday     => '1',
