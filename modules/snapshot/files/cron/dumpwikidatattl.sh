...
while true; do
	...
	while [ $i -lt $shards ]; do
		(
			...
			php $multiversionscript extensions/Wikidata/extensions/Wikibase/repo/maintenance/dumpRdf.php --wiki wikidatawiki --shard $i --sharding-factor $shards --format ttl --memory-limit 8G 2>> $errorLog | gzip > $tempDir/wikidataTTL.$i.gz
			exitCode=$?
