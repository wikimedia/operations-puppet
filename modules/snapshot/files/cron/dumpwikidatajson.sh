...
while true; do
	...
	while [ $i -lt $shards ]; do
		(
			...
			# NOTE: We temporary set the shard size differently for each shard. T177486#3674942.
			# Remove --no-cache once this runs on hhvm (or everything is back on Zend), see T180048.
			php5 $multiversionscript extensions/Wikidata/extensions/Wikibase/repo/maintenance/dumpJson.php --wiki wikidatawiki --shard $i --sharding-factor $shards --batch-size `expr $(expr $i + 1) \* 500` --snippet 2 --no-cache 2>> $errorLog | gzip -9 > $tempDir/wikidataJson.$i.gz
			exitCode=$?
