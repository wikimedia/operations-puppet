...
numberOfBatchesNeeded=`expr $numberOfBatchesNeeded / $shards`
...
while [ $i -lt $shards ]; do
	(
		...
		while [ $batch -lt $numberOfBatchesNeeded ]; do
			firstPageIdParam="--first-page-id "`expr $batch \* $pagesPerBatch \* $shards + 1`
			lastPageIdParam="--last-page-id "`expr \( $batch + 1 \) \* $pagesPerBatch \* $shards`

			if [ `expr $batch + 1` -eq $numberOfBatchesNeeded ]; then
			...
			if [ $exitCode -gt 0 ]; then
				...
				if [ $retries -gt 5 ]; then
					...
					echo -e "\n\n(`date --iso-8601=minutes`) Giving up after `expr $retries - 1` retries." >> $errorLog
					rm -f $tempDir/wikidataJson.$i.gz
					break
				fi

				# Increase the sleep time for every retry
				sleep `expr 900 * $retries`
				continue
			fi

			# Make sure the last concat has finished before starting a new one
			wait
			rm -f $tempDir/wikidataJson.$i-batch`expr $batch - 1`.gz
