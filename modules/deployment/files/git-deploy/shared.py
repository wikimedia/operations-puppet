...
def main():
    ...
    p = subprocess.Popen("sudo salt-call --json-out pillar.data",
    ...
    try:
        ...
        print "This repo isn't configured. Have you added it in puppet? \
        Exiting."
        return 1

    # Ensure the fetch will work for the repo
    p = subprocess.Popen('git update-server-info', cwd=repodir + '/.git/',
                         shell=True, stderr=subprocess.PIPE)
    err = p.communicate()[0]
    if err:
        print err
    # Ensure the fetch will work for the extensions
    #TODO: make this generic for submodules - it doesn't need to be specific to
    #      extensions
    if os.path.isdir(repodir + '/extensions'):
        extensiondir = repodir + '/extensions'
        p = subprocess.Popen('git submodule foreach "git tag %s"' % tag,
                             cwd=repodir, shell=True, stderr=subprocess.PIPE)
        out = p.communicate()[0]
        for extension in os.listdir(repodir + '/.git/modules/extensions'):
            p = subprocess.Popen('git update-server-info', cwd=extensiondir +
    ...
    print "Running: sudo salt-call publish.runner deploy.checkout '%s,%s'" \
        % (prefix, force)
    p = subprocess.Popen("sudo salt-call publish.runner deploy.checkout \
            '%s,%s'" % (prefix, force), shell=True, stdout=subprocess.PIPE)
    out = p.communicate()[0]
    print(out)
