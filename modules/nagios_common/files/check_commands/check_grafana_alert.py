...
try:
    ...
    print('UNKNOWN: failed to check %s/dashboard/%s' % (args.grafana_url, args.dashboard))
    sys.exit(3)
...
if len(alerting_names) > 0:
    ...
    print('OK: %s/dashboard/%s is not alerting.' % (args.grafana_url, args.dashboard), file=sys.stderr)
    sys.exit(1)
