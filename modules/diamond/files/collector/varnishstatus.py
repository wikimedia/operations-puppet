...
def get_status_counts(varnishtop='/usr/bin/varnishtop', varnishname=None):
    """
    Iterates through the return from parse_varnishtop() and builds a
    dict with request HTTP status counts grouped by by individual HTTP
    status codes, as well as status code classes.  In addition, 'ok'
    is the number of 2xx + 3xx requests, 'error' is the number of
    4xx + 5xx requests,  'total' is the total number of requests,
    and 'availability' is the percent 'ok' requests served.
    """
    status_counts = {
        '2xx': 0,
        '3xx': 0,
