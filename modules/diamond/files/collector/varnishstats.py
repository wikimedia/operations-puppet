...
"""
Diamond collector for counting varnish HTTP requests and groups by several dimensions, including:
  - Total requests
...
If run from the command line, these counts will be printed to stdout.  Otherwise
diamond will send them off to statsd/graphite.
"""
...
def print_counts(counts):
    ...
    keys = counts.keys()
    keys.sort()
    print(datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S').center(31, '-'))
    for k in keys:
...
def init_counter_process(varnishname):
    ...
    counter_process = Process(target=count_requests, args=(shared_dict, varnishname))
...
class VarnishStatusCollector(Collector):
    ...
    def __init__(self, *args, **kwargs):
        ...
        (self.counter_process, self.shared_dict) = init_counter_process(self.config['varnishname'])
...
if __name__ == '__main__':
    ...
    (counter_process, shared_dict) = init_counter_process(arguments.varnishname)
