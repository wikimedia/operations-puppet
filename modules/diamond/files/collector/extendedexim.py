...
EximQueueLine = namedtuple('EximQueueLine', ['age', 'size', 'mail_id', 'frozen'])
...
class ExtendedEximCollector(diamond.collector.Collector):
    ...
    def _get_queue(self):
        ...
        # return only non-indented and non-empty lines
        return [l.strip() for l in queue.split("\n") if l and l[0] not in " \n"]
    ...
    def _parse_line(self, line):
        ...
        return EximQueueLine(age=age, size=size, mail_id=mail_id, frozen=frozen)
    ...
    def collect_queue(self):
        ...
        self.publish('queue.oldest', max(l.age for l in parsed_queue).total_seconds())
        self.publish('queue.youngest', min(l.age for l in parsed_queue).total_seconds())
