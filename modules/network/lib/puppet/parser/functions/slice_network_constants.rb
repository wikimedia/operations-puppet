...
module Puppet::Parser::Functions
  newfunction(:slice_network_constants, :type => :rvalue, :arity => -2) do |args|
    ...
    if !all_network_subnets.key?(realm)
        fail ArgumentError, 'slice_network_constants() realm non existant in networks::constants::all_network_subnets'
    end
    if requested_site and !all_network_subnets[realm].key? requested_site
        fail ArgumentError, "slice_network_constants() site specified must be present in all_network_subnets[#{realm}]. #{requested_site} was provided"
    end
    if requested_sphere and !['public', 'private'].include? requested_sphere
        fail ArgumentError, "slice_network_constants() sphere can only be public/private. #{requested_sphere} was provided"
    end
    if af and !['ipv4', 'ipv6'].include? af
    ...
    result = all_network_subnets[realm].collect { |site, value_site|
        if requested_site and site != requested_site
            next
        end
        value_site.collect { |sphere, value_sphere|
            if requested_sphere and sphere != requested_sphere
                next
            end
            value_sphere.collect { |subnet, value_subnet|
                if description and !(/#{description}/ =~ subnet)
                    next
                end
                if af
                    value_subnet[af]
