...
# sphere has to be public/private, defaults to both if not specified. Also
# 'both' is accepted
...
module Puppet::Parser::Functions
  newfunction(:slice_network_constants, :type => :rvalue, :arity => -2) do |args|
    ...
    all_network_subnets = function_lookup('network::constants::all_network_subnets')
    realm = args[0]
    requested_sphere = args[1] if args.length > 1
    af = args[2] if args.length > 2

    # Make sure we got sane arguments
    if defined? requested_sphere && (!['public', 'private', 'both'].include? requested_sphere)
        fail 'slice_network_constants() sphere can only be public/private/both. #{requested_sphere} was provided'
    end
    if defined? af && (!['ipv4', 'ipv6'].include? af)
    ...
    result = all_network_subnets[realm].collect { |_, value_site|
        value_site.collect { |sphere, value_sphere|
            if defined? requested_sphere && requested_sphere != 'both' && sphere != requested_sphere
                next
            end
