class openstack2::nova::common(
    ...
    file {
        '/etc/nova/nova.conf':
            content => template("openstack2/${version}/nova/common/nova.conf.erb"),
            owner   => 'nova',
            group   => 'nogroup',
            mode    => '0440',
            require => Package['nova-common'];
        '/etc/nova/api-paste.ini':
            content => template("openstack2/${version}/nova/common/api-paste.ini.erb"),
            owner   => 'nova',
            group   => 'nogroup',
            mode    => '0440',
            require => Package['nova-common'];
    ...
    file { '/etc/nova/policy.json':
        source => "puppet:///modules/openstack2/${version}/nova/common/policy.json",
        mode   => '0644',
        owner  => 'root',
        group  => 'root',
    }
