...
from typing import Any, Dict, List, Tuple, Optional, Set, Type
...
class CloudVPS(object):
    ...
    def new_stack(self, host_prefix):
        ...
        if fqdn in self.nova.fqdns():
            log.error(
                f"The server {fqdn} exists in project {self.nova.project_id}, choose a different prefix."
            )
            return False
    ...
    def enroll_hosts(self, role: Optional[str], force=False):
        ...
        if missing_on_server:
            ...
            log.error(f"You might need to push an updated rolemap.yaml")
            return False
