...
class Page(object):
    ...
    def validate_response(cls, response):
        ...
        if response.status_code != 200:
            raise Exception("elasticsearch returned status {0.status_code}: {1}".format(response, response.json()))
...
class EmailMessage(object):
    ...
    def send(self):
        ...
        Deliver this report.
        """
        proc = Popen(["/usr/sbin/exim4", "-i", self.message["To"]], stdout=PIPE, stdin=PIPE, stderr=PIPE)
...
def main():
    ...
    count = 0
    results = {}
    log_message_re = re.compile(r"Writing large partition (?P<partition>.+) \((?P<bytes>[\d]+) bytes\)")
