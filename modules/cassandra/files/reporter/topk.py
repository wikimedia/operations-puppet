...
class ElasticSearch(object):
    ...
    def search(self, index, query):
        ...
        for hit in res.hits:
            yield hit
        for _ in xrange((res.total / ElasticSearch.page_size) + 1):
...
class Page(object):
    ...
    def __init__(self, response):
        ...
        self.__total = json_obj["hits"]["total"]
        self.__hits = json_obj["hits"]["hits"]
...
def index_names(days=7):
    ...
    for i in xrange(days + 1):
        yield (now - datetime.timedelta(days=i)).strftime("logstash-%Y.%m.%d")
...
class EmailMessage(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.message["To"] = kwargs["email_address"]
        self.message["From"] = kwargs["message_from"]

        # Variables used to expand HTML template
        template_vars = {}
        for var in ("cluster_name",
                    "logstash_host",
                    "email_address",
                    "results",
                    "execution_host",
    ...
    def send(self):
        ...
        """
        proc = Popen(
            ["/usr/sbin/exim4", "-i", self.message["To"]],
...
def send_email(values, args, count, exec_start_time):
    ...
    email = EmailMessage(
        ...
        message_from="{}@{}".format(getpass.getuser(), socket.getfqdn()),
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        help="Optionally email the report to a specified address")
    parser.add_argument(
        '-s', '--csv', action='store_true',
        help='Optionally output report in comma-delimited format')
    parser.add_argument(
        "-H", "--logstash-host",
        metavar="HOST",
...
def main():
    ...
    if args.email and args.csv:
        logging.error('--email and --csv are mutually exclusive')
    elif args.email:
        # send the e-mail
