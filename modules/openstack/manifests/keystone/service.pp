    ...
    if $::fqdn == hiera('labs_nova_controller') {
        ...
        cron {
            'cleanup_novaobserver_keystone_tokens':
                ...
                user    => 'root',
                minute  => 30,
                command => "/usr/bin/mysql ${keystone_db_name} -h${keystone_db_host} -u${keystone_db_user} -p${keystone_db_pass} -e 'DELETE FROM token WHERE user_id=\"novaobserver\" AND NOW() + INTERVAL 7 day > expires LIMIT 10000;'",
        }
        cron {
            'cleanup_novaadmin_keystone_tokens':
                ...
                user    => 'root',
                minute  => 40,
                command => "/usr/bin/mysql ${keystone_db_name} -h${keystone_db_host} -u${keystone_db_user} -p${keystone_db_pass} -e 'DELETE FROM token WHERE user_id=\"novaadmin\" AND NOW() + INTERVAL 7 day > expires LIMIT 10000;'",
        }
