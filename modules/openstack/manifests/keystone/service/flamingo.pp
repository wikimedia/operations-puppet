...
class openstack::keystone::service::flamingo(
    ...
    file {
        '/etc/logrotate.d/keystone':
            ...
            group   => 'root',
            mode    => '0644',
            source  => 'puppet:///modules/openstack/flamingo/keystone/keystone_logrotate',
        ...
        '/etc/keystone/keystone.conf':
            ...
            mode      => '0444',
            show_diff => false,
            content   => template('openstack/flamingo/keystone/keystone.conf.erb'),
        ...
        '/etc/keystone/policy.yaml':
            ...
            owner   => 'root',
            group   => 'root',
            source  => 'puppet:///modules/openstack/flamingo/keystone/policy.yaml',
            notify  => Service[$wsgi_server],
            require => Package['keystone'];
        '/etc/keystone/logging.conf':
            ensure  => 'present',
            source  => 'puppet:///modules/openstack/flamingo/keystone/logging.conf',
            owner   => 'root',
            group   => 'root',
            mode    => '0644',
            notify  => Service[$wsgi_server],
        ...
        '/etc/keystone/keystone.my.cnf':
            ...
            mode      => '0400',
            show_diff => false,
            content   => template('openstack/flamingo/keystone/keystone.my.cnf.erb');
        '/usr/lib/python3/dist-packages/wmfkeystoneauth':
            ensure  => 'present',
            owner   => 'root',
            group   => 'root',
            mode    => '0644',
            source  => 'puppet:///modules/openstack/flamingo/keystone/wmfkeystoneauth',
            notify  => Service[$wsgi_server],
            recurse => true;
        '/usr/lib/python3/dist-packages/wmfkeystoneauth.egg-info':
            ensure  => 'present',
            source  => 'puppet:///modules/openstack/flamingo/keystone/wmfkeystoneauth.egg-info',
            owner   => 'root',
            group   => 'root',
            mode    => '0644',
            notify  => Service[$wsgi_server],
    ...
    file {'/etc/keystone/domains/keystone.default.conf':
            ...
            mode      => '0444',
            show_diff => false,
            content   => template('openstack/flamingo/keystone/keystone.default.conf.erb'),
            notify    => Service[$wsgi_server],
            require   => Package['keystone'];
    }
    ...
    file {'/etc/keystone/domains/keystone.toolsbeta.conf':
            ...
            mode      => '0444',
            show_diff => false,
            content   => template('openstack/flamingo/keystone/keystone.toolsbeta.conf.erb'),
            notify    => Service[$wsgi_server],
            require   => Package['keystone'];
    }
}
