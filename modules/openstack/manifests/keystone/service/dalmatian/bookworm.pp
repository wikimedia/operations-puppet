...
class openstack::keystone::service::dalmatian::bookworm(
    ...
    file { '/etc/init.d/keystone':
        mode    => '0755',
        content => template('openstack/dalmatian/keystone/keystone-public-service.erb'),
        require => Package['keystone'];
    }
    file { '/etc/init.d/keystone-admin':
        mode    => '0755',
        content => template('openstack/dalmatian/keystone/keystone-admin-service.erb'),
        require => Package['keystone'];
    }
    service {'keystone':
        ensure  => 'running',
