...
class openstack::keystone::service::zed(
    $controller_hosts,
    $osm_host,
    $db_name,
    $db_user,
    ...
    class { "openstack::keystone::service::zed::${::lsbdistcodename}":
    ...
    file {
        '/etc/logrotate.d/keystone':
            ...
            group   => 'root',
            mode    => '0644',
            source  => 'puppet:///modules/openstack/zed/keystone/keystone_logrotate',
        ...
        '/etc/keystone/keystone.conf':
            ...
            mode      => '0444',
            show_diff => false,
            content   => template('openstack/zed/keystone/keystone.conf.erb'),
        ...
        '/etc/keystone/keystone-paste.ini':
            ...
            group   => 'root',
            mode    => '0644',
            source  => 'puppet:///modules/openstack/zed/keystone/keystone-paste.ini',
            notify  => Service[$wsgi_server],
            require => Package['keystone'];
        '/etc/keystone/policy.yaml':
            ...
            owner   => 'root',
            group   => 'root',
            source  => 'puppet:///modules/openstack/zed/keystone/policy.yaml',
            notify  => Service[$wsgi_server],
            require => Package['keystone'];
        '/etc/keystone/logging.conf':
            ensure  => 'present',
            source  => 'puppet:///modules/openstack/zed/keystone/logging.conf',
            owner   => 'root',
            group   => 'root',
            mode    => '0644',
            notify  => Service[$wsgi_server],
        ...
        '/etc/keystone/keystone.my.cnf':
            ...
            mode      => '0400',
            show_diff => false,
            content   => template('openstack/zed/keystone/keystone.my.cnf.erb');
        '/usr/lib/python3/dist-packages/wmfkeystoneauth':
            ensure  => 'present',
            owner   => 'root',
            group   => 'root',
            mode    => '0644',
            source  => 'puppet:///modules/openstack/zed/keystone/wmfkeystoneauth',
            notify  => Service[$wsgi_server],
            recurse => true;
        '/usr/lib/python3/dist-packages/wmfkeystoneauth.egg-info':
            ensure  => 'present',
            source  => 'puppet:///modules/openstack/zed/keystone/wmfkeystoneauth.egg-info',
            owner   => 'root',
            group   => 'root',
            mode    => '0644',
            notify  => Service[$wsgi_server],
    ...
    file {$patch_file:
        source => 'puppet:///modules/openstack/zed/keystone/hacks/projects.py.patch',
        owner  => 'root',
        group  => 'root',
        mode   => '0644',
    }
    ...
    file {'/etc/keystone/domains/keystone.default.conf':
            ...
            mode      => '0444',
            show_diff => false,
            content   => template('openstack/zed/keystone/keystone.default.conf.erb'),
            notify    => Service[$wsgi_server],
            require   => Package['keystone'];
    }
}
