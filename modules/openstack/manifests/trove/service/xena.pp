...
class openstack::trove::service::xena(
    ...
    file {
        '/etc/trove/trove.conf':
            content   => template('openstack/xena/trove/trove.conf.erb'),
            owner     => 'trove',
            group     => 'trove',
            mode      => '0440',
            show_diff => false,
        ...
        '/etc/trove/trove-guestagent.conf':
            content   => template('openstack/xena/trove/trove-guestagent.conf.erb'),
            owner     => 'trove',
            group     => 'trove',
            mode      => '0440',
            show_diff => false,
        ...
        '/etc/trove/policy.yaml':
            source  => 'puppet:///modules/openstack/xena/trove/policy.yaml',
            owner   => 'trove',
            group   => 'trove',
            mode    => '0644',
            notify  => Service['trove-api', 'trove-taskmanager', 'trove-conductor'],
        ...
        '/etc/trove/api-paste.ini':
            source  => 'puppet:///modules/openstack/xena/trove/api-paste.ini',
            owner   => 'trove',
            group   => 'trove',
            mode    => '0644',
    ...
    #  (merged upstream in version Y)
    $file_to_patch = '/usr/lib/python3/dist-packages/trove/taskmanager/models.py'
    $patch_file = "${file_to_patch}.patch"
    file {$patch_file:
        source => 'puppet:///modules/openstack/xena/trove/hacks/taskmanager/models.py.patch',
        owner  => 'root',
        group  => 'root',
        mode   => '0644',
    }
    exec { "apply ${patch_file}":
        ...
        require => [File[$patch_file], Package['trove-api']],
        notify  => Service['trove-api'],
    }
    ...
    file {$instance_patch_file:
        source => 'puppet:///modules/openstack/xena/trove/hacks/instance/models.py.patch',
        owner  => 'root',
        group  => 'root',
        mode   => '0644',
    }
