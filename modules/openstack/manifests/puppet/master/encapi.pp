class openstack::puppet::master::encapi(
    ...
    $horizon_rule = "'route-remote-addr' => ^${horizon_host_ip}\$ continue:\nroute-if=equal:\${REQUEST_METHOD};POST break:403 Forbidden"
    $labweb_rule = inline_template("<% @labweb_hosts.each do |host|  'route-remote-addr' => ^scope.function_ipresolve([<%= host %>, 4])\$ continue:\nroute-if=equal:\${REQUEST_METHOD};POST break:403 Forbidden<br /><% end %>")
    $labweb_rule_v6 = inline_template("<% @labweb_hosts.each do |host|  'route-remote-addr' => ^scope.function_ipresolve([<%= host %>, 6])\$ continue:\nroute-if=equal:\${REQUEST_METHOD};POST break:403 Forbidden<br /><% end %>")
