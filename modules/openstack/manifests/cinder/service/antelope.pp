...
class openstack::cinder::service::antelope(
    ...
    require 'openstack::cinder::config::antelope'
    ...
    file {
        '/etc/cinder/policy.yaml':
            source  => 'puppet:///modules/openstack/antelope/cinder/policy.yaml',
            owner   => 'cinder',
            group   => 'cinder',
            mode    => '0644',
            require => Package['cinder-api'];
        '/etc/cinder/resource_filters.json':
            source  => 'puppet:///modules/openstack/antelope/cinder/resource_filters.json',
            owner   => 'cinder',
            group   => 'cinder',
            mode    => '0644',
            require => Package['cinder-api'];
        '/etc/init.d/cinder-api':
            content => template('openstack/antelope/cinder/cinder-api'),
            owner   => 'root',
            group   => 'root',
            mode    => '0755',
        ...
        '/etc/cinder/cinder-api-uwsgi.ini':
            source  => 'puppet:///modules/openstack/antelope/cinder/cinder-api-uwsgi.ini',
            owner   => 'root',
            group   => 'root',
            mode    => '0644',
            notify  => Service['cinder-api'],
