class openstack::cinder::service::train(
    ...
    file {
        '/etc/cinder/cinder.conf':
            content   => template('openstack/train/cinder/cinder.conf.erb'),
            owner     => 'cinder',
            group     => 'cinder',
            mode      => '0440',
            show_diff => false,
        ...
        '/etc/cinder/policy.yaml':
            source  => 'puppet:///modules/openstack/train/cinder/policy.yaml',
            owner   => 'cinder',
            group   => 'cinder',
            mode    => '0644',
            require => Package['cinder-api'];
        '/etc/cinder/resource_filters.json':
            source  => 'puppet:///modules/openstack/train/cinder/resource_filters.json',
            owner   => 'cinder',
            group   => 'cinder',
            mode    => '0644',
            require => Package['cinder-api'];
        '/etc/init.d/cinder-api':
            content => template('openstack/train/cinder/cinder-api'),
            owner   => 'root',
            group   => 'root',
            mode    => '0755',
