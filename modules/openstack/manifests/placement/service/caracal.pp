...
class openstack::placement::service::caracal(
    ...
    file {
        '/etc/placement/placement.conf':
            content   => template('openstack/caracal/placement/placement.conf.erb'),
            owner     => 'placement',
            group     => 'nogroup',
            mode      => '0440',
            show_diff => false,
        ...
        '/etc/placement/policy.yaml':
            source  => 'puppet:///modules/openstack/caracal/placement/policy.yaml',
            owner   => 'placement',
            group   => 'placement',
            mode    => '0644',
            require => Package['placement-api'];
        '/etc/init.d/placement-api':
            content => template('openstack/caracal/placement/placement-api.erb'),
            owner   => 'root',
            group   => 'root',
            mode    => '0755',
            notify  => Service['placement-api'],
