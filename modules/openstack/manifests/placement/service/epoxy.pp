...
class openstack::placement::service::epoxy(
    ...
    file {
        ...
        '/etc/placement/policy.yaml':
            source  => 'puppet:///modules/openstack/epoxy/placement/policy.yaml',
            owner   => 'placement',
            group   => 'placement',
            mode    => '0644',
            require => Package['placement-api'];
        '/etc/init.d/placement-api':
            content => template('openstack/epoxy/placement/placement-api.erb'),
            owner   => 'root',
            group   => 'root',
            mode    => '0755',
            notify  => Service['placement-api'],
