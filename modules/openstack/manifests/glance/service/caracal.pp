...
class openstack::glance::service::caracal(
    $db_user,
    $db_pass,
    $db_name,
    $db_host,
    ...
    file {
        '/etc/glance/glance-api.conf':
            content   => template('openstack/caracal/glance/glance-api.conf.erb'),
            owner     => 'glance',
            group     => 'nogroup',
            mode      => '0440',
            show_diff => false,
        ...
        '/etc/glance/policy.yaml':
            source  => 'puppet:///modules/openstack/caracal/glance/policy.yaml',
            owner   => 'root',
            group   => 'root',
            mode    => '0644',
            notify  => Service['glance-api'],
        ...
        '/etc/init.d/glance-api':
            content => template('openstack/caracal/glance/glance-api'),
            owner   => 'root',
            group   => 'root',
            mode    => '0755',
            notify  => Service['glance-api'],
        ...
        '/etc/glance/glance-api-uwsgi.ini':
            content => template('openstack/caracal/glance/glance-api-uwsgi.ini.erb'),
            owner   => 'root',
            group   => 'root',
            mode    => '0755',
            notify  => Service['glance-api'],
    ...
    openstack::patch { '/usr/lib/python3/dist-packages/glance_store/_drivers/rbd.py':
        source  => 'puppet:///modules/openstack/caracal/glance/hacks/rbd.py.patch',
        require => Package['glance'],
        notify  => Service['glance-api'],
    }
}
