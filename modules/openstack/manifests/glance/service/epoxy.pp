...
class openstack::glance::service::epoxy(
    $db_user,
    $db_pass,
    $db_name,
    $db_host,
    ...
    file {
        '/etc/glance/glance-api.conf':
            content   => template('openstack/epoxy/glance/glance-api.conf.erb'),
            owner     => 'glance',
            group     => 'nogroup',
            mode      => '0440',
            show_diff => false,
        ...
        '/etc/glance/policy.yaml':
            source  => 'puppet:///modules/openstack/epoxy/glance/policy.yaml',
            owner   => 'root',
            group   => 'root',
            mode    => '0644',
            notify  => Service['glance-api'],
        ...
        '/etc/init.d/glance-api':
            content => template('openstack/epoxy/glance/glance-api'),
            owner   => 'root',
            group   => 'root',
            mode    => '0755',
            notify  => Service['glance-api'],
        ...
        '/etc/glance/glance-api-uwsgi.ini':
            content => template('openstack/epoxy/glance/glance-api-uwsgi.ini.erb'),
            owner   => 'root',
            group   => 'root',
            mode    => '0755',
            notify  => Service['glance-api'],
