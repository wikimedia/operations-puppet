...
class openstack::heat::service::antelope(
    String $db_user,
    ...
    file {
        '/etc/heat/heat.conf':
            content   => template('openstack/antelope/heat/heat.conf.erb'),
            owner     => 'heat',
            group     => 'heat',
            mode      => '0440',
            show_diff => false,
        ...
        '/etc/heat/policy.yaml':
            source  => 'puppet:///modules/openstack/antelope/heat/policy.yaml',
            owner   => 'root',
            group   => 'root',
            mode    => '0644',
            notify  => Service['heat-api', 'heat-engine', 'heat-api-cfn'],
        ...
        '/etc/init.d/heat-api':
            ...
            group   => 'root',
            mode    => '0755',
            content => template('openstack/antelope/heat/heat-api.erb');
        '/etc/init.d/heat-api-cfn':
            owner   => 'root',
            group   => 'root',
            mode    => '0755',
            content => template('openstack/antelope/heat/heat-api-cfn.erb');
    }
}
