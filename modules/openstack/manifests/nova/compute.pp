class openstack::nova::compute(
    ...
    if ( $::realm == 'production' ) {
        ...
        class openstack::nova::compute::rsync { "novainstances_${hostname}":
        }
        Openstack::nova::compute::rsync <<| |>>
    ...
    if os_version('ubuntu >= trusty') and (versioncmp($::kernelrelease, '3.13.0-46') < 0) {
        ...
        fail('nova-compute not installed on buggy kernels.  On 3.19 series kernels, instance clocks die after resuming from suspension.')
    } else {
...
class openstack::nova::compute::rsync {
    # Support rsyncing instance files between labvirts.
