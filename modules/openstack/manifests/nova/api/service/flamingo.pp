...
class openstack::nova::api::service::flamingo(
    ...
    openstack::patch { '/usr/lib/python3/dist-packages/nova/api/openstack/compute/servers.py':
        source  => 'puppet:///modules/openstack/flamingo/nova/hacks/servers.py.patch',
        require => Package['nova-api'],
        notify  => Service['nova-api'],
    }

    file { '/etc/init.d/nova-api-metadata':
        content => template('openstack/flamingo/nova/api/nova-api-metadata'),
        owner   => 'root',
        group   => 'root',
        mode    => '0755',
    ...
    file { '/etc/nova/nova-api-metadata-uwsgi.ini':
        content => template('openstack/flamingo/nova/api/nova-api-metadata-uwsgi.ini.erb'),
        owner   => 'root',
        group   => 'root',
        mode    => '0755',
        notify  => Service['nova-api-metadata'],
    ...
    file { '/etc/nova/nova-api-uwsgi.ini':
        content => template('openstack/flamingo/nova/api/nova-api-uwsgi.ini.erb'),
        owner   => 'root',
        group   => 'root',
        mode    => '0755',
        notify  => Service['nova-api-metadata'],
