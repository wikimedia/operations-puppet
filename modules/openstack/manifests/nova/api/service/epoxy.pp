...
class openstack::nova::api::service::epoxy(
    ...
    file { '/etc/init.d/nova-api':
        content => template('openstack/epoxy/nova/api/nova-api'),
        owner   => 'root',
        group   => 'root',
        mode    => '0755',
    ...
    openstack::patch { '/usr/lib/python3/dist-packages/nova/api/openstack/compute/servers.py':
        source  => 'puppet:///modules/openstack/epoxy/nova/hacks/servers.py.patch',
        require => Package['nova-api'],
        notify  => Service['nova-api'],
    }

    file { '/etc/init.d/nova-api-metadata':
        content => template('openstack/epoxy/nova/api/nova-api-metadata'),
        owner   => 'root',
        group   => 'root',
        mode    => '0755',
    ...
    file { '/etc/nova/nova-api-metadata-uwsgi.ini':
        content => template('openstack/epoxy/nova/api/nova-api-metadata-uwsgi.ini.erb'),
        owner   => 'root',
        group   => 'root',
        mode    => '0755',
        notify  => Service['nova-api-metadata'],
    ...
    file { '/etc/nova/nova-api-uwsgi.ini':
        content => template('openstack/epoxy/nova/api/nova-api-uwsgi.ini.erb'),
        owner   => 'root',
        group   => 'root',
        mode    => '0755',
        notify  => Service['nova-api-metadata'],
