        ...
        for source_group in source_groups:
            ...
            for rule in source_group.rules:
                ...
                if "cidr" in rule["ip_range"] and (
                    rule["ip_range"]["cidr"] == "10.0.0.0/8"
                    or rule["ip_range"]["cidr"] == "10.4.0.0/16"
                ):
...
if __name__ == "__main__":
    ...
    argparser = argparse.ArgumentParser(
        "region-migrate-security-groups",
        description="Copy security groups " "from one region to another",
    )
    argparser.add_argument("project", help="project to migrate")
    args = argparser.parse_args()

    project = NovaProject(args.project, config["common"], config["source"], config["dest"])
