...
from wikimediapuppettab import puppet_tables
from wikimediapuppettab import views

urlpatterns = patterns(
    '',
    url(r'^$', panel.IndexView.as_view(), name='index'),
    url(r'^(?P<instanceid>[^/]+)/(?P<roleid>[^/]+)/applypuppetrole$',
        puppet_tables.ApplyView.as_view(), name='applypuppetrole'),
    url(r'^(?P<fqdn>[^/]+)/(?P<tenantid>[^/]+)/(?P<instanceid>[^/]+)/edithiera$',
