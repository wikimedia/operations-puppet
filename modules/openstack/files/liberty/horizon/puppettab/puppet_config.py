...
class puppet_config():
    ...
    def refresh(self):
        ...
        # unknown_classes is a list of roles that weren't enumerated by the puppet API.
        ...
        for role in list(self.roles):
            if role in allrole_dict.keys():
                ...
                #  params.
                self.unknown_classes.append(role)
                self.roles.remove(role)
            else:
                # Sometimes we get empty strings from crappy parsing
                self.roles.remove(role)

        self.unknown_classes_text = "\n".join(self.unknown_classes)
    ...
    def set_unknown_class_list(self, unknown_class_list):
        self.set_roles(unknown_class_list + self.roles)

    def set_role_list(self, role_list):
        self.set_roles(role_list + self.unknown_classes)
