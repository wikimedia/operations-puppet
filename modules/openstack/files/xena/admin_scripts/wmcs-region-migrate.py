...
class NovaInstance(object):
    ...
    def get_proxy_endpoint(self, keystoneclient, region):
        ...
        endpoints = keystoneclient.endpoints.list(service=serviceid, region=region)
                ...
                for recordset in recordsets:
                    if source_ip in recordset["records"]:
                        ...
                        client.recordsets.update(
                            zone["id"], recordset["id"], {"records": recordset["records"]}
                        )
    ...
    def ssh_test(self, source=True, command="hostname"):
        ...
        if command == "hostname":
            if r.decode("utf-8").strip().lower() != instance.name.lower():
                print(
                    "ssh test to %s failed.  Returned hostname %s but expected %s"
    ...
    def wait_for_status(self, desiredstatus, source=True):
        ...
        while watched_instance.status != desiredstatus:
            if watched_instance.status != oldstatus:
                ...
                print(
                    "Current status for %s is %s; waiting for it to change to %s."
    ...
    def make_destination_vm(self, name, image_id, flavor_id):
        ...
        self.create_novaclient = novaclient.Client(
            "2", session=create_session, region_name=self.dest_config["region"]
        )
    ...
    def migrate(self):
        ...
        print(
            "Source instance %s is now on host %s with state %s"
            % (self.instance_id, source_host, self.source_instance.status)
        )
        ...
        sshargs = [
            ...
            "nova@%s.%s" % (self.dest_host, self.dnsdomain),
            "true",
        ]
        ...
        args = [
            "mysql",
            "--user=%s" % self.dest_config["db_username"],
            "--password=%s" % self.dest_config["db_password"],
            "--host",
            self.dest_config["db_hostname"],
            self.dest_config["db_name"],
            "-e",
            'update instances set user_id="%s" '
            'where uuid="%s";' % (self.user_id, self.dest_instance_id),
        ]
        r = subprocess.call(args)
...
if __name__ == "__main__":
    ...
    argparser = argparse.ArgumentParser(
        "region-migrate", description="Move an instance to a " "different region"
    )
    argparser.add_argument("instanceid", help="instance id to migrate")
    args = argparser.parse_args()

    instance = NovaInstance(args.instanceid, config["common"], config["source"], config["dest"])
    instance.migrate()
