    ...
    try:
        ...
        if 'error' in error_body and 'root_cause' in error_body['error']:
            ...
            if root_cause['type'] == 'invalid_regex_exception':
                sys.stderr.write('Error parsing regex: {0}\n{1}\n'
                                 .format(args.term, root_cause['reason']))
            else:
                sys.stderr.write('Unknown error: \
                                 {0}\n'.format(root_cause['reason']))
        else:
            sys.stderr.write('Received unexpected json body \
                             from elasticsearch: \n{0}\n'
                             .format(json.dumps(error_body,
                                                indent=4,
                                                separators=(',', ': '))))
    except ValueError, e:
        sys.stderr.write("Error '{0}' while parsing \
