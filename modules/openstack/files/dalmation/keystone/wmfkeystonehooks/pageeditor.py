...
wiki_opts = [
    cfg.StrOpt(
        "wiki_host",
        default="test.wikipedia.beta.wmflabs.org",
...
class PageEditor:
    ...
    def edit_page(
        ...
        LOG.debug("Writing wiki page http://%s/wiki/%s" % (self.host, pagename))
        ...
        if failed and not second_try:
            self.edit_page(page_string, resource_name, delete_page, second_try=True)
