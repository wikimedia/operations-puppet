...
class eventlogging::deployment::target($path = '/srv/deployment/eventlogging/eventlogging') {
    ...
    # New Debian systemd based host use scap3.
        ...
        # Allow eventlogging user to sudo -u eventlogging.
        # (This is needed by scap3???)
        sudo::user { 'scap_eventlogging':
            user => 'eventlogging',
            privileges => 'ALL=(eventlogging) NOPASSWD: ALL'
        }
        ...
        # This needs to be writable by deploy user (eventlogging)
        # dirname() stdlib puppet function not available?
        $parent_dir = inline_template('<%= File.dirname(@path) %>')
