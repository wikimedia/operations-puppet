...
)
{
    ...
    if $log_config_template {
        ...
        file { $log_config:
            ...
            before  => Base::Service_unit["eventlogging-service-${basename}"],
        }
    }
    ...
    nrpe::monitor_service { "eventlogging-service-${basename}":
        description  => "Check that eventlogging-service-${basename} is running",
        nrpe_command => "/usr/lib/nagios/plugins/check_procs -c 1:1 -C '${$executable_path} @${config_file}",
        require      => Base::Service_unit["eventlogging-service-${basename}"],
    }
}
