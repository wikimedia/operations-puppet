...
def extract_lag(json_response):
    date = datetime.strptime(json_response['results']['bindings'][0]['mindate']['value'], '%Y-%m-%dT%H:%M:%SZ')
    lag = datetime.utcnow() - date
    return lag


def execute_query(base_url, timeout, query):
    url = '{base_url}?format=json&query={query}'.format(base_url=base_url, query=query)
