...
def count_requests(request_tuples, shared_dict):
    ...
    for (http_method, http_status) in request_tuples:
        # Only count this request if it is valid
        if (validate_http_method(http_method) and
...
def varnishncsa_request_tuples(varnishname=None):
    """
...
class TestVarnishstats(unittest.TestCase):
    def test_validate_http_method(self):
        self.assertTrue(validate_http_method('GET'))
        self.assertTrue(validate_http_method('get'))
        self.assertTrue(validate_http_method('post'))
        self.assertFalse(validate_http_method('nogood'))

    def test_validate_http_status(self):
        self.assertTrue(validate_http_status('200'))
        self.assertTrue(validate_http_status('404'))
        self.assertTrue(validate_http_status(301))
        self.assertFalse(validate_http_method('nogood'))
        self.assertFalse(validate_http_method('1000'))
