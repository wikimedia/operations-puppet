...
"""
  ...
  Usage:
    ...
    def my_callback(tag, value, transaction_id, remote_party):
      print(tag, value, transaction_id, remote_party)
...
def varnishlog(vsl_args, callback):
    ...
    def _vsl_handler(priv, tag_id, fd, length, spec, ptr, bitmap):
        ...
        callback(tag, record, fd, remote_party)
        return 0
...
def parse_cli_vsl_args(args):
    ...
    passing to vanrnishlog.  These are not checked to
...
def print_tags_callback(tag, value, transaction_id, remote_party):
    print("%s\t%s\t%s\t%s" % (tag, value, transaction_id, remote_party))
