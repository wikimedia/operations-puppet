...
class VarnishTLSInpector(object):
    ...
    def __init__(self, argument_list):
        ...
        self.cmd = [
            ...
            '-q', 'ReqMethod ne PURGE and ReqHeader:X-Forwarded-Proto eq https and ReqHeader:X-CP-Key-Exchange eq RSA',
    ...
    def handle_line(self, line):
        ...
        if tag == 'Begin':
            ...
            if 'request-User-Agent' in self.tx and 'log-CP-Full-Cipher' in self.tx:
                self.logger.info(self.tx['log-CP-Full-Cipher'] + self.tx['request-User-Agent'], extra=self.tx)
            self.tx = {}
        elif tag == 'ReqHeader':
            ...
            if log_name in ['CP-TLS-Version', 'CP-Key-Exchange', 'CP-Auth', 'CP-Cipher', 'CP-Full-Cipher']:
                if len(splitagain) == 2:
