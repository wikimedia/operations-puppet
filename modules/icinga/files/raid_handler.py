...
PHABRICATOR_CONFIG_FILE = '/etc/phabricator_opsbot.conf'
...
def get_phabricator_client():
    """Return an instantiated Phabricator client"""
...
def acknowledge_alert(host, service_description, task_uri):
...
def main():
    ...
    url = ICINGA_URL.format(
        host=args.host_address, service=args.service_description)
    task = open_phabricator_task(
        phab_client, project_ids, args.host_address, raid_status, url)

    acknowledge_alert(args.host_address, args.raid_type,
                      args.service_description, task['uri'])
