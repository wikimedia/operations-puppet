...
def send_email(mail_to,
    ...
    mail_info_line = "Sending message '{}' to '{}' from '{}' via '{} (dry_run = {}).\n"
    print(mail_info_line.format(mail_msg, contact_addr, MAIL_FROM, MAIL_SERVER, dry_run))
...
if __name__ == "__main__":
    ...
    # list all possible (SMS'able) contacts along with their address1 (usage: -f | --listfull
    if args.listfull:
    ...
    # send SMS (via E-mail gateway) to an Icinga contact (usage: -s | --send <contact name>)
    if args.send:
        contact_name = sys.argv[2]
        mail_msg = sys.argv[3]
        contact_addr = get_address(contact_name)
    ...
    # send SMS (via E-mail gateway) to ALL SMS'able Icinga contacts (usage: -x | --sendall <message>)
    if args.sendall:
        contact_list = list_contacts_sms(CONTACTS_FILE)
        mail_msg = sys.argv[2]
        num_contacts = len(contact_list)
        print("{} contacts found. Are you sure you want to send '{}' to all of them? You can answer 'yes', 'no' or 'dry' for a dry-run.".format(num_contacts, mail_msg, dry_run))
        yes = {'yes','y'}
        no = {'no','n', ''}
        dry = {'dry', 'd'}
        choice = input().lower()
        if choice in yes or choice in dry:
            ...
            print("Please respond with 'yes (y)' or 'no (n)' or 'dry (d)'. Canceled.")
            exit(0)
