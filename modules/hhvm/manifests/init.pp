...
class hhvm(
    ...
    file { '/usr/lib/hphp/extensions/current':
        ensure  => link,
        target  => $::hhvm_extension_path,
        require => Package['hhvm-fss', 'hhvm-luasandbox', 'hhvm-wikidiff2'],
        before  => File['/etc/hhvm/php.ini', '/etc/hhvm/fcgi/php.ini'],
    }
}
