...
class hhvm(
    ...
    # Prune stale symbol translation maps from /tmp.
    # These files are generated by HHVM to supply `perf` with
    # language-level context.

    exec { 'remove_old_perf_maps':
        command => '/usr/bin/find /tmp -name "perf-*.map" -not -name perf-`/bin/pidof -s hhvm`.map -delete',
        onlyif  => '/usr/bin/find /tmp -name "perf-*.map" -not -name perf-`/bin/pidof -s hhvm`.map -print -quit | /bin/grep -q .',
    }
}
