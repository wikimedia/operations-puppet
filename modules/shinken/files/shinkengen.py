...
if __name__ == '__main__':
    ...
    for project in config['projects']:
        instances = clients.allinstances(projectid=project, allregions=True)
        host_configs = []

        for instance in instances:
            ip = ""
            for value in instance.addresses.values():
                for ipentry in value:
                    if ipentry['OS-EXT-IPS:type'] == 'fixed':
                        ip = str(ipentry['addr'])
            if not ip:
                # This VM is broken in some serious way; skip.
                next
