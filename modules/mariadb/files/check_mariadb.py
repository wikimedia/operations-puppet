...
def icinga_check(options):
    ...
    if options.num_processes is not None:
        if 'mysqld_processes' not in status:
            ...
            if num_processes != options.num_processes:
                crit_msg.append('{} mysqld process(es) running, expected {}'. format(num_processes, options.num_processes))
            else:
    ...
    if options.crit_connections is not None and status['threads_connected'] >= options.crit_connections:
        crit_msg.append('{} client(s)'.format(status['threads_connected']))
    elif options.warn_connections is not None and status['threads_connected'] >= options.warn_connections:
