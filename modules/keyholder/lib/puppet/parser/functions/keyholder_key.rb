  ...
  newfunction(:keyholder_key, :type => :rvalue, :arity => -2) do |args|
    ...
    begin
      ...
      if !File.exists?("#{fullpath}/#{key_name}") && args[1] == true
        ...
        raise "ssh-keygen return code is #{$CHILD_STATUIS.to_i}" unless $CHILD_STATUS.success?
      end
    rescue => e
    ...
    if args[2] == 'fingerprint'
      return %x(`/usr/bin/ssh-keygen -l -f #{fullpath}/#{key_name}`).strip
    elsif args[2] == 'pubkey'
      return IO.read("#{fullpath}/#{key_name}.pub")
    else
      return IO.read("#{fullpath}/#{key_name}")
    end
  end
end
