...
# $limn::user           - Limn instance will run as this user.  Default: limn
# $limn::group          - Limn instance will run in this group.  Default: limn
  ...
  $ensure         = 'present')
{
  ...
  if ($proxy) {
    limn::instance::proxy { $name:
      limn_port     => $port,
      document_root => "${base_directory}/var",
    }
  }
