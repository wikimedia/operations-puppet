...
then
  ...
  # the tail|sed|cut is just to get the number of the
  # unpartitioned span on the disk from parted
  ...
  then
    # this grep|tail|sed|sed|cut is to make sure
    # we have a partitionned space, and that we
