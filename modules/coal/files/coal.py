...
class WhisperLogger(object):
    def __init__(self, args):
        ...
        for schema in self.args.schemas:
            self.oldest_boundary[schema] = int(time.time() - (time.time() % 60))
                ...
                for message in consumer:
                    ...
                    if 'error' in message:
                        ...
                        if offset_to_commit is None:
                            ...
                            # Kafka wants you to commit the NEXT offset to be
                            # read, not the LAST offset read
                            offset_to_commit = offset_to_commit + 1

                        # Need to commit!
                        self.log.info('[{}] Committing offset {}'.format(
                                        value['schema'], offset_to_commit))
                        consumer.commit({
                            TopicPartition(message.topic, message.partition):
                                OffsetAndMetadata(offset_to_commit, None)
                            })
                        self.log.debug('[{}] Committed offset {}'.format(
                                        value['schema'], offset_to_commit))
