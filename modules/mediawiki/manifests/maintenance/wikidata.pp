class mediawiki::maintenance::wikidata( $ensure = present ) {
    ...
    cron { 'wikibase-rebuildTermSqlIndex':
        ...
        command => 'timeout 3500s /usr/local/bin/mwscript extensions/Wikidata/extensions/Wikibase/repo/maintenance/rebuildTermSqlIndex.php --wiki wikidatawiki --entity-type=item --batch-size 500 --sleep 50 --from-id $(ls /var/log/wikidata/rebuildTermSqlIndex.log* | sort -n -r | tail -n 100 | xargs -d '\\n' grep -h -E "Processed up to page (\d+?)" /var/log/wikidata/rebuildTermSqlIndex.log* | tail | sed -E "s/Processed up to page //; s/ \(Q.+?//" | tail -1) >> /var/log/wikidata/rebuildTermSqlIndex.log 2>&1',
        user    => $::mediawiki::users::web,
        minute  => 30,
        hour    => '*',
        weekday => '*'
