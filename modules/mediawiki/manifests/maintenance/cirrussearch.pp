class mediawiki::maintenance::cirrussearch( $ensure = present ) {
    ...
    cron { 'cirrus_sanitize_jobs':
        command => '/usr/local/bin/expanddblist all | xargs -I{} sh -c \'/usr/local/bin/mwscript extensions/CirrusSearch/maintenance/saneitizeJobs.php --wiki={} --push --refresh-freq=7200 >> /var/log/mediawiki/cirrus-sanitize/push-jobs.{}.log 2>&1 || true\'',
        minute  => 10,
        hour    => '*/2',
    }
