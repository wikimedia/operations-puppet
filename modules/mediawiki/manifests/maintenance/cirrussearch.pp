class mediawiki::maintenance::cirrussearch( $ensure = present ) {
    ...
    cron { 'cirrus_build_completion_indices_eqiad':
        command => '/usr/local/bin/expanddblist all | xargs -I{} -P 4 sh -c \'PHP=php7.2 /usr/local/bin/mwscript extensions/CirrusSearch/maintenance/updateSuggesterIndex.php --wiki={} --masterTimeout=10m --replicationTimeout=5400 --indexChunkSize 3000 --cluster=eqiad --optimize > /var/log/mediawiki/cirrus-suggest/{}.eqiad.log 2>&1 || true\'',
        minute  => 30,
        hour    => 2,
    }

    cron { 'cirrus_build_completion_indices_codfw':
        command => '/usr/local/bin/expanddblist all | xargs -I{} -P 4 sh -c \'PHP=php7.2 /usr/local/bin/mwscript extensions/CirrusSearch/maintenance/updateSuggesterIndex.php --wiki={} --masterTimeout=10m --replicationTimeout=5400 --indexChunkSize 3000 --cluster=codfw --optimize > /var/log/mediawiki/cirrus-suggest/{}.codfw.log 2>&1 || true\'',
        minute  => 30,
        hour    => 2,
    }
