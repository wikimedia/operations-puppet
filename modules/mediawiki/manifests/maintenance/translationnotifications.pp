class mediawiki::maintenance::translationnotifications( $ensure = present ) {
    ...
    cron { 'translationnotifications-metawiki':
        ...
        command => 'PHP=php7.2 /usr/local/bin/mwscript extensions/TranslationNotifications/scripts/DigestEmailer.php --wiki metawiki >> /var/log/translationnotifications/digests.log 2>&1',
        weekday => 1, # Monday
        hour    => 10,
        minute  => 0,
    }
    ...
    cron { 'translationnotifications-mediawikiwiki':
        ...
        user    => $::mediawiki::users::web,
        command => 'PHP=php7.2 /usr/local/bin/mwscript extensions/TranslationNotifications/scripts/DigestEmailer.php --wiki mediawikiwiki >> /var/log/translationnotifications/digests.log 2>&1',
        weekday => 1, # Monday
        hour    => 10,
        minute  => 5,
    }
