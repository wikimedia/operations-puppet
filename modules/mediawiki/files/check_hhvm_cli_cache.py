...
def main():
    ...
    if cache_size > critical_threshold:
        print("CRITICAL: HHVM CLI cache is %d bytes (%d MiB)" % (cache_size, cache_size / 1048576))
        sys.exit(2)
    elif cache_size > warning_threshold:
        print("WARNING: HHVM CLI cache is %d bytes (%d MiB)" % (cache_size, cache_size / 1048576))
        sys.exit(1)
    else:
        print("OK: HHVM CLI cache is %d bytes (%d MiB)" % (cache_size, cache_size / 1048576))
        sys.exit(0)
