...
Each table entry can have these options:
 ...
 - canonicality (required): One of the following options:
   - canonical: Canonical data whose loss would cause irreversible damage to data integrity. Examples: revision, user, ...
   - canonical with acceptable loss: Canonical data but its loss wouldn't be considered too bad. Examples: recentchanges, cu_changes, ...
   - derivative: Mostly or all derivative data that can be regenerated from the content of pages. Examples: pagelinks, geo_tags, linter, ...
 - visibility (required): Determining data should be replicated to cloud replicas. One of the following options:
   - public: All of the data can be queried by anyone in the internet. Example: pagelinks
