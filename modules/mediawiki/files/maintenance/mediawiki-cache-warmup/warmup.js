...
var urlList = util.reduceTxtLines( fs.readFileSync( process.argv[ 2 ] ).toString().split( '\n' ) );
...
function doRequests( urlList, target ) {
	var target_str = target ? target : 'debug-clone';
	...
	//	}
		...
		return util.worker(
			...
			function ( uri ) {
				...
				console.log( `[${new Date().toISOString()}] Request ${uri} (${target_str})` );
