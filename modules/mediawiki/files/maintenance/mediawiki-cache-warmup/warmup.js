...
if ( mode === 'clone' ) {
	let cluster, dc, confctlCmd, confctlData, serverList, key;
	cluster = process.argv[ 4 ] ? process.argv[ 4 ] : 'appserver';
	dc = process.argv[ 5 ] ? process.argv[ 5 ] : 'codfw';

	// This mode runs each of the listed urls on each of the servers.
	// This is meant for warming up APC caches on each app server.
	confctlCmd = 'confctl tags \'dc=' + dc + ',cluster=' + cluster + ',service=apache2\' --action get all';
	...
	let target = process.argv[ 4 ] ? process.argv[ 4 ] : 'appservers.svc.codfw.wmnet';
