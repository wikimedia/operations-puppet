...
Environment Variables:
  ...
  KAFKA_BOOTSTRAP_SERVERS - If this is set, any commands that take a --broker-list or
                            ...
                            Also any command that take a --authorizer-property
...
# Set ZOOKEEPER_CONNECT_OPT if KAFKA_ZOOKEEPER_URL is set and '--authorizer-property zookeeper.connect'
# has not also been passed in as a CLI arg.  This will be included
# in command functions that take '--authorizer-property zookeeper.connect' argument.
if [ -n "${KAFKA_ZOOKEEPER_URL}" -a -z "$(echo $@ | egrep -- '--authorizer-property\ *zookeeper\.connect')" ]; then
