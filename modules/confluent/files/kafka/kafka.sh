...
broker_list_commands="kafka-console-producer kafka-consumer-perf-test kafka-replica-verification kafka-simple-consumer-shell kafka-verifiable-consumer kafka-verifiable-producer"
zookeeper_commands="kafka-configs kafka-console-consumer kafka-consumer-groups kafka-consumer-perf-test kafka-preferred-replica-election kafka-reassign-partitions kafka-replay-log-producer kafka-topics"

EXTRA_OPTS=""
echo "${broker_list_commands}" | /bin/grep -q "${script}" && EXTRA_OPTS="${EXTRA_OPTS} ${BROKER_LIST_OPT}"
echo "${zookeeper_commands}" | /bin/grep -q "${script}" && EXTRA_OPTS="${EXTRA_OPTS} ${ZOOKEEPER_OPT}"
