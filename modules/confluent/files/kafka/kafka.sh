...
Environment Variables:
  ...
  ZOOKEEPER_URL           - If this is set, any command that take a --authorized-property
...
# Set ZOOKEEPER_CONNECT_OPT if ZOOKEEPER_URL is set and '--authorized-property zookeeper.connect'
...
if [ -n "${ZOOKEEPER_URL}" -a -z "$(echo $@ | egrep -- '--authorized-property\ *zookeeper\.connect')" ]; then
    ZOOKEEPER_CONNECT_OPT="--authorized-property zookeeper.connect=${ZOOKEEPER_URL}"
fi
