...
class dynomite(
    ...
    $seeds_by_region = $store_servers.map |$region, $servers| {
        $servers.map |$server, $token| {
            # Exclude the node itself from its seed list (https://github.com/Netflix/dynomite/issues/423)
            if ( $server == $::hostname ) {
                next()
            }
            "${server}:${port}:${pool}:${region}:${token}"
        }
    }
    ...
    file { '/etc/default/dynomite':
        ensure  => $ensure,
        content => template('dynomite/default.erb'),
        owner   => 'root',
        group   => 'root',
        mode    => '0444',
        notify  => Service['dynomite'],
    }

    file { '/etc/init/dynomite.override':
        ensure  => $ensure,
