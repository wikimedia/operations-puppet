class base::grub($ioscheduler = 'deadline', $enable_memory_cgroup = false) {
    ...
    if versioncmp($::augeasversion, '1.2.0') >= 0 and os_version('Debian >= jessie') {
        $cgroup_line = $enable_memory_cgroup ? {
            true => 'set GRUB_CMDLINE_LINUX_DEFAULT/value[. = "cgroup_enable=memory"] cgroup_enable=memory',
            false => 'rm GRUB_CMDLINE_LINUX_DEFAULT/value[. = "cgroup_enable=memory"]'
        }
        $swapaccount_line = $enable_memory_cgroup ? {
            true => 'set GRUB_CMDLINE_LINUX_DEFAULT/value[. = "swapaccount=1"] swapaccount=1',
            false => 'rm GRUB_CMDLINE_LINUX_DEFAULT/value[. = "swapaccount=1"]'
        }
