...
describe 'pick_initscript' do
  ...
  it 'Fails if on systemd and only upstart is provided, and vice versa' do
    should run.with_params('upstart', true, false, false).and_raise_error(ArgumentError)
    should run.with_params('systemd', false, true, false).and_raise_error(ArgumentError)
  end
