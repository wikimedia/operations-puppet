...
class NutcrackerCollector(diamond.collector.Collector):
    ...
    def get_nutcracker_stats(self, host, port):
        try:
            ...
            sock.settimeout(1)
            raw_data = sock.recv(65536).decode('utf-8', errors='ignore').strip()
        ...
        # Nutcracker does not properly escape quotation marks in server aliases.
        data = re.sub(r'"("\w+")"(?=:)', r'\1', raw_data)
        stats = json.loads(data)

        # The pool names are all the keys that aren't part of standard set of keys.
