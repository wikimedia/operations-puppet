...
class ExternalCloudVendor:
    ...
    def get_networks(self, session: Session) -> Set[str]:
        ...
        nets = {prefix.get(key) for key in self.subkeys for prefix in data[self.prefixes]}
        nets.discard(None)
        return nets
...
class ExternalCloudVendorAzure:
    ...
    def get_networks(self, session: Session) -> Set[str]:
        ...
        nets = {
            prefix for item in ips["values"] for prefix in item["properties"]["addressPrefixes"]
        }
        return nets
    ...
    def get_networks(self, session: Session) -> Set[str]:
        ...
        for asn in self.asns:
            data = session.get(
                ...
                f"{int(time.time()-86400)}&min_peers_seeing=10"
            ).json()
...
class CSVExternalCloudVendor:
    ...
    def get_networks(self, session: Session) -> Set[str]:
        ...
        lines = (line for line in ips_request.text.splitlines() if not line.startswith("#"))
        ips = csv.DictReader(
            lines,
...
def requestctl_apply(slug, data, api_token) -> subprocess.CompletedProcess:
    ...
    try:
        ...
        result = subprocess.run(
            cmd, check=True, text=True, capture_output=True, env={"REQUESTCTL_API_TOKEN": api_token}
        )
        ...
        for entity in entities:
            try:
                ...
                if len(nets) == 0:
                    ...
                    runtime_error = True
                    continue
                logging.debug("%s nets: %s", entity.name, nets)
                logging.info(
                    "%s new nets: %d",
                    entity.name,
                    len(nets),
            ...
            try:
                slug = f"{ipblock_type}/{entity.name.lower()}"
                to_update = {
                    "cidrs": nets,
                    "comment": f"Automatically generated IPs for {entity.name}",
                }
