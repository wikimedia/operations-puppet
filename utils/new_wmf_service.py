...
QUESTIONS = [
    {
        ...
        'validator':
            lambda x: True if re.match('^[a-z_]{6,20}$', x) else False,
    ...
    },
    {
        ...
        'validator':
            lambda x: True if re.match('^[Yy]([Ee][Ss])?|[Nn]([Oo])?|$', x)
            else False,
        'transformer':
            lambda x: True if re.match('^[Yy]([Ee][Ss])?|$', x) else False,
    },
    {
        'qname': 'port',
    ...
    },
    {
        ...
        'validator': lambda x:
        True if re.match('^http(s)?://gerrit.wikimedia.org/r/', x) else False,
        'transformer': lambda x: x,
    },
    {
        'qname': 'lvs_class',
        'qstring': 'Which LVS class shall serve it ' +
                   '(high-traffic1, high-traffic2, low-traffic)',
    ...
    },
    {
        ...
        'validator': lambda x: True if re.match(
            '^[a-z0-9]+\.svc\.(eqiad|codfw)\.wmnet$', x) else False,
        'transformer': lambda x: x,
    },
    {
        'qname': 'lvs_ip',
        'qstring': 'LVS IP',
        'validator':
            lambda x: True if re.match('^10(\.[0-9]{1,3}){3}$', x) else False,
        'transformer': lambda x: x,
    },
    {
        'qname': 'point_person',
...
''')
        ...
        # Populate templates
        with open('modules/%s/templates/config.yaml.erb' %
                  self.service_name, 'w') as f:
            f.write('# Generated by new_wmf_service.py.\n{}\n')

        # Populate manifests
        if self.service_node:
            with open('modules/%s/manifests/init.pp' %
                      self.service_name, 'w') as f:
                f.write('''
# Class: %(name)s
#
...
def main():
    ...
    Git.commit('Setup LVS for %s service on %s cluster' %
               (s.service_name, s.cluster))
...
def handle_args():
    parser = argparse.ArgumentParser(
        description='shell helper for automating introduction of new services')
