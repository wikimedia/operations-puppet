...
class openstack::database-server($openstack_version="essex", $novaconfig, $keystoneconfig, $glanceconfig) {
	...
	exec {
		'set_root':
			...
			require => [File["/etc/nova/mysql.sql"], Class['generic::mysql::packages::server']],
		...
		'create_nova_db_user':
			...
			require => [File["/etc/nova/nova-user.sql", "/etc/nova/nova-user.cnf", "/root/.my.cnf"], Class['generic::mysql::packages::server']];
		'create_nova_db':
			unless => "/usr/bin/mysql -uroot $nova_db_name -e 'exit'",
			command => "/usr/bin/mysql -uroot -e \"create database $nova_db_name;\"",
			require => [File["/root/.my.cnf"], Class['generic::mysql::packages::server']],
		...
		'create_puppet_db_user':
			...
			require => [File["/etc/puppet/puppet-user.sql", "/etc/puppet/puppet-user.cnf", "/root/.my.cnf"], Class['generic::mysql::packages::server']];
		'create_puppet_db':
		...
		'create_glance_db_user':
			...
			require => [File["/etc/glance/glance-user.sql","/etc/glance/glance-user.cnf","/root/.my.cnf"], Class['generic::mysql::packages::server']];
		'create_glance_db':
