...
# 1) create the new account entry in accounts::all and add to the
#    bottom of the list (ordering by uid)
# 2) make sure base::accounts is included on your node or in your role
# 3) if there is an existing admins::xxx list (roots, mortals etc.)
#    for you to use, add an entry there; if not, create a new one and add it
# 4) declare the admins::xxx list class in the role or on the node
#    you need
#
# To add an account to an admin::xxx list (roots, mortals, etc):
# 1) add an entry for it in the list; Puppet will do the rest.
#
# To add a new admin::xxx list (roots, mortals, etc) to a node or role:
# 1) create the list if needed, following the format of the existing lists
# 2) include it on the node or role
# 3) make sure base::accounts is included on the node or role too
#
# To completely disable an account on all hosts:
# 1) add its entry to accounts::disabled - this removes it from all
#    cluster hosts
# 2) remove it from all admins::xxx lists (root, mortals etc.) just
#    to be nice
# 3) move all ssh key entries to the $sshkeys_disabled parameter for
#    that account, or if there is none, simply change $sshkeys_enabled
#    to $sshkeys_disabled so that all ssh keys will be gone if this
#    account is someday re-enabled
# 4) if they were a root, remove their key from
#    /root/private/files/ssh/root-authorized-keys on sockpuppet,
#    git commit and pull to /var/lib/git/operations/private/ on stafford.
#
# To disable an account on all hosts but keep their home directory:
# 1) do 1 through 4 above
# 2) set manage_home => false in the parameters for $unixaccount.
#    This will still remove all ssh keys.
#
# To remove an account from all hosts with a given admins::xxx list:
# 1) remove the account from the list
# 2) if this is the admins::roots list, remove their key from
#    /root/private/files/ssh/root-authorized-keys on sockpuppet,
#    git commit and pull to /var/lib/git/operations/private/ on stafford.
#
# To remove an admin::xxx list (root, mortals etc.) from a node or role:
# 1) remove the admin list class declaration
# 2) make sure that base::accounts is still included on the node; puppet will
#    do the rest.
#
# To remove a host from puppet altogether:
# 1) remove all admin::xxx lists from it and make sure that base::accounts is
#    still included on the node
# 2) verify that the puppet run completed and that no user accounts are left
#    on the host
# 3) proceed with regular decommissioning/reassignment for reinstallation/etc.

define accounts::unixaccount (
	$realname,
	$username,
	$uid=$title,
	$gid=500,
	...
	require accounts::disabled

	$disabled = getvar("accounts::disabled::${username}")
	if $disabled {
		# globally disabled across the cluster
		$ensure = absent
	}
	elsif defined(Accounts::Wanted[$username]) {
		# enabled on this specific host
		$ensure = present
	}
	else {
		# not specifically enabled on this host
		$ensure = absent
	}
	...
	if $ensure == absent {
		if $manage_home == true {
			# remove home dir manually til we have puppet >= 2.7.19, see
			...
			# we're not removing home dir so let's at least toss
			# the authorized_keys_file
			file {
...
define accounts::ssh_key (
	...
	# if the user doesn't exist, we deal with ssh keys in the unixaccount type
	$disabled = getvar("accounts::disabled::${user}")
	if !$disabled and defined(Accounts::Wanted[$user]) {
		ssh_authorized_key {
			$keyname:
				ensure => $ensure,
				user   => $user,
...
define accounts::account (
        $unixaccount,
        $user = $title,
        $sshkeys_enabled = undef,
        $sshkeys_disabled = undef,
        $groupname = undef
        ) {
        if $groupname {
                include "groups::${groupname}"
	}
        create_resources(accounts::unixaccount, $unixaccount, { username => $user })
        if $sshkeys_enabled {
	        create_resources(accounts::ssh_key, $sshkeys_enabled, { ensure => present, user => $user } )
	}
        if $sshkeys_disabled {
	        create_resources(accounts::ssh_key, $sshkeys_disabled, { ensure => absent, user => $user } )
	}
}

define accounts::wanted (
	$user=$title
	) {
	# does nothing but exist :-)
}

# disabled across the cluster
class accounts::disabled {
	$ashields = true
	$austin = true
	$bastique = true
	$ben = true
	...
	$hcatlin = true
	$jdavis = true
	$neilk = true       # revoked access per RT 2345
	$nimishg = true
	$pdhanda = true
	$preilly = true
	$raindrift = true   # access revoked per RT 3088
	$ram = true
	$rcole = true
	$samreed = true
	$sara = true
...
class accounts::all {
	...
	##### pseudo-system accounts (should be moved later)
	accounts::account { 'file_mover':
		groupname       => 'file_mover',
		unixaccount     => {
			'30001' => {
				realname => 'file_mover'
			}
	...
	##### accounts with weird uids, who knows why
	accounts::account { 'jdavis':
		unixaccount      => {
			'1004' => {
				realname => 'Jon Davis'
			}
	...
	##### keep in uid order: add new accounts at end
	accounts::account { 'brion':
		unixaccount     => {
			'500' => {
	...
	accounts::account { 'tparscal':
		...
		sshkeys_disabled => {
			'tparscal@Trevor-Parscals-MacBook-Air.local' => {
	...
	accounts::account { 'jpostlethwaite':
		unixaccount     => {
			'577' => {
		...
		sshkeys_enabled => {
			'jpostlethwaite@WMF299s-MacBook-Pro.local' => {
	...
	accounts::account { 'lcarr':
		...
		sshkeys_enabled => {
			'lcarr@Administrators-MacBook-Air.local' => {
	...
	accounts::account { 'dandreescu':
		unixaccount     => {
			'610' => {
				realname    => 'Dan Andreescu (disabled)',
				manage_home => false # want to keep home dir (why?)
			}
		},
		sshkeys_enabled => {
	...
	accounts::account { 'mholmquist':
		...
		sshkeys_enabled => {
			'administrator@WMF-ThinkPad-T420s' => {
	...
	accounts::account { 'tnegrin':
		...
		sshkeys_enabled => {
			'tnegrin@Administrators-MacBook-Air.local' => {
