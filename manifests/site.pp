...
node /^cp30(5[02468]|6[02])\.esams\.wmnet$/ {
    role(cache::text)
}
...
node /^cp40(2[789]|3[0156])\.ulsfo\.wmnet$/ {
    role(cache::text)
}
...
node /^cp50(0[789]|1[0156])\.eqsin\.wmnet$/ {
    role(cache::text)
}
...
node 'ganeti1028.eqiad.wmnet' {
    role(insetup)
}

# Ganeti virtualization hosts - eqiad
node /^ganeti10(0[5-9]|1[0-9]|2[0-7])\.eqiad\.wmnet$/ {
    role(ganeti)
}
