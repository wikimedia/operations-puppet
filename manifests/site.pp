...
node /^aqs10(1[0-2]|1[4-9]|2[0-3])\.eqiad\./ {
    role(aqs)
}

# Hardware refreshes: T407032
node /^aqs102[4-7]\.eqiad\./ {
...
node /^wikikube-worker1(2[4-9][0-9]|3[0-1][0-9]|32[0-7]|33[5-9]|3[4-5][0-9])\.eqiad\./ {
    role(kubernetes::worker)
}
...
# Old eqiad backup worker hosts, to be decom. T420464
node /^ms-backup100[12]\.eqiad\./ {
    role(mediabackup::worker)
}

# New eqiad backup worker hosts
