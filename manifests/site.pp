...
node /^cp20(07|1[02369]|2[379]|3[13])\.codfw\.wmnet$/ {
    role(cache::text)
}

node /^cp20(08|1[1478]|2[024568]|3[02])\.codfw\.wmnet$/ {
    role(cache::upload)
}
...
node /^cp20(3[4-9]|4[0-2])\.codfw\.wmnet$/ {
    role(insetup_noferm)
}
...
# Temporarily running Ceph based VMs
