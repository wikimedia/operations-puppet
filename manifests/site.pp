...
node /^cp30(3[4579]|4[345679])\.esams\.wmnet$/ {
    interface::add_ip6_mapped { 'main': }
    role(cache::upload)
}

node /^cp303[68]\.esams\.wmnet$/ {
    interface::add_ip6_mapped { 'main': }
...
node /^db1(081|084|091|121|138)\.eqiad\.wmnet/ {
    role(mariadb::core)
}
...
node /^db2(046|053|060|067|076|114|117)\.codfw\.wmnet/ {
    role(mariadb::core)
}
...
node /^db2(040|054|061|068|077|118|120)\.codfw\.wmnet/ {
    role(mariadb::core)
}
...
node /^db11(28|32|33|35|36)\.eqiad\.wmnet/ {
    role(spare::system)
}
