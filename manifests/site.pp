...
# new an-test-masters T255518
node /^an-test-master100[1-2]\.eqiad\.wmnet$/ {
    role(insetup)
}
...
node /^an-worker110[0-9]\.eqiad\.wmnet$/ {
    role(insetup)
}

node /^an-worker111[0-7]\.eqiad\.wmnet$/ {
    role(insetup)
}
...
node /^db1(084|089|091|106|118|119|134|135)\.eqiad\.wmnet/ {
    role(mariadb::core)
}
...
# eqiad replicas
# See also db1090, db1105, db1146 below
...
node /^db1(121|138|141|142|143|147|148|149)\.eqiad\.wmnet/ {
    role(mariadb::core)
}
...
node /^db1(069|079|094|127|136)\.eqiad\.wmnet/ {
    role(mariadb::core)
}
...
node /^db1(092|087|109|111|114|126)\.eqiad\.wmnet/ {
    role(mariadb::core)
}
...
## es2031 will replace es2015 T261717
...
node /^cloudvirt101[7-9]\.eqiad\.wmnet$/ {
...
node /^cloudvirt101[2-6]\.eqiad\.wmnet$/ {
    role(wmcs::openstack::eqiad1::virt_ceph)
}
