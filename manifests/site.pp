...
node /^db11(85|91|92|93|94).eqiad.wmnet$/ {
    role(insetup)
}
...
node /^db2(103|112|116|130|145|146|153|174|176|191)\.codfw\.wmnet/ {
    role(mariadb::core)
}
...
# ceph storage nodes
node /^cloudcephosd10(0[1-9]|1[0-9]|2[0-7])\.eqiad\.wmnet$/ {
    role(wmcs::ceph::osd)
}

# ceph storage nodes
node /^cloudcephosd10(2[8-9]|3[0-4])\.eqiad\.wmnet$/ {
    role(insetup)
}
