...
node /^cp10(7[579]|8[13])\.eqiad\.wmnet$/ {
    role(cache::text)
}

# HAproxy test T290005
node /^cp108[579]\.eqiad\.wmnet$/ {
    role(cache::text_haproxy)
}

node /^cp10(7[68]|8[02])\.eqiad\.wmnet$/ {
    role(cache::upload)
}

# HAProxy test T290005
node /^cp10(8[468]|90)\.eqiad\.wmnet$/ {
    role(cache::upload_haproxy)
}

node /^cp20(2[79]|3[13])\.codfw\.wmnet$/ {
    role(cache::text)
}

# HAProxy test T290005
node /^cp20(3[579]|41)\.codfw\.wmnet$/ {
    role(cache::text_haproxy)
}

node /^cp20(28|3[024])\.codfw\.wmnet$/ {
    role(cache::upload)
}

# HAProxy test T290005
node /^cp20(3[68]|4[02])\.codfw\.wmnet$/ {
    role(cache::upload_haproxy)
}
...
node /^cp305[0246]\.esams\.wmnet$/ {
    role(cache::text)
}

# HAProxy test - T290005
node /^cp30(58|6[024])\.esams\.wmnet$/ {
    role(cache::text_haproxy)
}

node /^cp305[1357]\.esams\.wmnet$/ {
    role(cache::upload)
}

# HAProxy test T290005
node /^cp30(59|6[135])\.esams\.wmnet$/ {
    role(cache::upload_haproxy)
}
...
node /^cp40(2[1-3]|33)\.ulsfo\.wmnet$/ {
    role(cache::upload)
}

# HAProxy test
node /^cp40(2[4-6]|34)\.ulsfo\.wmnet$/ {
    role(cache::upload_haproxy)
}

node /^cp40(2[789]|35)\.ulsfo\.wmnet$/ {
    role(cache::text)
}

# HAProxy test - T290005
node /^cp403[026].ulsfo.wmnet$/ {
    role(cache::text_haproxy)
}
...
# codfw master
node 'db2104.codfw.wmnet' {
    role(mariadb::core)
}

# codfw replicas
# See also db2088 and db2138 below
node /^db2(107|125|148)\.codfw\.wmnet/ {
    role(mariadb::core)
}
...
node /^db1(104|111|114|126|172|177)\.eqiad\.wmnet/ {
    role(mariadb::core)
}
...
# m5 future eqiad master T301654
...
# ceph storage nodes
node /^cloudcephosd10(0[1-9]|1[0-9]|2[0-4])\.eqiad\.wmnet$/ {
    role(wmcs::ceph::osd)
}
...
node /^ping[123]00[12]\.(eqiad|codfw|esams)\.wmnet$/ {
    role(ping_offload)
}
...
node /^restbase20(09|1[0-9]|2[0-6])\.codfw\.wmnet$/ {
    role(restbase::production)
}
...
# new cloudvirt server T293391
node 'cloudvirt1047.eqiad.wmnet' {
    role(insetup)
}
