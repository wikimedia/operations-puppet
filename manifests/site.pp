...
# to make sure the hostname -> /datacenter/rack/row id is correct.
# This is used for Hadoop network topology awareness.
node /^an-worker1(10[0-9]|11[0-9]|12[0-9]|13[0-9]|14[0-9]|15[0-9]|16[0-9]|17[0-6]|18[7-9]|19[0-9]|20[0-8])\.eqiad\./ {
    role(analytics_cluster::hadoop::worker)
}

# New an-worker nodes - See #T377878 and #T386390
node /^an-worker1(17[7-9]|18[0-6])\.eqiad\./ {
    role(insetup::data_platform_ferm)
}
...
## x1 shard
# eqiad
node /^db1(179|183|220|224|237)\.eqiad\./ {
    role(mariadb::core)
}
...
node  /^elastic20(57|58|63|64|70|77|78|79|80|92|93|94|95|96|97)\.codfw\./ {
    role(elasticsearch::cirrus)
...
node /^kafka-jumbo10(0[7-9]|1[0-6])\.eqiad\./ {
    role(kafka::jumbo::broker)
}

# New kafka-jumbo hosts - See #T377874
node /^kafka-jumbo101[7-8]\.eqiad\./ {
    role(insetup::data_platform_ferm)
}
...
node /^wikikube-worker10(0[2-7]|1[12569]|[2-9][0-9])\.eqiad\./ {
...
node /^puppetserver[12]00[1234]\.(codfw|eqiad)\./ {
    role(puppetserver)
}
...
node /^cloudvirt10(3[1-9]|4[0-9]|5[0-9]|6[0-9]|7[0-6])\.eqiad\./ {
