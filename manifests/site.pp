...
# Envoy test T271421
node 'cp2040.codfw.wmnet' {
    role(cache::upload_envoy)
}

# HAProxy test T290005
node 'cp2042.codfw.wmnet' {
    role(cache::upload_haproxy)
}
...
# Envoy test - T271421
node 'cp3062.esams.wmnet' {
    role(cache::text_envoy)
}

# HAProxy test - T290005
node 'cp3064.esams.wmnet' {
    role(cache::text_haproxy)
}
...
node /^ml-serve200[12345]\.codfw\.wmnet/ {
    role(ml_k8s::worker)
}

# New ml-serve nodes T294945
node /^ml-serve200[678]\.codfw\.wmnet/ {
    role(insetup)
}
