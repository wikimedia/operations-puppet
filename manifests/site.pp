...
node /^db1(208|212|213|216|219)\.eqiad\.wmnet$/ {
...
node /^db1(106|118|119|128|132|134|135|163|169|184|186|206|207|218)\.eqiad\.wmnet$/ {
    role(mariadb::core)
}
...
node /^db1(122|129|162|182|188|197|222)\.eqiad\.wmnet$/ {
    role(mariadb::core)
}
...
node /^db1(123|157|166|175|189|198|223)\.eqiad\.wmnet$/ {
    role(mariadb::core)
}
...
node /^db1(100|110|130|183|185|200|210)\.eqiad\.wmnet$/ {
    role(mariadb::core)
}
...
## m1 shard
# See also multiinstance misc hosts db1117, db1217, db2160 below
...
node /^dumpsdata100[367]\.eqiad\.wmnet$/ {
    role(dumps::generation::server::spare)
}
...
node /^kafka-logging100[234]\.eqiad\.wmnet$/ {
    role(kafka::logging)
}
...
node /^prometheus[456]002\.(ulsfo|eqsin|drmrs)\.wmnet$/ {
  role(insetup::observability)
}
