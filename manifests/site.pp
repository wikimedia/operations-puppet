...
node /^cp305[02]\.esams\.wmnet$/ {
    role(cache::text)
}

# HAProxy test - T290005
node /^cp30(5[468]|6[024])\.esams\.wmnet$/ {
    role(cache::text_haproxy)
}

node /^cp305[135]\.esams\.wmnet$/ {
    role(cache::upload)
}

# HAProxy test T290005
node /^cp30(5[79]|6[135])\.esams\.wmnet$/ {
    role(cache::upload_haproxy)
}
...
node /^cp40(2[12]|33)\.ulsfo\.wmnet$/ {
    role(cache::upload)
}

# HAProxy test
node /^cp40(2[3-6]|34)\.ulsfo\.wmnet$/ {
    role(cache::upload_haproxy)
}

node /^cp40(2[78]|35)\.ulsfo\.wmnet$/ {
    role(cache::text)
}

# HAProxy test - T290005
node /^cp40(2[9]|3[026])\.ulsfo.wmnet$/ {
    role(cache::text_haproxy)
}
...
node /^cp50(0[78]|15)\.eqsin\.wmnet$/ {
    role(cache::text)
}

# HAProxy test - T290005
node /^cp50(0[9]|1[0126])\.eqsin\.wmnet$/ {
    role(cache::text_haproxy)
}
...
# cp drmrs nodes
#
node /^cp600[1-7]\.drmrs\.wmnet$/ {
    role(cache::upload)
}

# HAProxy test T290005
node /^cp600[8]\.drmrs\.wmnet$/ {
    role(cache::upload_haproxy)
}
