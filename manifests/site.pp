...
node /^cp20(4[13579]|5[1357])\.codfw\./ {
    role(cache::text)
}

node /^cp20(4[2468]|5[02468])\.codfw\./ {
    role(cache::upload)
}
...
node /^db12(65|66|67|68|69|70|71|72|73|74|75|76|77|78|79|80|81|82|83|84|85|86|87|88|89|90)\.eqiad\./ {
    role(insetup::data_persistence_ferm)
}

# Refresh hosts T418911
node /^db22(50|51|52|53)\.codfw\./ {
    role(insetup::data_persistence_ferm)
}
...
node /^dse-k8s-worker10(0[1-9]|1[01346789]|2[4-8])\.eqiad\./ {
    role(dse_k8s::worker)
}

# Temporarily set two dse-k8s-worker nodes to the insetup role.
node /^dse-k8s-worker101[2,5]\.eqiad\./ {
    role(insetup::data_platform_ferm)
}
...
node /^wikikube-worker1(2[4-9][0-9]|3[0-1][0-9]|32[0-7]|35[0-9])\.eqiad\./ {
    role(kubernetes::worker)
}
...
node /^wikikube-worker13(3[5-9]|4[0-9])\.eqiad\./ {
    role(insetup::serviceops_ferm)
}
