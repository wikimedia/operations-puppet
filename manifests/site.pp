...
node /^kubernetes20(0[5-68-9]|1[0-7]|2[0-478]|[34][0-9]|5[0-9]|60)\.codfw\./ {
    role(kubernetes::worker)
}
...
node /^wikikube-worker20(0[1-47-9]|[1-4][0-9]|5[01])\.codfw\./ {
    role(kubernetes::worker)
}
...
node /^mw238[1-9]\.codfw\./ {
  role(kubernetes::worker)
}
node /^mw239[04-9]\.codfw\./ {
  role(kubernetes::worker)
}
node /^mw240[267]\.codfw\./ {
  role(kubernetes::worker)
}
