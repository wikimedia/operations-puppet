...
node /^an-presto100[1-6]\.eqiad\.wmnet$/ {
    role(analytics_cluster::presto::server)
}

# New an-presto nodes in eqiad T306835
node /^an-presto10(0[7-9]|1[0-5])\.eqiad\.wmnet/ {
    role(insetup::data_engineering)
}
...
# New contint node T313830
node /^(contint1002)\.wikimedia\.org$/ {
...
node /^cp50(1[34]|2[56789]|3[012])\.eqsin\.wmnet$/ {
    role(cache::upload)
}

node /^cp50(1[1256789]|2[01234])\.eqsin\.wmnet$/ {
    role(cache::text)
}
...
node /^ganeti400[568]\.ulsfo\.wmnet$/ {
    role(ganeti)
}

node /^ganeti500[1234]\.eqsin\.wmnet$/ {
    role(ganeti)
}
...
# new cloudvirt servers T313983
node /^cloudvirt10(5[4-9]|6[0-1])\.eqiad\.wmnet/ {
    role(insetup::wmcs)
}
