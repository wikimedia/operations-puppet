...
class base::puppet($server="puppet", $certname=undef) {
	...
	case $::realm {
		...
		'labs': {
			exec { "puppet snmp trap":
				command => "snmptrap -v 1 -c public nagios-main.pmtpa.wmflabs .1.3.6.1.4.1.33298 \$(cat /etc/wmflabs-instancename).$::site.wmflabs 6 1004 `uptime | awk '{ split(\$3,a,\":\"); print (a[1]*60+a[2])*60 }'`",
				path => "/bin:/usr/bin",
				require => Package["snmp"]
			}
		}
