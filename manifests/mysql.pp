...
class mysql {
	...
	#######################################################################
	if $hostname =~ /^db(38|52|31|35|39|43|37|48)$/ {
		$master = "true"
		$writable = "true"
	} else {
		$master = "false"
	...
	#######################################################################
	if $hostname =~ /^db(12|32|36|38|42|59|60|1001|1017|1033|1043|1047)$/ {
		$db_cluster = "s1"
	}
	elsif $hostname =~ /^db(13|30|52|53|54|57|1002|1018|1034)$/ {
		$db_cluster = "s2"
	}
...
#	}
	...
	class slow_digest {
		...
		$mysql_pw = $passwords::mysql::querydigest::mysql_ops_db
		$mysql_user = "ops"
