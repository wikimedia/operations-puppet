...
class mysql {
	...
	#######################################################################
	if $hostname =~ /^db(38|52|39|31|35|43|37|48)$/ {
		$master = "true"
		$writable = "true"
	} else {
		$master = "false"
	...
	#######################################################################
	if $hostname =~ /^db(12|32|36|38|42|59|60|61|62|1001|1017|1033|1042|1043|1047)$/ {
		$db_cluster = "s1"
	}
	...
	)
{
	...
	# run directory has moved from /var/run
	# to /run in Ubuntu Precise.
