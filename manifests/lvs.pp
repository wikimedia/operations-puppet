...
define monitor_service_lvs_http ( $ip_address, $check_command, $critical="true", $contact_group="admins" ) {
	# Virtual resource for the monitoring host
	@monitor_host { $title: ip_address => $ip_address, group => "lvs", critical => $critical, contact_group => $contact_group }
...
class lvs::monitor {
	...
	monitor_service_lvs_http { "parsoid.svc.pmtpa.wmnet": ip_address => "10.2.1.28", check_command => "check_http_on_port!8000", critical => false, contact_group => "admins,sms,parsoid" }
	monitor_service_lvs_http { "parsoid.svc.eqiad.wmnet": ip_address => "10.2.2.28", check_command => "check_http_on_port!8000", critical => false, contact_group => "admins,sms,parsoid" }
	monitor_service_lvs_http { "parsoidcache.svc.pmtpa.wmnet": ip_address => "10.2.1.29", check_command => "check_http_on_port!6081", critical => false, contact_group => "admins,sms,parsoid" }
