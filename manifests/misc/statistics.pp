...
class misc::statistics::gerrit_stats {
	...
	cron { "gerrit-stats-daily":
		command => "/usr/bin/python $gerrit_stats_path/gerritstats/stats.py --dataset $gerrit_stats_data_path --toolkit dygraphs --settings /a/gerrit-stats/gerrit-stats/gerritstats/settings.yaml | tee -a $gerrit_stats_base/gerrit-stats.log && (cd $gerrit_stats_data_path && git add . && git commit -m \"Updating gerrit-stats data after gerrit-stats run at $(date)\" && git push)",
