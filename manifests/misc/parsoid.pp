class misc::parsoid {
	...
	git::clone {
		"parsoid":
			directory => "/var/lib/parsoid/VisualEditor",
			owner => parsoid,
			group => parsoid,
			origin => "https://gerrit.wikimedia.org/r/p/mediawiki/extensions/VisualEditor.git",
	...
	exec {
		"parsoid-npm-install":
			...
			cwd => "/var/lib/parsoid/VisualEditor/modules/parser",
			user => "parsoid",
			# Needed so we don't try to write /root/.npm and fail
			environment => "HOME=/var/lib/parsoid",
			creates => "/var/lib/parsoid/VisualEditor/modules/parser/node_modules",
