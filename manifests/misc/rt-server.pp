...
class misc::rt::server ( $site = 'rt.wikmedia.org', $datadir = '/var/lib/mysql' ) {
  ...
  exec { 'rt-db-initialize':
    command => "/bin/echo '' | rt-setup-database --action init --dba root --prompt-for-dba-password",
  ...
  exec { 'update-rt-siteconfig':
    ...
    path        => '/usr/sbin',
    require => [ exec[ "rt-db-initialize" ],
                 file[ "/etc/request-tracker3.8/RT_SiteConfig.d/50-debconf",
                       "/etc/request-tracker3.8/RT_SiteConfig.d/80-wikimedia" ] ],
    refreshonly => true,
