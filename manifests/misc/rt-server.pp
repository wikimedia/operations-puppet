...
class misc::rt::server ( $site = 'rt.wikmedia.org', $datadir = '/var/lib/mysql' ) {
  ...
  # If we're a new install, set up the RT database.  The unless
  # clause prevents this from running a second time on labs ('rtdb')
  # or in production (where we have a db hosted elsewhere named 'rt')
  exec { 'rt-db-initialize':
    ...
    require => [ package[ 'request-tracker3.8', 'rt3.8-db-mysql', 'rt3.8-clients', 'libcgi-fast-perl', 'lighttpd',
      'libdbd-pg-perl' ] ],
    unless  => ['/usr/bin/mysqlshow rt', '/usr/bin/mysqlshow rtdb'];
  }
