...
class icinga::monitor::firewall {
    ...
    $allowed_networks= [$PMTPA_PRIVATE_PRIVATE_IPV4,
                        $ESAMS_PRIVATE_PRIVATE1_ESAMS_IPV4,
                        $EQIAD_PRIVATE_PRIVATE1_A_EQIAD_IPV4,
                        $EQIAD_PRIVATE_PRIVATE1_B_EQIAD_IPV4,
                        $ULSFO_PRIVATE_PRIVATE1_ULSFO_IPV4,
                        $PMTPA_PRIVATE_VIRT_HOSTS_IPV4,
                        $PMTPA_PUBLIC_PUBLIC_SERVICES_IPV4,
                        $PMTPA_PUBLIC_PUBLIC_SERVICES_2_IPV4,
                        $ESAMS_PUBLIC_PUBLIC_SERVICES_IPV4,
                        $ULSFO_PUBLIC_PUBLIC1_ULSFO_IPV4,
                      ]
    #ncsa on port 5667
    ferm::rule { 'ncsa_allowed':
        rule => 'saddr (127.0.0.1 $allowed_networks) proto tcp dport 5667 ACCEPT';
    }

    #snmptrap on port 162
    ferm::rule { 'snmptrap_allowed':
        rule => 'saddr (127.0.0.1 $allowed_networks) proto tcp dport 162 ACCEPT';
    }

    #snmp on port 161
    ferm::rule { 'snmp_allowed':
        rule => 'saddr (127.0.0.1 $allowed_networks) proto tcp dport 161 ACCEPT';
    }
}
