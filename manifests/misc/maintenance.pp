...
class misc::maintenance::update_flaggedrev_stats( $ensure = present ) {
    ...
    cron { 'update_flaggedrev_stats':
        ...
        command => 'foreachwikiindblist flaggedrevs.dblist extensions/FlaggedRevs/maintenance/updateStats.php > /dev/null',
        user    => $::mediawiki::users::web,
        hour    => '*/2',
        minute  => '0',
    }
...
class misc::maintenance::update_special_pages( $ensure = present ) {
    cron { 'update_special_pages':
        ...
        command  => 'flock -n /var/lock/update-special-pages foreachwiki updateSpecialPages.php > /var/log/mediawiki/updateSpecialPages.log 2>&1',
        user     => $::mediawiki::users::web,
        monthday => '*/3',
        hour     => 5,
        minute   => 0,
