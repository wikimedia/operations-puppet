...
class misc::maintenance::wikidata( $ensure = present ) {
    ...
    cron { 'wikibase-dispatch-changes4':
        ...
        command => '/usr/local/bin/mwscript extensions/Wikidata/extensions/Wikibase/repo/maintenance/dispatchChanges.php --wiki wikidatawiki --max-time 1600 --batch-size 275 --dispatch-interval 25 2>&1 >> /dev/null',
        user    => $::mediawiki::users::web,
        minute  => '*/3',
    }
