...
class gerrit::crons {
	...
	cron { list_reviewer_counts:
		...
		command => "ssh -p 29418 localhost gerrit gsql --format JSON -c \"'SELECT change_id.s AS change_id, COUNT(DISTINCT patch_set_approvals.account_id) AS reviewer_count FROM change_id LEFT JOIN patch_set_approvals ON (change_id.s = patch_set_approvals.change_id) GROUP BY change_id.s'\" > /var/www/reviewer-counts.json",
		user => gerrit2,
		hour => 1
	}
