...
class role::cxserver::common {
    package { [
        'nodejs', # To run cxserver
        'npm', # NL: Do we need this for anything?
        'build-essential', # NL: Or this?
        ]: ensure => present,
    }
}
...
class role::cxserver::beta {
    ...
    sudo_user { 'jenkins-deploy': privileges => [
        ...
        # NL: How to tell Jenkins to use this command?
        'ALL = (root)  NOPASSWD:/usr/sbin/service cxserver restart',
    ] }

    # NL: What ensures /srv/deployment/cxserver/deploy is present on labs?
    ...
    $cxserver_port = '8080'
    $cxserver_parsoid_url = 'http://parsoid.wmflabs.org'
    ...
    # NL: I do not understand what this is for
