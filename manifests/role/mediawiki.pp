...
class role::mediawiki::common {
    ...
    # Redis only listens on localhost, but exempt it from connection tracking
    ferm::rule { 'skip_redis_conntrack_out':
        desc  => 'Skip outgoing connection tracking for Redis',
        table => 'raw',
        chain => 'OUTPUT',
        rule  => 'proto tcp sport 6380 NOTRACK;',
    }

    ferm::rule { 'skip_redis_conntrack_in':
        desc  => 'Skip incoming connection tracking for Redis',
        table => 'raw',
        chain => 'PREROUTING',
        rule  => 'proto tcp dport 6380 NOTRACK;',
    }
