...
class role::analytics::refinery::data::drop {
    ...
    cron {'refinery-drop-eventlogging-partitions':
        command => "export PYTHONPATH=\${PYTHONPATH}:${role::analytics::refinery::path}/python && ${role::analytics::refinery::path}/bin/refinery-drop-eventlogging-partitions -d ${raw_retention_days} -l /wmf/data/raw/eventlogging >> ${log_file} 2>&1",
        user    => 'hdfs',
        minute  => '15',
        hour    => '*/4',
    }
