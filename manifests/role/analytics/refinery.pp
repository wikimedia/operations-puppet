...
class role::analytics::refinery::data::check {
    # Let's be lenient, and give Camus and Oozie 3 hours to
    # successfully import a dataset.
    ...
    monitor_service { 'hive_partition_webrequest-mobile':
        description     => 'Raw webrequest mobile data imported into HDFS and Hive.',
    ...
    monitor_service { 'hive_partition_webrequest-text':
        description     => 'Raw webrequest text data imported into HDFS and Hive.',
        check_command   => "analytics_cluster_data_import-FAIL!wmf_raw.webrequest text!${$alert_return_code}",
        passive         => 'true',
        freshness       => $freshness_threshold,
        retries         => 1,
    }
    monitor_service { 'hives_partition_webrequest-bits':
        description     => 'Raw webrequest bits data imported into HDFS and Hive.',
    ...
    monitor_service { 'hive_partition_webrequest-upload':
        description     => 'Raw webrequest upload data imported into HDFS and Hive.',
