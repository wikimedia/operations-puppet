    ...
    # crontab should run once a day at 23 hours
     if !$debug {
        logster::job { 'wikimetrics-apache-requests':
             ...
             logster_options => "-o statsd --statsd-host=labmon1001.eqiad.wmnet:8125 --metric-prefix='analytics.wikimetrics.ui.request' --parser-options '--regex=.*(report|cohort|metrics|login|a
bout|contact).*'"
        }
     }
}
