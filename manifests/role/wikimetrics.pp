...
class role::wikimetrics {
    ...
    logster::job { 'wikimetrics-apache-visits':
        minute          => '*/1',
        parser          => 'LineCountLogster --regex=\"report|cohort|metrics|login|about|contact\" ',
        logfile 		=> '/var/log/apache2/access.wikimetrics.log',
        logster_options => "-o statsd --statsd-host=labmon1001.equiad.wmnet:8125 --metric-prefix='wikimetrics'",
    }
}
