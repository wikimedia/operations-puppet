	...
	class upload {
		if $::site == "eqiad" {
			...
			varnish::instance { "upload-backend":
				...
				port => 3128,
				admin_port => 6083,
				storage => "-s main-sda3=file,/srv/sda3/varnish.persist,100G " + \
				           "-s main-sdb3=file,/srv/sdb3/varnish.persist,100G " + \
				           "-s bigobj-sda3=file,/srv/sda3/large-objects.persist,10G " + \
				           "-s bigobj-sdb3=file,/srv/sdb3/large-objects.persist,10G",
