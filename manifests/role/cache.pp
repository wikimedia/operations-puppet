...
class role::cache {
    ...
    # Beta cluster only has two Apaches which are handling any kind of
    # requests. Avoid code duplication by definiting the hash there and reuse
    # it in the configuration hash below.
    $beta_apaches = [
      ...
      'labs' => {
        'api'    => { 'pmtpa' => $beta_apaches, },
        'bits'   => { 'pmtpa' => $beta_apaches, },
        'mobile' => { 'pmtpa' => $beta_apaches, },
        'text'   => { 'pmtpa' => $beta_apaches, },
        'upload' => { 'pmtpa' => $beta_apaches, },
      },
		}
