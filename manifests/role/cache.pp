	...
	class varnish::kafka($topic)
	{
		if $::realm == 'production' {
			...
			class { '::varnishkafka':
				...
				format_key               => "%{fake_tag1?${::fqdn}}x",
				# large timeouts to account for potential cross DC latencies
				topic_message_timeout_ms => 300000, # local timeout
				topic_request_timeout_ms => 180000, # request ack timeout
