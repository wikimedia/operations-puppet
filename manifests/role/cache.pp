...
class role::cache {
	...
	class mobile {
		...
		if( $::realm == 'production' ) {
			...
			$varnish_be_backends = $lvs::configuration::lvs_service_ips[$::realm]['apaches'][$::mw_primary]
			$api_backend = $lvs::configuration::lvs_service_ips[$::realm]['api'][$::mw_primary]
		} else {
			# beta on labs, only one varnish instance having both a
			# frontend and a backend
			$varnish_fe_backends = '127.0.0.1'
			$varnish_fe_directors = {
				'pmtpa' => { 'backend' => '127.0.0.1' },
				'eqiad' => {},
				'esams' => {},
			}
			$varnish_be_backends = $role::cache::configuration::beta_varnish_backends
			$api_backend = '10.4.0.166'  # FIXME: should be first in $varnish_be_backends
		}
		...
		if( $::realm == 'production' ) {
			...
			# Umount /dev/vdb from /mnt (default mount point on labs)
			mount { 'unmount /dev/vdb from /mnt':
				device => '/dev/vdb',
				name => '/mnt',
				ensure => absent,
			}
			...
			varnish::setup_filesystem{ ["vdb"]:
				before => Varnish::Instance["mobile-backend"],
				# Better have /dev/vdb unmounted
				require => Mount['unmount /dev/vdb from /mnt']
			}
		}
		...
		if( $::realm == 'production' ) {
			...
			# Wikimedia varnishncsa has a custom patch which override the -w
			# parameter so it must point to a host:port instead of a file.
			#
			# Just send everything to localhost, we will use tcpdump to read them.
			varnish::logging {'localhost': listener_address => '127.0.0.1', cli_args => '-m RxRequest:^(?!PURGE\$) -D' }
		}
	}
}
