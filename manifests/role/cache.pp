...
class role::cache {
	...
	# Configuration variables used for 'beta' cluster
	# aka deployment-prep project on labs.
	class configuration::beta {
		# Please prefix all variables with 'beta' to avoid conflict
		# with the variables in role::cache::configuration
	...
	class bits {
		...
		if( $::realm == 'production' ) {
			...
			# beta on labs
			$varnish_backends = $role::cache::configuration::beta::beta_varnish_backends
		}
	...
	class mobile {
		...
		if( $::realm == 'production' ) {
			...
			# beta on labs
			$varnish_fe_backends = $role::cache::configuration::beta::beta_varnish_backends
			$varnish_fe_directors = {
				'pmtpa' => {},
				'eqiad' => {},
				'esams' => {},
			...
			$varnish_be_backends = $role::cache::configuration::beta::beta_varnish_backends
		}
