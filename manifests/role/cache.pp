...
class role::cache {
	...
	class upload {
		if $::site == "eqiad" {
			...
			varnish::logging { "locke" : IP_address => "208.80.152.138" , cli_args => "-m RxRequest:^(?!PURGE\$) -D" }
			varnish::logging { "emery" : IP_address => "208.80.152.184" , cli_args => "-m RxRequest:^(?!PURGE\$) -D" }
			varnish::logging { "multicast_relay" : IP_address => "208.80.154.15" , port => "8419", cli_args => "-m RxRequest:^(?!PURGE\$) -D" }
