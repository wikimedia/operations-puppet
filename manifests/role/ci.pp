...
class role::ci::raita {
    ...
    exec { 'raita riot precompilation':
        command     => '/usr/bin/npm run build',
        cwd         => $dir,
        subscribe   => Git::Clone['integration/raita'],
        refreshonly => true,
    }

    exec { 'raita import elasticsearch mappings':
        command => "/usr/bin/node scripts/mappings.js import ${index_url}",
        unless  => "/usr/bin/node scripts/mappings.js check ${index_url}",
        cwd     => $dir,
