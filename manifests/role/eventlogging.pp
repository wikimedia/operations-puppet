...
class role::eventlogging {
    ...
    $kafka_default_topic_config = 'default.topic.config={"auto.offset.reset":-1}'
...
class role::eventlogging::forwarder inherits role::eventlogging {
    ...
    eventlogging::service::forwarder { 'legacy-zmq':
        input   => "${kafka_mixed_uri}&auto.commit.enable=False&${kafka_default_topic_config}&identity=legacy_zmq",
...
class role::eventlogging::consumer::mysql inherits role::eventlogging {
    ...
    $kafka_consumer_args  = hiera(
        ...
        "auto.commit.enable=True&auto.commit.interval.ms=1000&${kafka_default_topic_config}",
    )
...
class role::eventlogging::consumer::files inherits role::eventlogging {
    ...
    $kafka_consumer_args  = "auto.commit.enable=True&auto.commit.interval.ms=10000&${kafka_default_topic_config}"
    $kafka_consumer_group = hiera(
