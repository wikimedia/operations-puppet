...
class role::eventlogging {
    ...
    eventlogging::service::forwarder { 'server-side forwarder':
        ensure  => present,
        outputs => [
            'tcp://*:8421',
            'WIP - Kafka broker with the right topic goes here',
        ],
        count   => true,
    }
    ...
    eventlogging::service::processor { 'server-side events kafka':
        format        => '%{seqId}d EventLogging %j',
        input         => "tcp://${processor}:8421",
        output        => 'WIP - Kafka broker with the right topic goes here',
        outputInvalid => 'WIP - Kafka broker with the right topic goes here',
    }
    ...
    eventlogging::service::forwarder { 'client-side forwarder':
        ensure  => present,
        outputs => [
            'tcp://*:8422',
            'WIP - Kafka broker with the right topic goes here',
