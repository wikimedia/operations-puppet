...
class role::eventlogging::forwarder inherits role::eventlogging {
    ...
    # Temporarily a zmq forwarder consuming from kafka so that
    # reporter can keep working until we disable zmq altogether.
    eventlogging::service::forwarder { 'server-side-raw-zmq':
        input   => "${kafka_server_side_raw_uri}&auto_offset_commit=False&auto_offset_reset=-1"
