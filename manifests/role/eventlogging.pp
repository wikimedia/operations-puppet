...
class role::eventlogging::consumer::files inherits role::eventlogging {
    ...
    $kafka_consumer_server_side_uri = "${kafka_server_side_raw_uri}&zookeeper_connect=${kafka_zookeeper_url}&${kafka_consumer_args}",
    $kafka_consumer_client_side_uri = "${kafka_client_side_raw_uri}&zookeeper_connect=${kafka_zookeeper_url}&${kafka_consumer_args}",
    $kafka_consumer_mixed_uri       = "${kafka_mixed_uri}&zookeeper_connect=${kafka_zookeeper_url}&${kafka_consumer_args}",

    eventlogging::service::consumer {
        'server-side-events.log':
            input  => $kafka_consumer_server_side_uri,
            output => "file://${log_dir}/server-side-events.log",
            sid    => $kafka_consumer_group;
        'client-side-events.log':
            input  => $kafka_consumer_client_side_uri,
            output => "file://${log_dir}/client-side-events.log",
            sid    => $kafka_consumer_group;
        'all-events.log':
            input  => $kafka_consumer_mixed_uri,
