...
-prompt; rm /var/tmp/indexes.cmds";
		...
		'modify_default_global_aci':
			subscribe => Exec['start_opendj'],
			refreshonly => true,
			command => "/usr/bin/ldapmodify -x -D 'cn=Directory Manager' -H ldaps://${fqdn}:636 -w ${initial_password} -f /etc/ldap/global-aci.ldif",
...
class ldap::client::utils($ldapconfig) {
	...
	exec { "checkout_user_ldap_tools":
		command => "/usr/bin/svn co -r115488 http://svn.wikimedia.org/svnroot/mediawiki/trunk/tools/subversion/user-management",
		cwd => "/usr/local/lib",
