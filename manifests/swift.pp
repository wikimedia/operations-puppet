...
class swift::cleaner {
	...
	cron { "swiftcleaner-incremental":
		command => "$swiftcleaner_basedir/swiftcleanermanager -c ./swiftcleaner-incremental.conf",
		user => root,
		minute => 1,
	...
	cron { "swiftcleaner-full":
		...
		command => "pgrep swiftcleanermanager && rm /var/lib/swiftcleaner-full/*.prev; $swiftcleaner_basedir/swiftcleanermanager -c ./swiftcleaner-full.conf",
		user => root,
		minute => 0,
		hour => 22, #the beginning of the daily trough
		ensure => present
