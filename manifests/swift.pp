...
define swift::label_filesystem() {
	...
	exec { "/usr/sbin/xfs_admin -L $label $device":
		onlyif => "/usr/bin/test $(mount | /bin/grep $device | /usr/bin/wc -l) -eq 0 && /usr/bin/test $(/usr/sbin/grub-probe -t fs -d  $device) = 'xfs' && /usr/bin/test $(/usr/sbin/xfs_admin -l $device | /bin/grep swift | /usr/bin/wc -l) -eq 0"
	}
}
