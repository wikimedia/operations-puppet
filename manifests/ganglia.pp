...
class misc::monitoring::jobqueue {
	...
	cron {
		all_jobqueue_length:
			command => "/usr/bin/gmetric --name='Global JobQueue length' --type=int32 --conf=/etc/ganglia/gmond.conf --value=$(sudo -u apache php /usr/local/apache/common/multiversion/MWScript.php extensions/WikimediaMaintenance/getJobQueueLengths.php --totalonly | grep -oE '[0-9]+') > /dev/null 2>&1",
			user => mwdeploy,
			ensure => present;
	}
	...
	cron {
		all_jobqueue_length_spoofed:
			command => "/usr/bin/gmetric --name='Global JobQueue length' --type=int32 --conf=/etc/ganglia/gmond.conf --spoof 'www.wikimedia.org:www.wikimedia.org' --value=$(sudo -u apache php /usr/local/apache/common/multiversion/MWScript.php extensions/WikimediaMaintenance/getJobQueueLengths.php --totalonly | grep -oE '[0-9]+') > /dev/null 2>&1",
			user => mwdeploy,
			ensure => present;
	}
}
